...
@Controller
@RequestMapping(value = "/report/pac04")
public class PaReportC04Ctroller extends SqlMapClientSupport {
	@Autowired
	...
	public void exportDatilyReport(HttpServletRequest request,
		...
		String CpnyId = admin.getCpnyId();
		String servletPath = request.getRealPath("");
		String reportName = StringUtil.checkNull(request
				.getParameter("reportName"));
	
		String suffix = StringUtil.checkNull(request.getParameter("suffix")
				.toLowerCase());
		...
		if (!reportName.equals("") && !suffix.equals("")) {
			...
			String filePath = servletPath + "/resources/reportFile/" + CpnyId
					+ "/painfo" + CpnyId + "." + suffix;
	...
	public String createPDFByXmlName1(AdminBean admin,
		...
		try {
			...
			LinkedHashMap paramMap = ObjectBindUtil.getRequestParamData(
					request, "seach_");
			if("insurdetail".equals(request.getParameter("reportName")) ){
			//子报表达的路径   默认的名称
			...
			if (request.getParameter("id") == "11"
					|| "11".equals(request.getParameter("id"))) {
				paramMap.put("PERSON_ID", request
						.getParameter("dwz.person.personId"));
				paramMap.put("GNDR", request.getParameter("dwz.person.sex"));
				paramMap.put("SUBSD_HIRE_DATE", request
						.getParameter("dwz.person.jion"));
				paramMap.put("PROB_END_DATE", request
						.getParameter("dwz.person.end"));
				paramMap.put("JOB_POSI_CD", request
						.getParameter("dwz.person.posision"));
				paramMap.put("JOB_GRADE", request
						.getParameter("dwz.person.emppostGradeName"));
				paramMap.put("JOB_DUTY_CD", request
						.getParameter("dwz.person.duty"));
				paramMap.put("CN_ORG_NM", request
						.getParameter("dwz.person.empDept"));
			}
			if ("".equals(paramMap.get("JOB_TP"))
					|| paramMap.get("JOB_TP") == null) {
				paramMap.put("JOB_TP", "211807");
			}
			paramMap.put("PERSON_ID_JOB", admin.getPersonId());
			if ("211807".equals(paramMap.get("JOB_TP"))
					|| paramMap.get("JOB_TP") == "211807") {
				paramMap.put("JOB_NAME", "全部人员类型");
			}
			if ("".equals(paramMap.get("PROD_TP"))
					|| paramMap.get("PROD_TP") == null) {
			...
			if (paramMap.get("ATT_CD") == null
					|| "".equals(paramMap.get("ATT_CD"))) {
				paramMap.put("ATT_NAME", "全部考勤类型");
			}
			// List mapList =
			// (List)this.paReportC04Ser.retrievePaJasperReportPayrollData(paramMap);
			jasperReport = (JasperReport) JRLoader.loadObject(file);
			// JRDataSource dataSource = new
			// JRBeanCollectionDataSource(mapList);
			// 此处为关键，将对象列表设为数据源
			// jasperPrint = JasperFillManager.fillReport(jasperReport,paramMap,
			// dataSource);
			...
			if ("xls".equals(suffix)) {
				...
				exporter.setParameter(
						JRXlsExporterParameter.CHARACTER_ENCODING, "UTF-8");
			...
			session.getServletContext().setAttribute("FilePath",
					servletPath + "/" + suffix);
			return fileName;
		} catch (JRException e) {
			e.printStackTrace();
	...
	public void exportDatilyReport2(HttpServletRequest request,
		...
		if (!reportName.equals("") && !suffix.equals("")) {
			String reportId = reportName;
			String fileName = createPDFByXmlName2(admin, request, reportId,
					suffix, request.getSession(), response);
			request.getSession().setAttribute("PDFNAME", fileName);
			...
			String filePath = servletPath + "/resources/reportFile/" + CpnyId
					+ "/" + reportName + "." + suffix;
			/*
			 * if("xls".equals(suffix)){//导出excel时才加密 String passWord =
			...
			response.setHeader("Content-Disposition", "attachment;filename="
					+ reportName + "." + suffix);
			/*
			 * 为特殊时间的工资报表PDF 写死的日期和数据 这样 只能输出PDF格式文件 时间是 201512 和 201601
			 * 这两个日期的对比
			 */
			String fromTime = request.getParameter("date_pro");
			String toTime = request.getParameter("date_now");
			String G_P = request.getParameter("G_P");

			if ("201512".equals(fromTime) && "201601".equals(toTime)) {
				if ("G".equals(G_P)) {
					filePath = servletPath + "/resources/reportFile/TSTO/G.pdf";
					response.setHeader("Content-Disposition",
							"attachment;filename=G.pdf");
				} else if ("P".equals(G_P)) {
					filePath = servletPath + "/resources/reportFile/TSTO/P.pdf";
					response.setHeader("Content-Disposition",
							"attachment;filename=P.pdf");
				} else {
					filePath = servletPath
							+ "/resources/reportFile/TSTO/ALL.pdf";
					response.setHeader("Content-Disposition",
	...
	public String createPDFByXmlName2(AdminBean admin,
		...
		try {
			...
			String deptNo = paramMap.get("DEPTNO") == null ? "" : paramMap.get(
					"DEPTNO").toString();
			String cnpyId = paramMap.get("CPNY_ID") == null ? "" : paramMap
					.get("CPNY_ID").toString();
			String year = paramMap.get("YEAR") == null ? "2014" : paramMap.get(
					"YEAR").toString();
			String month = paramMap.get("MONTH") == null ? "" : paramMap.get(
					"MONTH").toString();
			// 获取部门对应的大区Code
			String deptName = cycleSer.getDeptNameByDeptNo(request, deptNo,
					cnpyId);
			paramMap.put("PA_MONTH", year + month);
	...
	public void exportDatilyHtmlReport2(HttpServletRequest request,
		...
		if (!reportName.equals("") && !suffix.equals("")) {
			if (suffix.equals("html")) {
				...
				String year = paramMap.get("YEAR") != null ? paramMap.get(
						"YEAR").toString() : "2014";
				String month = paramMap.get("MONTH") != null ? paramMap.get(
						"MONTH").toString() : "07";
				paramMap.put("PA_MONTH", year + month);
				if (paramMap.get("DEPTNO") == null
						&& "".equals(paramMap.get("DEPTNO"))) {
					paramMap.put("DEPTNO", admin.getDeptNo());
				}
				paramMap.put("SUBSD_CD",
						paramMap.get("CPNY_ID") != null ? paramMap.get(
								"CPNY_ID").toString() : admin.getCpnyId());
				if ("".equals(paramMap.get("JOB_TP"))
						|| paramMap.get("JOB_TP") == null) {
					paramMap.put("JOB_TP", "211807");
				} else {
				...
				if (!reportFile.exists()) {
					JasperCompileManager.compileReportToFile(filePath
							+ ".jrxml", filePath + ".jasper");
				}
				JasperReport jasperReport = (JasperReport) JRLoader
						.loadObject(reportFile);
				JasperPrint jasperPrint = JasperFillManager.fillReport(
						jasperReport, paramMap, super.getConnection());
	...
	private LinkedHashMap fillParamsInMapForReportPop(
			HttpServletRequest request, LinkedHashMap paramMap) {
	...
	public void exportDatilyHtmlReport3(HttpServletRequest request,
		...
		if (!reportName.equals("") && !suffix.equals("")) {
			if (suffix.equals("html")) {
				...
				if ("".equals(paramMap.get("JOB_TP"))
					...
					paramMap.put("JOB_NAME", request
							.getParameter("seach_JOB_NAME"));
				}
				paramMap.put("PERSON_ID_JOB", admin.getPersonId());
				if ("211807".equals(paramMap.get("JOB_TP"))
						|| paramMap.get("JOB_TP") == "211807") {
				...
				paramMap.put("PA_MONTH", paramMap.get("YEAR").toString()
						+ paramMap.get("MONTH").toString());
				File reportFile = new File(filePath + ".jasper");
				if (!reportFile.exists()) {
					JasperCompileManager.compileReportToFile(filePath
							+ ".jrxml", filePath + ".jasper");
		...
		// return new ModelAndView("/report/pac04/exportDatilyHtmlReport",
		// modelMap);
	}
	 ...
	 */
	@SuppressWarnings( { "deprecation", "unchecked" })
	@RequestMapping(value = "/hrCardPreview")
	public void hrCardPreview(HttpServletRequest request,
			HttpServletResponse response, ModelMap modelMap) throws Exception {
		AdminBean admin = SessionUtil.getLoginUserFromSession(request);
		String servletPath = request.getRealPath("");
		
		String filePath = servletPath + "/resources/reportFile/HRM/hrmCard/"
...
//		File reportFile = new File("/resources/reportFile/HRM/hrmCard/"
//				+ admin.getCpnyId() + "/hrmCarden.jasper");
//		if (!reportFile.exists()) {
//			JasperCompileManager.compileReportToFile(filePath
//					+ ".jrxml", filePath + ".jasper");
...
//				.loadObject(reportFile);
	...
	public static String createPDFByXmlName(List mapList, Map parameters,
		...
		try {
			...
			exporter.setParameter(JRPdfExporterParameter.CHARACTER_ENCODING,
					"UTF-8");
			exporter.setParameter(JRPdfExporterParameter.OUTPUT_FILE_NAME,
					servletPath + "/resources/reportFile/" + CpnyId + "/"
							+ fileName);
			// 注意此处用的不是JRPdfExporterParameter.OUTPUT_FILE，要用这个，还需新建File
			exporter.exportReport();
			session.getServletContext().setAttribute("FilePath",
	 ...
	 */
	@SuppressWarnings( { "deprecation", "static-access" })
	@RequestMapping(value = "/exportDatilyReport_va")
	public void exportDatilyReport_va(HttpServletRequest request,
			HttpServletResponse response, ModelMap modelMap) throws Exception {
		AdminBean admin = SessionUtil.getLoginUserFromSession(request);
		String CpnyId = admin.getCpnyId();
		String servletPath = request.getRealPath("");
		...
		if (!reportName.equals("") && !suffix.equals("")) {
			String reportId = reportName;
			String fileName = createPDFByXmlName_va(admin, request, reportId,
					suffix, request.getSession(), response);
			request.getSession().setAttribute("PDFNAME", fileName);
			ReportUtil rUtil = new ReportUtil();
			String filePath = servletPath + "/resources/reportFile/" + CpnyId
	...
	@SuppressWarnings( { "unchecked", "deprecation", "rawtypes" })
	public String createPDFByXmlName_va(AdminBean admin,
			HttpServletRequest request, String reportId, String suffix,
			HttpSession session, HttpServletResponse response)
			throws UnsupportedEncodingException, JRException, ServletException,
			ConfigurationException, SQLException {
		...
		try {
			...
			String PARAM = "";
			if ("".equals(paramMap.get("EMPID"))
					|| paramMap.get("EMPID") == null) {
				PARAM = "SELECT EMPID FROM HR_EMPLOYEE WHERE cpny_id = "
						+ "\'"
						+ paramMap.get("SUBSD_CD")
						+ "\'"
						+ " AND  deptno in (SELECT DISTINCT (DEPTNO)  FROM HR_DEPARTMENT WHERE CPNY_ID = "
						+ "\'"
						+ paramMap.get("SUBSD_CD")
						+ "\'"
						+ " CONNECT BY PRIOR DEPTNO = PARENT_DEPT_NO START WITH DEPTNO = "
						+ "\'" + paramMap.get("ORG_ID") + "\')";
			} else {
				// paramMap.put("ORG_ID",
				// request.getParameter("dwz.person.deptNo"));
				PARAM = "\'" + paramMap.get("EMPID").toString() + "\'";
			}
	 ...
	 */
	@SuppressWarnings( { "deprecation", "unchecked" })
	@RequestMapping(value = "/exportDatilyHtmlReport_va")
	public void exportDatilyHtmlReport_va(HttpServletRequest request,
			HttpServletResponse response, ModelMap modelMap) throws Exception {
		AdminBean admin = SessionUtil.getLoginUserFromSession(request);
		String servletPath = request.getRealPath("");
		String reportName = StringUtil.checkNull(request
		...
		if (!reportName.equals("") && !suffix.equals("")) {
			if (suffix.equals("html")) {
				...
				if (request.getParameter("id") == "11"
						|| "11".equals(request.getParameter("id"))) {
					paramMap.put("EMPID", request
							.getParameter("dwz.person.empId"));
				}
				String PARAM = "";
				if ("".equals(paramMap.get("EMPID"))
						|| paramMap.get("EMPID") == null) {
				...
				if ("".equals(paramMap.get("EMPID"))
						|| paramMap.get("EMPID") == null) {
					PARAM = "SELECT EMPID FROM HR_EMPLOYEE WHERE deptno in (SELECT DISTINCT (DEPTNO)  FROM HR_DEPARTMENT WHERE CPNY_ID = "
							+ "\'"
							+ paramMap.get("SUBSD_CD")
							+ "\'"
							+ " CONNECT BY PRIOR DEPTNO = PARENT_DEPT_NO START WITH DEPTNO = "
