...
@Controller
@RequestMapping(value = "/report/pac08")
public class PaReportC08Ctroller extends SqlMapClientSupport {
	@Autowired
	...
	public void exportDatilyReport2(HttpServletRequest request,
		...
		String reportName = StringUtil.checkNull(request.getParameter("reportName"));
		String suffix = StringUtil.checkNull(request.getParameter("suffix").toLowerCase());
		if("211794".equals(JOB_TP)){
	          reportName="payPayDetailListUpdateJnLGEHZ";
		}else {                
			  reportName="payPayDetailListUpdatSwLGEHZ";
		}
		if (!reportName.equals("") && !suffix.equals("")) {
  			String reportId = reportName;
	...
	public void exportDatilyHtmlReport2(HttpServletRequest request,
		...
		if (!reportName.equals("") && !suffix.equals("")) {
			if(suffix.equals("html")){
				...
				String month = paramMap.get("MONTH")!=null?paramMap.get("MONTH").toString():"10";
			    paramMap.put("ATT_MON", year+month);
			    if(paramMap.get("ORG_ID")==null && "".equals(paramMap.get("ORG_ID"))){
				       paramMap.put("ORG_ID",admin.getDeptNo());
				}
