 ...
 * @Description:
 * @Create date: 2012-2-20 下午06:08:05
...
public class Upload {
	...
	public Upload(HttpServletRequest request, HttpServletResponse response) throws ServletException {
		m_totalBytes = 0;
		m_request = request;
		m_response = response;
		rootpath = "";
	...
	private void setFileExname() { // 设置文件扩展名
		...
		for (int i = 0; i < filenum; i++) {
			if (isup[i]) {
				...
				filexname[i] =java.lang.String.valueOf(System.currentTimeMillis())
						+ i;
			}
		}
	}
	...
	public void WriteMdata() { // 写数据
		for (int i = 0; i < filenum; i++) {
			if (isup[i]) {
				try {
					if(filetype[i].equals("docx") || filetype[i].equals("xlsx") || filetype[i].equals("pptx") || filetype[i].equals("doc") || filetype[i].equals("xls") || filetype[i].equals("ppt") 
							|| filetype[i].equals("txt") || filetype[i].equals("pdf")
							|| filetype[i].equals("jpg")){
...
//					oldfilename[i] = new String(temp.getBytes("iso-8859-1"),"utf-8");
//				} catch (Exception e) {
//				}
//			}
//		}
