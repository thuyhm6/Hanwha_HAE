 ...
 * @Description:
 * @Create date: 2012-2-20 下午06:35:18
...
public class uploadpic {
	...
	public uploadpic(HttpServletRequest request, HttpServletResponse response) throws ServletException {
...
//			rootpath = rootpath + "\\resources\\photo\\";
//		}else{
//			rootpath = rootpath + "/resources/photo/";
//		}
		try {
			...
			m_request = request;
			m_response = response;
			rootpath = config.getString("hrm.photo.temp.path");
			setServerOs(); // 服务器OS分析
			if("win".equals(server)){
				rootpath = rootpath + "\\";
			}else{
	...
	private void setOldFileName(String empid) { // 提取原始文件名
		...
		for (int i = 0; i < filenum; i++) {
			if (isup[i]) {
				...
				tmpend = newindex;
				tempstr = empid+".jpg" ;//new String(m_binArray, tmpstart + "filename=\"".length(), tmpend - tmpstart - "filename=\"".length());
