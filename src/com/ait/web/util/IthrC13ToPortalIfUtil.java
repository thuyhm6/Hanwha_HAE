...
public class IthrC13ToPortalIfUtil {
	/**
	 * 生成TXT文件 的目录 
	...
	//C13的att文件
	private static final String[] attC13NameList={
		"DEPT",
		"USERINFO"
	};
	//C13的每天吃饭次数
	private static final String[] eatC13NameList={
		"AREATCOUNT"
	};
	//C13的SAP数据
	private static final String[] sapC13NameList={
		"EMY",
		"FI_SA"
	};
	//C13的EMC数据  （销售）
	private static final String[] emcC13NameList={
		"EMC"
	};
	
	public IthrC13ToPortalIfUtil(String ftpPath,Map<String , List<String[]>> map,String cpnyId,String ftpSapPath,String ftpMdPath,String sapType){
	 ...
	 */
	public List<String> makeTxtForC13Att(){
		Date date = new Date();
		...
		//生成SAP工资信息
		for(String ispName : attC13NameList){
			...
			if(paraNameList!=null && paraNameList.size()>0){
				...
				try {
					...
					ftpUploadFileSample.uploadC13(ispName+dateStr+"001.txt",ftpMdPath);
				} catch (Exception e) {
	 ...
	 * @return  所有生成 txt文件 的结果
	 */
	public List<String> makeTxtForC13Eat(){
		Date date = new Date();
		...
		//生成SAP工资信息
		for(String ispName : eatC13NameList){
			List<String[]> paraNameList = map.get(ispName);
			if(paraNameList!=null && paraNameList.size()>0){
				StringBuilder htmls = new StringBuilder();
				htmls.append("");
				...
				try {
					...
					FtpUploadFileSample ftpUploadFileSample=new FtpUploadFileSample();
					ftpUploadFileSample.uploadC13(dateStr+"("+sapType+")"+".txt",ftpMdPath);
				} catch (Exception e) {
	 ...
	 * @return  所有生成 txt文件 的结果
	 */
	public List<String> makeTxtForC13Sap(){
		Date date = new Date();
		String dateStr = (new SimpleDateFormat("yyyyMMddHHmmss")).format(date);
		...
		//生成SAP工资信息
		for(String ispName : sapC13NameList){
			if(ispName!=null && "EMY".equals(ispName)){
				...
				if(paraNameList!=null && paraNameList.size()>0){
					...
					try {
						...
						FtpUploadFileSample ftpUploadFileSample=new FtpUploadFileSample();
						ftpUploadFileSample.uploadC13(ispName+dateStr1+"01"+dateStrm1+".txt",ftpSapPath);
					} catch (Exception e) {
				...
				if(paraNameList!=null && paraNameList.size()>0){
					...
					try {
						...
						FtpUploadFileSample ftpUploadFileSample=new FtpUploadFileSample();
						ftpUploadFileSample.uploadC13(ispName+dateStr+".txt",ftpSapPath);
					} catch (Exception e) {
	 ...
	 * @return  所有生成 txt文件 的结果
	 */
	public List<String> makeTxtForC13Emc(){
		Date date = new Date();
		String dateStr = (new SimpleDateFormat("yyyyMMdd")).format(date);
		String result = "";
		List<String> resultList = new ArrayList<String>();
		...
		//生成SAP工资信息
		for(String ispName : emcC13NameList){
			List<String[]> paraNameList = map.get(ispName);
			if(paraNameList!=null && paraNameList.size()>0){
				StringBuilder htmls = new StringBuilder();
				htmls.append("");
