...
@Component
public class SelectDeptListInTableTag extends RequestContextAwareTag {
	...
	public int doStartTagInternal() throws JspException {
		...
		try {
			...
			scriptStr.append(" var oldDept_" + sysLong + ";");
			scriptStr.append(" var ajaxGet_" + sysLong + ";");
			scriptStr.append(" oldDept_" + sysLong + " = '';");
			scriptStr.append(" function  ajaxAdd_" + sysLong + "(obj) { ");
			scriptStr.append("  var newDept_" + sysLong + " = obj.value+$(\":input[sysLong='" + id + "']\").attr('value');");
			scriptStr.append("  if(newDept_" + sysLong + " != oldDept_" + sysLong + " ){");
			scriptStr.append("  	$(\":input[sysLong='" + id + "']\").val(''); ");
			scriptStr.append(" 		oldDept_" + sysLong + " = newDept_" + sysLong + ";");
			scriptStr.append("  }else{");
			scriptStr.append(" 		if(oldDept_" + sysLong + " == ''){");
			scriptStr.append(" 		 	oldDept_" + sysLong + " = newDept_" + sysLong + ";");
			scriptStr.append("  	}");
			scriptStr.append("  }");
			...
			//生成INPUT和隐藏字段
			scriptStr.append("<input id=\"" + name + "\" name=\"" + name + "\" type=\"hidden\" size=10 value=\"" + selected + "\" sysLong=\"" + id + "\"/>");
			scriptStr.append("<input id=\"" + id + "\" name=\"seach_deptName\" type=\"text\" onkeyup=\"ajaxAdd_" + sysLong + "(this)\" onfocus=\"ajaxAdd_" + sysLong + "()\"/>");
