...
@Component
public class EvsCodeMulti extends RequestContextAwareTag {
	...
	protected String resumeSeq = null;
	...
	public int doStartTagInternal() throws JspException {
		...
		if (resumeSeq != null) {
			resumeSeq = eval("resumeSeq", resumeSeq, Object.class).toString();
		} else {
			resumeSeq = "";
		}
		...
		paramMap.put("RESUME_SEQ",resumeSeq);

		List codeList = null;
		if("EVS_GROUP".equals(limit)){
			paramMap.put("EVS_PARAM","GROUP");
			codeList = hrmDao.getDeptTree("hrm.getEvsParamByResume",paramMap);
		}else if("EVS_OCC_GROUP".equals(limit)){
			paramMap.put("EVS_PARAM","FAMILY");
			codeList = hrmDao.getDeptTree("hrm.getEvsParamByResume",paramMap);
		}else if("EVS_DEPT1".equals(limit)){
			paramMap.put("AFFIRM_LEVEL",1);
			codeList = hrmDao.getDeptTree("hrm.getEvsDeptnoByResume",paramMap);
		}else if("EVS_DEPT2".equals(limit)){
			paramMap.put("AFFIRM_LEVEL",2);
			codeList = hrmDao.getDeptTree("hrm.getEvsDeptnoByResume",paramMap);
		}else if("EVS_DEPT3".equals(limit)){
			paramMap.put("AFFIRM_LEVEL",3);
			codeList = hrmDao.getDeptTree("hrm.getEvsDeptnoByResume",paramMap);
		}else if("EVS_DEPT4".equals(limit)){
			paramMap.put("AFFIRM_LEVEL",4);
			codeList = hrmDao.getDeptTree("hrm.getEvsDeptnoByResume",paramMap);
		}
