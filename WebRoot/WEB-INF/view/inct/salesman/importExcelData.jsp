						...
						if (data.refCurrent ){
							...
							if (typeof($("#searchForm_"+ data.navTabId +"_aa")  != 'undefined' )){
								url = url + (url.indexOf('?') == -1 ? "?" : "&")
									+ $("#searchForm_"+ data.navTabId +"_aa").serialize() + "&navTabId=" + data.navTabId;
							}						
	...
	<center>
		<div>
			...
			<form id="addimgform" method="post"
				...
				<input type="hidden" id="LEAVE_TYPE" name="LEAVE_TYPE" value="${paramMap.LEAVE_TYPE}" />
				<input type="hidden" id="REGISTER_SEQ" name="REGISTER_SEQ" value="${paramMap.REGISTER_SEQ}" />
				<input type="hidden" id="ACTIVITY_SEQ" name="ACTIVITY_SEQ" value="${paramMap.ACTIVITY_SEQ}" />
				<input type="hidden" id="RESUME_NO" name="RESUME_NO" value="${paramMap.RESUME_NO}" />
