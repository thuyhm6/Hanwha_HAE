...
<!--
function validateAffirmLeaveApplyCallback_ess0227(form,callback,flag) {
	var $form=null;
	if($('#'+form).length>0)
		$form=$('#'+form);
	else
    ...
    $form.attr("action","/ess/affirmApply/approveLeaveApplyInBatch?AFFIRM_FLAG="+flag);
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchLeaveApplyAffirmForm_ess0227");
...
function pageFromSea(a){                         
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", "/ess/affirmApply/viewLikeLeaveAffirmList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_START_DATE="+seach_START_DATE+"&seach_END_DATE="+seach_END_DATE+"&seach_STATUS_CODE="+seach_STATUS_CODE+"&seach_APPLY_TYPE_CODE="+seach_APPLY_TYPE_CODE);
}
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/affirmApply/viewLikeLeaveAffirmList" method="post" 
	      name="searchLeaveApplyAffirmForm" id="searchLeaveApplyAffirmForm_ess0227" rel="pagerForm">
	<input type="hidden" id="menuNo" name="menuNo" value="124826"/>
	<div class="searchBar">
				...
				<li>
					<div class="subBar">
						    <div class="buttonActive">
						         <a class="update" onclick="validateAffirmLeaveApplyCallback_ess0227('updateLeaveApplyAffirmForm_ess0227',DWZ.ajaxDone,'1')" href="#" >
						         <span><spring:message code="ess.title.passInBatch"/><!--批量通过--></span></a>
                            </div>
						    <div class="buttonActive">
						         <a class="update" onclick="validateAffirmLeaveApplyCallback_ess0227('updateLeaveApplyAffirmForm_ess0227',DWZ.ajaxDone,'2')" href="#" >
...
<form name="updateLeaveApplyAffirmForm" id="updateLeaveApplyAffirmForm_ess0227" method="post" action="/ess/affirmApply/approveLeaveApplyInBatch" 
	  onsubmit="return validateAffirmLeaveApplyCallback_ess0227(this, navTabAjaxDone);">    
	<table class="table" width="100%" height="80%" layoutH="150" nowrapTD="false">
		<thead>
			<tr>
				...
				<th width="120"><spring:message code="heran.ess.viewLikeLeaveApplyInfo.shenqingshijian"/><!--申请时间--></th>
				<th width="160"><spring:message code="heran.ess.viewLikeLeaveApplyInfo.bancitime"/><!--班次时段--></th>				
				...
				<th width="40"><spring:message code="pa.ins.alert.message.exportdata.fileAdd"/><!--附件--></th>									
			</tr>
		</thead>
		...
		<tbody>
			<c:forEach items="${leaveApplyAffirmList}" var="leaveApplyAffirm" varStatus="i">			
				<tr target="sid">
					...
					<td>${fn:substring(leaveApplyAffirm.APPLY_TIME,0,10)}</td>
					<td>
			    	    <dt style="padding: 1px;">
                            ${fn:substring(leaveApplyAffirm.SHIFT_START_TIME,0,16)}
						</dt>
			    	    <dt style="padding: 1px;">
                            ${fn:substring(leaveApplyAffirm.SHIFT_END_TIME,0,16)}
						</dt>												
                    </td>
					<td>${fn:substring(leaveApplyAffirm.APPLY_TYPE_NAME,0,10)}</td>
					<td>
			    	    <dt style="padding: 1px;">
                            ${fn:substring(leaveApplyAffirm.LEAVE_FROM_TIME,0,10)}
						</dt>
			    	    <dt style="padding: 1px;">
			                            ...
			                            <c:choose>
									        <c:when test="${affirmer.AFFIRMOR_ID == leaveApplyAffirm.CURRENT_AFFIRM_ID && affirmer.AFFIRM_LEVEL == leaveApplyAffirm.AFFIRM_LEVEL }">									
										         <c:if test="${affirmer.HREF_FLAG == 1}" >
												    <a href="/ess/affirmApply/approveLeaveApply?ESS_AFFIRM_NO=${affirmer.ESS_AFFIRM_NO}&&AFFIRM_FLAG=1&navTabId=ess0227" target="ajaxTodo">
							                           <span style="color:blue;">&nbsp;<spring:message code="ess.viewApply.title.pass"/><!--通过--></span>
													</a>
												    <a href="/ess/affirmApply/approveLeaveApply?ESS_AFFIRM_NO=${affirmer.ESS_AFFIRM_NO}&&AFFIRM_FLAG=2&navTabId=ess0227" target="ajaxTodo">
					...
					<td>
						<c:if test="${!empty leaveApplyAffirm.FILE_URL }">
							<a href="/ess/infoApply/downloadFile?fileName=${leaveApplyAffirm.FILE_URL }&file=${leaveApplyAffirm.FILE_NAME}" ><spring:message code="pa.ins.alert.message.button.dolowdFile"/></a>
						</c:if>
					</td>		                    														
				</tr>			
			</c:forEach>			
	...
	<c:set value="/ess/affirmApply/viewLeaveAffirmList" var="pageUrl"/>
