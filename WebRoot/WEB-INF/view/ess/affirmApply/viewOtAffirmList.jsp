...
function pageFromSea(a){  
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", 
			"/ess/affirmApply/viewOtAffirmList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_FROM_TIME="+seach_FROM_TIME
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/affirmApply/viewOtAffirmList" method="post" 
	      rel="pagerForm" name="viewOtAffirmList" id="viewOtAffirmList">
		<div class="searchBar">
...
<div class="pageContent" > 
	<div class="formBar">
		<ul>
			<li>
			    <div class="buttonActive">
			         <a class="update" onclick="validateAffirmOtApplyCallback('updateOtApplyAffirmForm',DWZ.ajaxDone,'1')" href="#" >
			         <span><spring:message code="ess.title.passInBatch"/><!--批量通过--></span></a>
				</div>
			    <div class="buttonActive">
			         <a class="update" onclick="validateAffirmOtApplyCallback('updateOtApplyAffirmForm',DWZ.ajaxDone,'2')" href="#" >
			         <span><spring:message code="ess.title.rejectInBatch"/><!--批量否决--></span></a>
                  	</div>	
			</li>
		</ul>
	</div>
	<form name="updateOtApplyAffirmForm" id="updateOtApplyAffirmForm" method="post" action="/ess/affirmApply/approveOvertimeApplyInBatch" 
		...
		<table class="table" width="100%" height="80%" layoutH="231" nowrapTD="false">
			<thead>
				<tr>
					...
					<th width="80" style="text-align: center"><!--加班日期-->
						加班日期
					</th>		
					...
					<c:if test="${defaultCpny ne 'AIT01' }">	
						<th width="120" style="text-align: center"><!--加班时间段-->
							加班时间段
						</th>
			...
			<tbody>
				<c:forEach items="${otApplyList}" var="otPerson" varStatus="i">			
					<tr target="sid">
						...
						<c:if test="${defaultCpny eq 'AIT01' }">
							<td style="text-align: center">
								${otPerson.OT_APPLY_HOUR}小时${otPerson.OT_APPLY_MINUTE}分
							</td>
						</c:if>
						
						<td style="text-align: center">
							<a rel="otAffirmRemark" href="/ess/infoApply/viewApplyContentInfo?seach_APPLY_NO=${otPerson.APPLY_NO}" title="加班事由"
					          target="dialog" mask="true" width="300" height="300" id="otAffirmRemarkHref" >${otPerson.INTRO}</a>
						</td>
						<td style="text-align: center">
							<c:if test="${otPerson.AFFIRM_PROGRESS_FLAG==-1}" >
							    <font color="grey">未提交</font>
							</c:if>
							<c:if test="${otPerson.AFFIRM_PROGRESS_FLAG==0}" >
							    <font color="blue">未审批</font>
							</c:if>	
							<c:if test="${otPerson.AFFIRM_PROGRESS_FLAG==1}" >
							    <font color="green">已通过</font>
							</c:if>	
							<c:if test="${otPerson.AFFIRM_PROGRESS_FLAG==2}" >
							    <font color="red">已否决</font>
							</c:if>	
							<c:if test="${otPerson.AFFIRM_PROGRESS_FLAG==3}" >
							    <font color="back">已取消</font>
							</c:if>	
							<c:if test="${otPerson.AFFIRM_PROGRESS_FLAG==4}" >
							    <font color="green">审批中</font>
							</c:if>		
						</td>
						<td style="text-align: center">
						    <c:if test="${otPerson.CURRENT_AFFIRM_ID eq otPerson.AFFIRMOR_ID}">
						    	<a class="add" href="/ess/affirmApply/viewFullApplyAffirmorList?seach_APPLY_NO=${otPerson.APPLY_NO}"  title="审批加班"
									target="navTab"><font color="red">审批</font></a>
							    <%-- 
							    <a rel="otAffirmRemark" href="/ess/affirmApply/viewFullApplyAffirmorList?seach_APPLY_NO=${otPerson.APPLY_NO}" title="审批"
					          		target="navTab" mask="true" width="1050" height="450" id="otAffirmRemarkHref"><font color="red">审批</font></a>
					          	--%>
							</c:if>
							<c:if test="${otPerson.CURRENT_AFFIRM_ID ne otPerson.AFFIRMOR_ID}" >
							    <a rel="otAffirmRemark" href="/ess/affirmApply/viewFullApplyCheckInfo?seach_APPLY_NO=${otPerson.APPLY_NO}" title="审批详情"
					          		target="dialog" mask="true" width="1250" height="450" id="otAffirmRemarkHref"><font color="red">审批查看</font></a>
							</c:if>
						</td>	                    														
					</tr>			
				</c:forEach>			
	...
	<div id="otAffirmRemark" style="border:10;overflow:auto;bottom:10;right:100;width:300;z-index:1;"></div>
	<c:set value="/ess/affirmApply/viewOtAffirmList" var="pageUrl"/>
