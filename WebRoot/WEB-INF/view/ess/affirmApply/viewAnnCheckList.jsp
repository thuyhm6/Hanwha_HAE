...
function pageFromSea(a){  
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", 
			"/ess/affirmApply/viewAnnCheckList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_FROM_TIME="+seach_FROM_TIME
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/affirmApply/viewAnnCheckList" method="post" 
		...
		<div class="searchBar">
			...
			<tr>
				...
				<td>
					<ait:deptList name="seach_DEPT_NO" limit="ar" id="viewAnnCheckList_seachDept"/>
					<ait:deptTreeIcon name="seach_DEPT_NO" limit="ar" id="viewAnnCheckList_seachDept" selected="${DEPT_NO}"/>
				</td>
				<td width="10%"><!-- 决裁状态 -->
...
<div class="pageContent" >    
	<table class="table" width="100%" height="80%" layoutH="231" nowrapTD="false">
		<thead>
			<tr>
				...
				<th width="100">年假总天数</th>
				<th width="100">已使用天数</th>
				<th width="120">剩余天数</th>
				<th width="160">调整原因</th>				
				<th width="100">调整后总天数</th>				
				<th width="100">调整后剩余天数</th>
				<th width="100">决裁状态</th>					
		...
		<tbody>
			<c:forEach items="${annualadjustmentInfoList}" var="annulist" varStatus="i">			
				 <tr target="sid" rel="${annulist.PERSON_ID}">
					...
					<td style="text-align: center">${annulist.TOT_VAC_CNT1}</td>
					<td style="text-align: center">${annulist.USE_VAC }</td>
					<td style="text-align: center">${annulist.SHENGYU}</td>
					<td style="text-align: center">${annulist.ANNUAL_LEAVE_REASON}</td>
					<td style="text-align: center">${annulist.TIAOZZONG}</td>
					<td style="text-align: center">${annulist.TIAOYU}</td>
					<td style="text-align: center">
						<c:if test="${annulist.AFFIRM_PROGRESS_FLAG==-1}" >
						    <font color="grey">未提交</font>
						</c:if>
						<c:if test="${annulist.AFFIRM_PROGRESS_FLAG==0}" >
						    <font color="blue">未决裁</font>
						</c:if>	
						<c:if test="${annulist.AFFIRM_PROGRESS_FLAG==1}" >
						    <font color="green">已通过</font>
						</c:if>	
						<c:if test="${annulist.AFFIRM_PROGRESS_FLAG==2}" >
						    <font color="red">已否决</font>
						</c:if>	
						<c:if test="${annulist.AFFIRM_PROGRESS_FLAG==3}" >
						    <font color="back">已取消</font>
						</c:if>	
						<c:if test="${annulist.AFFIRM_PROGRESS_FLAG==4}" >
						    <font color="green">决裁中</font>
						</c:if>		
					</td>
					<td style="text-align: center">
						<c:if test="${annulist.AFFIRM_FLAG_PERSON eq '1'}">
							<a class="add" href="/ess/annualadjustment/checkAnnApplyCheckInfo?seach_APPLY_TYPE_NO=216691&seach_APPLY_NO=${annulist.APPLY_NO}&seach_ESS_AFFIRM_NO=${annulist.ESS_AFFIRM_NO}&seach_IS_CHECK=1"  
					    	    title="CHECK" target="dialog" mask="true" width="1200" height="450"><font color="red">Check</font></a>
						</c:if>
						<c:if test="${annulist.AFFIRM_FLAG_PERSON ne '1'}">
						    <a rel="leaveAffirmRemark" href="/ess/annualadjustment/checkAnnApplyCheckInfo?seach_APPLY_TYPE_NO=216691&seach_APPLY_NO=${annulist.APPLY_NO}" title="审批详情"
	...
	<c:set value="/ess/infoApply/viewCwaCheckLis" var="pageUrl"/>
