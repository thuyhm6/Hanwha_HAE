...
<!--
function validateAffirmEvectionApplyCallback(form,callback,flag) {
	var $form=null;
	if($('#'+form).length>0)
		$form=$('#'+form);
	else
    ...
    $form.attr("action","/ess/affirmApply/approveEvectionApplyInBatch?AFFIRM_FLAG="+flag+"&AFFIRM_TYPE=LEAVE_AFFIRM");
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchEvectionApplyAffirmForm");
...
function pageFromSea(a){                         
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", "/ess/affirmApply/viewEvectionAffirmList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_FROM_TIME="+seach_FROM_TIME+"&seach_TO_TIME="+seach_TO_TIME+"&seach_STATUS_CODE="+seach_STATUS_CODE+"&seach_APPLY_TYPE_CODE="+seach_APPLY_TYPE_CODE);
}
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/affirmApply/viewEvectionAffirmList" method="post" 
	      name="searchEvectionApplyAffirmForm" id="searchEvectionApplyAffirmForm" rel="pagerForm">
	<div class="searchBar">
				...
				<li>
					<div class="subBar">
						    <div class="buttonActive">
						         <a class="update" onclick="validateAffirmEvectionApplyCallback('updateEvectionApplyAffirmForm',DWZ.ajaxDone,'1')" href="#" >
						         <span><spring:message code="ess.title.passInBatch"/><!--批量通过--></span></a>
                            </div>
						    <div class="buttonActive">
						         <a class="update" onclick="validateAffirmEvectionApplyCallback('updateEvectionApplyAffirmForm',DWZ.ajaxDone,'2')" href="#" >
...
<form name="updateEvectionApplyAffirmForm" id="updateEvectionApplyAffirmForm" method="post" action="/ess/affirmApply/approveEvectionApplyInBatch" 
	  onsubmit="return validateAffirmEvectionApplyCallback(this, navTabAjaxDone);">    
	<table class="table" width="100%" height="80%" layoutH="150" nowrapTD="false">
		...
		<tbody>
			<c:forEach items="${evectionApplyPersonList}" var="evectionApplyPerson" varStatus="i">			
				<tr target="sid">
				    <td>
				        <input type="checkbox" id="c1" name="c1" value="${evectionApplyPerson.ESS_AFFIRM_NO}" />
				    </td>
					...
					<td>${evectionApplyPerson.APPLY_TYPE_NAME}</td>
					<td>
					    ...
					    <c:forEach items="${evectionApplyPerson.affirmerList}" var="affirmer" varStatus="i">					    					       					      
						        <dt style="padding: 1px;">
							        <c:if test="${affirmer.AFFIRM_FLAG==0 }" >
			                            ...
			                            <c:choose>
									        <c:when test="${affirmer.AFFIRMOR_ID == evectionApplyPerson.CURRENT_AFFIRM_ID && affirmer.AFFIRM_LEVEL == evectionApplyPerson.AFFIRM_LEVEL }">									
										         <c:if test="${affirmer.HREF_FLAG == 1}" >
												    <a href="/ess/affirmApply/approveEvectionApply?ESS_AFFIRM_NO=${affirmer.ESS_AFFIRM_NO}&AFFIRM_FLAG=1&AFFIRM_TYPE=LEAVE_AFFIRM" target="ajaxTodo">
							                           <span style="color:blue;">&nbsp;<spring:message code="ess.viewApply.title.pass"/><!--通过--></span>
													</a>
												    <a href="/ess/affirmApply/approveEvectionApply?ESS_AFFIRM_NO=${affirmer.ESS_AFFIRM_NO}&AFFIRM_FLAG=2&AFFIRM_TYPE=LEAVE_AFFIRM" target="ajaxTodo">
					...
					<td>${evectionApplyPerson.REMARK}</td>		                    														
				</tr>			
			</c:forEach>			
		</tbody>
	</table>	
</form>
	<c:set value="/ess/affirmApply/viewEvectionAffirmList" var="pageUrl"/>
