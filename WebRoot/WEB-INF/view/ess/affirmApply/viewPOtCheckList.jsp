...
function pageFromSea(a){  
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", 
			"/ess/affirmApply/viewPOtCheckList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_FROM_TIME="+seach_FROM_TIME
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/affirmApply/viewPOtCheckList" method="post" 
	      rel="pagerForm" name="viewPOtCheckList" id="viewPOtCheckList">
		<div class="searchBar">
...
<div class="pageContent" >    
		...
		<tbody>
			<c:forEach items="${otCheckList}" var="otCheck" varStatus="i">			
				<tr target="sid">
					...
					<td style="text-align: center">
						<a rel="otCheckRemarkP" href="/ess/infoApply/viewApplyContentInfo?seach_APPLY_NO=${otCheck.APPLY_NO}" title="加班事由"
				          target="dialog" mask="true" width="300" height="300" id="otCheckRemarkPHref" >${otCheck.INTRO}</a>
					</td>
					<td style="text-align: center">
					...
					<td style="text-align: center">
					    <c:if test="${otCheck.CURRENT_CHECKOR_ID ne otCheck.CHECKOR_ID &&otCheck.CHECK_FLAG ne '1'  }">
						...
						<c:if test="${otCheck.APPLY_TYPE ne 'PERSON' &&otCheck.CHECK_FLAG ne '1'   }">
					    	...
					    	<a class="add" href="/ess/affirmApply/checkApplyCheckInfo?seach_APPLY_TYPE_WQ=31&seach_APPLY_NO=${otCheck.APPLY_NO}&seach_ESS_AFFIRM_NO=${otCheck.ESS_AFFIRM_NO}"  
					    		title="CHECK" target="dialog" mask="true" width="1200" height="450"><font color="red">Check</font></a>
						</c:if> <c:if test="${otCheck.CHECK_FLAG eq '1' }">
					    已Check
					 </c:if>
					</td>	                    														
				</tr>			
	...
	<div id="otCheckRemarkP" style="border:10;overflow:auto;bottom:10;right:100;width:300;z-index:1;"></div>
	<c:set value="/ess/affirmApply/viewPOtCheckList" var="pageUrl"/>
