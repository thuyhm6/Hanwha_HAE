...
$(document).ready(function(){
	...
	//查询
	$("#viewShopShiftSHList_Serch",navTab.getCurrentPanel()).click(function(){
		$("#viewShopShiftSHListForm",navTab.getCurrentPanel()).submit();
	});
	//上星期
	$("#viewShopShiftSHList_up",navTab.getCurrentPanel()).click(function(){
	...
	//下星期
	$("#viewShopShiftSHList_down",navTab.getCurrentPanel()).click(function(){
	...
	//保存
	$("#viewShopShiftSHList_Save",navTab.getCurrentPanel()).click(function(){	
		//获取页面的值
		var jsonData = '[';
		$("[sysLog='modifyFlag']",navTab.getCurrentPanel()).each(function(i, obj){
			if($(obj).html() == "modify"){
				...
				jsonData += ' "ITEM_NO": "' + $("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem1") + '" ,';
				jsonData += ' "ITEM_LEN_1": "' + $("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysLen1") + '" ,';
				jsonData += ' "ITEM_NO_2": "' + $("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem2") + '" ,';
				jsonData += ' "ITEM_LEN_2": "' + $("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysLen2") + '" ,';
		...
		alertMsg.confirm("确定要保存吗？",
			  	...
			  	$.ajax({
	  				type: 'POST',
					url: '/ess/tempEmp/addShopShiftSH?typeFlag=shop',
	...
	$("#viewShopShiftSHList_fill",navTab.getCurrentPanel()).click(function(){
		...
		$("input[name='BATCH_SHOP_SHIFT']",navTab.getCurrentPanel()).each(function(i, obj){
			if(obj.checked){
				...
				$("#ITEM_NO_" + week + "_" + index,navTab.getCurrentPanel()).attr("sysItem1",item);
			    $("#ITEM_NO_" + week + "_" + index,navTab.getCurrentPanel()).attr("sysItem2","");
			    $("#ITEM_NO_" + week + "_" + index,navTab.getCurrentPanel()).attr("sysLen1","");
			    $("#ITEM_NO_" + week + "_" + index,navTab.getCurrentPanel()).attr("sysLen2","");
...
function initEditFun_ess3441(){
			...
			if(val != ''){
				if(sysType != 'MINUS_LENGTH'){
					...
					if(minusLen >= 0){
						...
						$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem1","正常出勤");
					}else{
						$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).html( "早退" );
						$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem1","早退");
					}
	
					$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem2","");
					$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysLen1","");
					$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysLen2","");
				}
			}
	...
	$('.orderList tbody tr td:[sysLog="select"]',navTab.getCurrentPanel()).editable({type:'select',
		onblur:function(val,settings){
			...
			$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem1",val);
			$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem2","");
			$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysLen1","");
			$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysLen2","");
			...
			if(val == '多状态'){
				$(this).html("");
				$("#ITEM_NO_" + sysWeek + "_" + index,navTab.getCurrentPanel()).attr("sysItem1","");
				$.pdialog.open(encodeURI("/ess/tempEmp/viewSHItemList?seach_index=" + index + "&seach_sysWeek=" + sysWeek + "&seach_timeLimit=7"), "shopShift", "考勤", {width:500,height:270,mask:true});
			}else if(val == '休息'){
				$("td[sysWeek='" + sysWeek + "'][sysIndex='" + index + "']",navTab.getCurrentPanel()).css("background-color","red");
			}else{
...
function excelimport_ess3441(){
	$("#importExcelDialog_ess3441").attr('href','/pa/excelImport/importExcelData?importFunName=/importShopShiftTemp&TYPE=ess3441');
	$("#importExcelDialog_ess3441").click();
}
</script>
<a id="importExcelDialog_ess3441"  href="#" target="dialog" mask="true"></a>
<a id="importExcel_ess3441" href="#" target="navTab" mask="true"><span style="display:none;"><!-- 导入结果 --><spring:message code="ess.title.DAORUJIEGUO" /></span></a>
<div class="pageHeader">
<form id="viewShopShiftSHListForm" onsubmit="return navTabSearch(this);" action="/ess/tempEmp/viewShopShiftSHList" method="post">
<div class="searchBar">
<table class="searchContent">
	<tr>
		...
		<td>
			<ait:deptList name="seach_DEPTNO" limit="manager" id="viewShopShiftSHList_deptList" />
			<ait:deptTreeIcon name="seach_DEPTNO" limit="manager" id="viewShopShiftSHList_deptList" selected="${DEPTNO}"/>
		</td>
		<td><!--考勤日期--><spring:message code="ess.infoApply.attendance_date" /></td>
		<td>
			<input type="text" id="DDATE_STR" name="DDATE_STR" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy/MM/dd'})" value="${DDATE_STR}"/>
		</td>
	</tr>
</table>
</div>
...
<div class="pageHeader">
	<div class="searchBar">
		<table class="searchContent">
			<tr>
				<td><!--考勤日--><spring:message code="ess.title.KAOQINRI" /></td>
				<td>
				...
				<td><!--上班时间--><spring:message code="ess.infoApply.work_time" /></td>
				<td>
					<input type="text" id="SHIFT_FROM_TIME_FILL" size="5">
				</td>
				<td><!--下班时间--><spring:message code="ess.infoApply.out_work_time" /></td>
				<td>
					<input type="text" id="SHIFT_TO_TIME_FILL" size="5">
				</td>
				<td><!--工时--><spring:message code="ess.infoApply.working_hours" /></td>
				<td>
					<input type="text" id="WORKING_HOURS_FILL" size="5">
				</td>
				<td><!--差异--><spring:message code="ess.infoApply.difference" /></td>
				<td>
			...
			<div class="subBar">
				<ul class="toolBar">
	             <li>
	            	 <a class="buttonActive" id="viewShopShiftSHList_fill"><span><!--全部反应--><spring:message code="ess.message.all_reaction" /></span></a>
	             </li>
				</ul>
			</div>
	</div>
...
<div class="formBar">
	<ul class="toolBar">
		<li><a class="buttonActive" id="viewShopShiftSHList_Serch" href="#"><span><!--查询--><spring:message code="org.title.SELECT" /></span></a></li>
		<li><a class="buttonActive" id="viewShopShiftSHList_up" href="#"><span><!--上星期--><spring:message code="ess.message.last_week" /></span></a></li>
		<li><a class="buttonActive" id="viewShopShiftSHList_down" href="#"><span><!--下星期--><spring:message code="ess.message.next_week" /></span></a></li>
		...
		<li><a class="buttonActive" href="#" onclick="excelimport_ess3441();"><span><!--Excel导入--><spring:message code="ess.infoApply.EXCEL_IN" /></span></a></li>
		<li><a class="buttonActive" href="#" onclick="downloadExcel('viewShopShiftSHListForm','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=265','/ess/tempEmp/viewShopShiftSHList')"><span><!--导出到Excel--><spring:message code="hrm.empinfo.EXPORT" /></span></a></li>
		<li><a class="buttonActive" id="viewShopShiftSHList_Save" href="#"><span><!--保存--><spring:message code="ar.viewempcalender.title.save" /></span></a></li>
	</ul>
</div>
<div class="pageContent">
				<table class="orderList" width="2660px">
					<thead>
						<tr>
							<th width="20px" rowspan="2">NO.</th>
							<th width="60px" rowspan="2"><!--社号--><spring:message code="ess.infoApply.EMPID" /></th>
							<th width="60px" rowspan="2"><!--姓名--><spring:message code="ess.infoApply.NAME" /></th>
							...
							<c:forEach items="${viewWeekList}" var="item" varStatus="i">
								<th width="190px" colspan="5" id="changeFlag_${i.index }" sysDateStr="${item.DDATE_STR }">
							...
							<th width="560px" colspan="4"><!--汇总工时--><spring:message code="ess.title.HUIZONGGONGSHI" />((<!--额定工时--><spring:message code="ess.title.EDINGGONGSHI" />)(<!--实际工时--><spring:message code="ess.title.SHIJIGONGSHI" />)(<!--差异--><spring:message code="ess.infoApply.difference" />))</th>
						</tr>
						<tr>
							<c:forEach items="${viewWeekList}" var="item" varStatus="i">
									<c:if test="${item.TYPEID eq 1442}">
										...
										<th width="40px"><font color="red"><!--工时--><spring:message code="ess.infoApply.working_hours" /></font></th>
										<th width="40px"><font color="red"><!--差异--><spring:message code="ess.infoApply.difference" /></font></th>
										<th width="70px"><font color="red"><!--考勤状态--><spring:message code="ess.infoApply.attendState" /></font></th>
									</c:if>
									<c:if test="${item.TYPEID ne 1442}">
										...
										<th width="40px"><!--工时--><spring:message code="ess.infoApply.working_hours" /></th>
										<th width="40px"><!--差异--><spring:message code="ess.infoApply.difference" /></th>
										<th width="70px"><!--考勤状态--><spring:message code="ess.infoApply.attendState" /></th>
									</c:if>
							</c:forEach>
							<th width="140px">${viewWeekList[0].AR_MONTH}</th>
							<th width="140px">${viewWeekList[0].AR_MONTH + 1}<!--月--><spring:message code="hrm.empinfo.MONTH" /></th>
							<th width="140px">${viewWeekList[0].AR_MONTH + 2}<!--月--><spring:message code="hrm.empinfo.MONTH" /></th>
							<th width="140px"><!--当季合计--><spring:message code="ess.title.DANGJIHEJI" /></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${viewShopShiftSHList}" var="item" varStatus="i">
							<tr target="SEQ" rel="${item.SEQ}">
							    ...
							    <td style="text-align: center">
							 	...
							 	<td class='td_center' sysWeek="1" sysActivity="${item.ACTIVITY_1}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_1 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_1 }" sysLen1="${item.ITEM_LEN_1_1 }" sysItem2="${item.ITEM_NO_2_1 }" sysLen2="${item.ITEM_LEN_2_1 }" id="ITEM_NO_1_${i.index}">${item.ITEM_NO_1}<c:if test="${not empty item.ITEM_NO_2_1}"> ${item.ITEM_LEN_1_1}<br/>${item.ITEM_NO_2_1} ${item.ITEM_LEN_2_1}</c:if><c:if test="${item.ACTIVITY_1 eq '2'}"><br/><!--审批中--><spring:message code="ess.affirmApply.title.remark.shenpizhong" /></c:if></td>
							 	...
							 	<td class='td_center' sysWeek="2" sysActivity="${item.ACTIVITY_2}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_2 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_2 }" sysLen1="${item.ITEM_LEN_1_2 }" sysItem2="${item.ITEM_NO_2_2 }" sysLen2="${item.ITEM_LEN_2_2 }" id="ITEM_NO_2_${i.index}">${item.ITEM_NO_2}<c:if test="${not empty item.ITEM_NO_2_2}"> ${item.ITEM_LEN_1_2}<br/>${item.ITEM_NO_2_2} ${item.ITEM_LEN_2_2}</c:if><c:if test="${item.ACTIVITY_2 eq '2'}"><br/><!--审批中--><spring:message code="ess.affirmApply.title.remark.shenpizhong" /></c:if></td>
							 	...
							 	<td class='td_center' sysWeek="3" sysActivity="${item.ACTIVITY_3}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_3 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_3 }" sysLen1="${item.ITEM_LEN_1_3 }" sysItem2="${item.ITEM_NO_2_3 }" sysLen2="${item.ITEM_LEN_2_3 }" id="ITEM_NO_3_${i.index}">${item.ITEM_NO_3}<c:if test="${not empty item.ITEM_NO_2_3}"> ${item.ITEM_LEN_1_3}<br/>${item.ITEM_NO_2_3} ${item.ITEM_LEN_2_3}</c:if><c:if test="${item.ACTIVITY_3 eq '2'}"><br/><!--审批中--><spring:message code="ess.affirmApply.title.remark.shenpizhong" /></c:if></td>
							 	...
							 	<td class='td_center' sysWeek="4" sysActivity="${item.ACTIVITY_4}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_4 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_4 }" sysLen1="${item.ITEM_LEN_1_4 }" sysItem2="${item.ITEM_NO_2_4 }" sysLen2="${item.ITEM_LEN_2_4 }" id="ITEM_NO_4_${i.index}">${item.ITEM_NO_4}<c:if test="${not empty item.ITEM_NO_2_4}"> ${item.ITEM_LEN_1_4}<br/>${item.ITEM_NO_2_4} ${item.ITEM_LEN_2_4}</c:if><c:if test="${item.ACTIVITY_4 eq '2'}"><br/><!--审批中--><spring:message code="ess.affirmApply.title.remark.shenpizhong" /></c:if></td>
							 	...
							 	<td class='td_center' sysWeek="5" sysActivity="${item.ACTIVITY_5}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_5 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_5 }" sysLen1="${item.ITEM_LEN_1_5 }" sysItem2="${item.ITEM_NO_2_5 }" sysLen2="${item.ITEM_LEN_2_5 }" id="ITEM_NO_5_${i.index}">${item.ITEM_NO_5}<c:if test="${not empty item.ITEM_NO_2_5}"> ${item.ITEM_LEN_1_5}<br/>${item.ITEM_NO_2_5} ${item.ITEM_LEN_2_5}</c:if><c:if test="${item.ACTIVITY_5 eq '2'}"><br/><!--审批中--><spring:message code="ess.affirmApply.title.remark.shenpizhong" /></c:if></td>
							 	...
							 	<td class='td_center' sysWeek="6" sysActivity="${item.ACTIVITY_6}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_6 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_6 }" sysLen1="${item.ITEM_LEN_1_6 }" sysItem2="${item.ITEM_NO_2_6 }" sysLen2="${item.ITEM_LEN_2_6 }" id="ITEM_NO_6_${i.index}">${item.ITEM_NO_6}<c:if test="${not empty item.ITEM_NO_2_6}"> ${item.ITEM_LEN_1_6}<br/>${item.ITEM_NO_2_6} ${item.ITEM_LEN_2_6}</c:if><c:if test="${item.ACTIVITY_6 eq '2'}"><br/><!--审批中--><spring:message code="ess.affirmApply.title.remark.shenpizhong" /></c:if></td>
							 	...
							 	<td class='td_center' sysWeek="0" sysActivity="${item.ACTIVITY_0}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_0 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_0 }" sysLen1="${item.ITEM_LEN_1_0 }" sysItem2="${item.ITEM_NO_2_0 }" sysLen2="${item.ITEM_LEN_2_0 }" id="ITEM_NO_0_${i.index}">${item.ITEM_NO_0}<c:if test="${not empty item.ITEM_NO_2_0}"> ${item.ITEM_LEN_1_0}<br/>${item.ITEM_NO_2_0} ${item.ITEM_LEN_2_0}</c:if><c:if test="${item.ACTIVITY_0 eq '2'}"><br/><!--审批中--><spring:message code="ess.affirmApply.title.remark.shenpizhong" /></c:if></td>
								
								<td class='td_center'><!--额--><spring:message code="ess.title.E" />:${item.CALENDER0 }  <!--实--><spring:message code="ess.title.SHI" />:${item.DETAIL0 }  <!--差--><spring:message code="ess.title.CHA" />:${item.DETAIL0 - item.CALENDER0 }</td>
								<td class='td_center'><!--额--><spring:message code="ess.title.E" />:${item.CALENDER1 }  <!--实--><spring:message code="ess.title.SHI" />:${item.DETAIL1 }  <!--差--><spring:message code="ess.title.CHA" />:${item.DETAIL1 - item.CALENDER1 }</td>
								<td class='td_center'><!--额--><spring:message code="ess.title.E" />:${item.CALENDER2 }  <!--实--><spring:message code="ess.title.SHI" />:${item.DETAIL2 }  <!--差--><spring:message code="ess.title.CHA" />:${item.DETAIL2 - item.CALENDER2 }</td>
								<td class='td_center'><!--额--><spring:message code="ess.title.E" />:${item.CALENDER0 + item.CALENDER1 + item.CALENDER2 }  <!--实--><spring:message code="ess.title.SHI" />:${item.DETAIL0 + item.DETAIL1 + item.DETAIL2 }   <!--差--><spring:message code="ess.title.CHA" />:${item.DETAIL0 + item.DETAIL1 + item.DETAIL2 - (item.CALENDER0 + item.CALENDER1 + item.CALENDER2)}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
