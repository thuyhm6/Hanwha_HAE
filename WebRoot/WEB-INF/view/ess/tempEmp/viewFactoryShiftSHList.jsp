...
$(document).ready(function(){
	...
	//查询
	$("#viewFactoryShiftSHList_Serch",navTab.getCurrentPanel()).click(function(){
		$("#viewFactoryShiftSHListForm",navTab.getCurrentPanel()).submit();
	});
	//上星期
	$("#viewFactoryShiftSHList_up",navTab.getCurrentPanel()).click(function(){
	...
	//下星期
	$("#viewFactoryShiftSHList_down",navTab.getCurrentPanel()).click(function(){
	...
	//保存
	$("#viewFactoryShiftSHList_Save",navTab.getCurrentPanel()).click(function(){	
		//获取页面的值
		var jsonData = '[';
		$("[sysLog='modifyFlag']",navTab.getCurrentPanel()).each(function(i, obj){
			if($(obj).html() == "modify"){
		...
		alertMsg.confirm("确定要保存吗？",
			  	...
			  	$.ajax({
	  				type: 'POST',
					url: '/ess/tempEmp/addShopShiftSH?typeFlag=factory',
	...
	$("#viewFactoryShiftSHList_fill",navTab.getCurrentPanel()).click(function(){
		...
		if(sysType == 'SHIFT_FROM_TIME_FILL'){
			if(val != ''){
				if($("#SHIFT_TO_TIME_FILL",navTab.getCurrentPanel()).val() == ''){
					$("#SHIFT_TO_TIME_FILL",navTab.getCurrentPanel()).val( addTime(val, 8.5) );
					minusLen = 0;
					workingHours = 8;
				}else{
					var shiftToTime = $("#SHIFT_TO_TIME_FILL",navTab.getCurrentPanel()).val();
					minusLen = addTime2(shiftToTime, val) - 8.5;
					workingHours = addTime2(shiftToTime, val) - 0.5;
				}
			}
		}else if(sysType == 'SHIFT_TO_TIME_FILL'){
			if(val != ''){
				if($("#SHIFT_FROM_TIME_FILL",navTab.getCurrentPanel()).val() == ''){
					$("#SHIFT_FROM_TIME_FILL",navTab.getCurrentPanel()).val( addTime3(val, 8.5) );
					minusLen = 0;
					workingHours = 8;
				}else{
					var shiftFromTime = parseFloat($("#SHIFT_FROM_TIME_FILL",navTab.getCurrentPanel()).val());
					minusLen = addTime2(val, shiftFromTime) - 8.5;
					workingHours = addTime2(val, shiftFromTime) - 0.5;
				}
			}
		}
		if(val != ''){
			...
			if(sysType == 'SHIFT_FROM_TIME'){
				if(val != ''){
					if($("#SHIFT_TO_TIME_" + sysWeek + "_" + index,navTab.getCurrentPanel()).html() == ''){
						$("#SHIFT_TO_TIME_" + sysWeek + "_" + index,navTab.getCurrentPanel()).html( addTime(val, 8.5) );
						minusLen = 0;
						workingHours = 8;
					}else{
				...
				if(val != ''){
					if($("#SHIFT_FROM_TIME_" + sysWeek + "_" + index,navTab.getCurrentPanel()).html() == ''){
						$("#SHIFT_FROM_TIME_" + sysWeek + "_" + index,navTab.getCurrentPanel()).html( addTime3(val, 8.5) );
						minusLen = 0;
						workingHours = 8;
					}else{
	...
	$('.orderList tbody tr td:[sysLog="select"]',navTab.getCurrentPanel()).editable({type:'select',
		onblur:function(val,settings){
			...
			if(val == '多状态'){
				...
				$.pdialog.open(encodeURI("/ess/tempEmp/viewSHItemList?seach_index=" + index + "&seach_sysWeek=" + sysWeek + "&seach_timeLimit=8"), "shopShift", "考勤", {width:500,height:270,mask:true});
			}else if(val == '休息'){
				$("td[sysWeek='" + sysWeek + "'][sysIndex='" + index + "']",navTab.getCurrentPanel()).css("background-color","red");
			}else{
...
function excelimport_ess3442(){
	$("#importExcelDialog_ess3442").attr('href','/pa/excelImport/importExcelData?importFunName=/importShopShiftTemp&TYPE=ess3442');
	$("#importExcelDialog_ess3442").click();
}
</script>
<a id="importExcelDialog_ess3442"  href="#" target="dialog" mask="true"></a>
<a id="importExcel_ess3442" href="#" target="navTab" mask="true"><span style="display:none;">导入结果</span></a>
<div class="pageHeader">
<form id="viewFactoryShiftSHListForm" onsubmit="return navTabSearch(this);" action="/ess/tempEmp/viewFactoryShiftSHList" method="post">
<div class="searchBar">
<table class="searchContent">
	<tr>
		...
		<td>
			<ait:deptList name="seach_DEPTNO" limit="manager" id="viewFactoryShiftSHList_deptList" />
			<ait:deptTreeIcon name="seach_DEPTNO" limit="manager" id="viewFactoryShiftSHList_deptList" selected="${DEPTNO}"/>
		</td>
	...
	<div class="searchBar">
		<table class="searchContent">
			...
			<div class="subBar">
				<ul class="toolBar">
	             <li>
	            	 <a class="buttonActive" id="viewFactoryShiftSHList_fill"><span><!--全部反应--><spring:message code="ess.message.all_reaction" /></span></a>
	             </li>
				</ul>
			</div>
	</div>
...
<div class="formBar">
	<ul class="toolBar">
		<li><a class="buttonActive" id="viewFactoryShiftSHList_Serch" href="#"><span><!--查询--><spring:message code="org.title.SELECT" /></span></a></li>
		<li><a class="buttonActive" id="viewFactoryShiftSHList_up" href="#"><span><!--上星期--><spring:message code="ess.message.last_week" /></span></a></li>
		<li><a class="buttonActive" id="viewFactoryShiftSHList_down" href="#"><span><!--下星期--><spring:message code="ess.message.next_week" /></span></a></li>
		<li><a class="buttonActive" href="/pa/excelExport/downloadShopShiftExcelTemplate?file=ShopShiftAction_Add&EMP_TYPE=FACTORY"><span><!--模板下载--><spring:message code="ess.message.template_download" /></span></a></li>
		<li><a class="buttonActive" href="#" onclick="excelimport_ess3442();"><span><!--Excel导入--><spring:message code="ess.infoApply.EXCEL_IN" /></span></a></li>
		<li><a class="buttonActive" href="#" onclick="downloadExcel('viewFactoryShiftSHListForm','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=266','/ess/tempEmp/viewFactoryShiftSHList')"><span>Excel导出</span></a></li>
		<li><a class="buttonActive" id="viewFactoryShiftSHList_Save" href="#"><span><!--保存--><spring:message code="ar.viewempcalender.title.save" /></span></a></li>
	</ul>
</div>
				...
				<table class="orderList" width="2660px">
					<thead>
						...
						<tr>
							...
							<th width="140px">${viewWeekList[0].AR_MONTH}<!--月--><spring:message code="hrm.empinfo.MONTH" /></th>
					...
					<tbody>
						<c:forEach items="${viewFactoryShiftSHList}" var="item" varStatus="i">
							<tr target="SEQ" rel="${item.SEQ}">
							    ...
							    <td style="text-align: center">
							 	...
							 	<td class='td_center' sysWeek="2" sysActivity="${item.ACTIVITY_2}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_2 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_2 }" sysLen1="${item.ITEM_LEN_1_2 }" sysItem2="${item.ITEM_NO_2_2 }" sysLen2="${item.ITEM_LEN_2_2 }" id="ITEM_NO_2_${i.index}">${item.ITEM_NO_2}<c:if test="${not empty item.ITEM_NO_2_2}"> ${item.ITEM_LEN_1_2}<br/>${item.ITEM_NO_2_2} ${item.ITEM_LEN_2_2}</c:if><c:if test="${item.ACTIVITY_2 eq '2'}"><br/>审批中</c:if></td>
							 	...
							 	<td class='td_center' sysWeek="5" sysActivity="${item.ACTIVITY_5}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_5 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_5 }" sysLen1="${item.ITEM_LEN_1_5 }" sysItem2="${item.ITEM_NO_2_5 }" sysLen2="${item.ITEM_LEN_2_5 }" id="ITEM_NO_5_${i.index}">${item.ITEM_NO_5}<c:if test="${not empty item.ITEM_NO_2_5}"> ${item.ITEM_LEN_1_5}<br/>${item.ITEM_NO_2_5} ${item.ITEM_LEN_2_5}</c:if><c:if test="${item.ACTIVITY_5 eq '2'}"><br/>审批中</c:if></td>
							 	...
							 	<td class='td_center' sysWeek="6" sysActivity="${item.ACTIVITY_6}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_6 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_6 }" sysLen1="${item.ITEM_LEN_1_6 }" sysItem2="${item.ITEM_NO_2_6 }" sysLen2="${item.ITEM_LEN_2_6 }" id="ITEM_NO_6_${i.index}">${item.ITEM_NO_6}<c:if test="${not empty item.ITEM_NO_2_6}"> ${item.ITEM_LEN_1_6}<br/>${item.ITEM_NO_2_6} ${item.ITEM_LEN_2_6}</c:if><c:if test="${item.ACTIVITY_6 eq '2'}"><br/>审批中</c:if></td>
							 	...
							 	<td class='td_center' sysWeek="0" sysActivity="${item.ACTIVITY_0}" sysType="ITEM_NO" <c:if test="${item.ACTIVITY_0 ne '2'}">sysLog="select"</c:if> sysValue='${shiftItem }' sysIndex="${i.index}" sysItem1="${item.ITEM_NO_0 }" sysLen1="${item.ITEM_LEN_1_0 }" sysItem2="${item.ITEM_NO_2_0 }" sysLen2="${item.ITEM_LEN_2_0 }" id="ITEM_NO_0_${i.index}">${item.ITEM_NO_0}<c:if test="${not empty item.ITEM_NO_2_0}"> ${item.ITEM_LEN_1_0}<br/>${item.ITEM_NO_2_0} ${item.ITEM_LEN_2_0}</c:if><c:if test="${item.ACTIVITY_0 eq '2'}"><br/>审批中</c:if></td>
								
								<td class='td_center'>额:${item.CALENDER0 }  实:${item.DETAIL0 }  差:${item.DETAIL0 - item.CALENDER0 }</td>
								<td class='td_center'>额:${item.CALENDER1 }  实:${item.DETAIL1 }  差:${item.DETAIL1 - item.CALENDER1 }</td>
								<td class='td_center'>额:${item.CALENDER2 }  实:${item.DETAIL2 }  差:${item.DETAIL2 - item.CALENDER2 }</td>
								<td class='td_center'>额:${item.CALENDER0 + item.CALENDER1 + item.CALENDER2 }  实:${item.DETAIL0 + item.DETAIL1 + item.DETAIL2 }  差:${item.DETAIL0 + item.DETAIL1 + item.DETAIL2 - (item.CALENDER0 + item.CALENDER1 + item.CALENDER2)}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
