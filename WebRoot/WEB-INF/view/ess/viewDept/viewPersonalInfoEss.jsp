...
<script type="text/javascript">
	...
	function fangdajingPER(flag) {
		...
		$('#fangdaPER',navTab.getCurrentPanel())
				.attr(
						'href',
						'/hrm/empinfo/viewEmpInfoListTanchu?pageNum=1&firstFlag=N&searchChange=viewPersonalInfoEss&seach_KEY=' + name);
		if (flag == 'onkeyup')
...
<div class="formBar"><!-- <input style="margin-left: 1240px;" type="button" value="查询"  onclick="navTabNum('/ess/viewDept/viewPersonalInfoEss','pageNum=1&menuNo=125244&navTabId=hr2100','hr2100','综合简介');"> -->
<ul class="toolBar">
	<li id="editLi"><a class="edit" href="#"
		onclick="navTabNum('/ess/viewDept/viewPersonalInfoEss?PERSON_ID=${personInfo.PERSON_ID}','pageNum=1&menuNo=90000444&navTabId=ess3307','ess3307','<spring:message code='hrm.empinfo.COOMPREHENSIVE_INTRODUCTION.Z' />');">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<c:if test="${LoginUser.cpnyId eq 'SPC_NJ'}">	
				...
				<td class="td_type" width="30%"><c:if
					...
					href="/ess/viewDept/viewEmpIdList?pageNum=1" lookupGroup="person"
					width="950"></a></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="td_title"><spring:message code="hrm.contract.POSITION_NO" /><!--职责--></td>
			...
			<tr>
				...
				<td class="td_type">
				<span>${personInfo.WORK_TIME} <spring:message code="inct.salesman.year" /> <!--年-->
					  ${personInfo.WORK_TIME_MONTH} <spring:message code="liang.hr.viewWorkInfo.title.MONTH" /> <!--月--></span>
				</td>
			</tr>
		</table>
		</td>
...
<div class="tabsContent" id="displaycheckbox"><c:forEach
	...
	<c:if test="${menu.MENU_NO eq '2540'}">
		...
		<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
				<li id="editLi"><a class="edit"
					href="/ess/viewDept/viewHrPersonalInfo?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<tr>
				...
				<span>${personInfo.AGE} <spring:message code="inct.salesman.year" /> <!--年-->
					  ${personInfo.AGE_MONTH} <spring:message code="liang.hr.viewWorkInfo.title.MONTH" /> <!--月--></span>
				</td>
				<td class="td_title">
		...
		<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewEmergencyAddress?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<div class="formBar">
			...
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
				<li id="editLi">
					<a class="edit"
						href="/ess/viewDept/viewAddressMatters?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewFamily?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
	...
	<c:if test="${menu.MENU_NO eq '14013949'}">
		<div style="display: block;" id="displaycheckbox_1"><div class="formBar">
						...
						<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
								<li id="editLi"><a class="edit"
									href="/ess/viewDept/viewStartPoint?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<tr>
				...
				<td class="td_title" style="text-align:center"><spring:message
					code="org.title.POSITION_NO" /> <!--职责--></td>
				<td class="td_title" style="text-align:center">
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewExperiencePoint?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
	...
	<c:if test="${menu.MENU_NO eq '14014294'}">
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
				<li id="editLi"><a class="edit"
					href="/ess/viewDept/viewEducationMatter?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewBidMatter?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewForeignLanguage?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<ul class="toolBar">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewTrainingBasic?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
	...
	<c:if test="${menu.MENU_NO eq '14014295'}">
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewRecognition?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewPunishment?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
	...
	<c:if test="${menu.MENU_NO eq '14014296'}">
		...
		<!--<ul class="toolBar">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewEvaluateInfo?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<tr>
				<td class="td_title"><spring:message
					code="hrm.empinfo.Evaluation_year" /><!--评价年度--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.January" /><!--1月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.February" /><!--2月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.March" /><!--3月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.April" /><!--4月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.May" /><!--5月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.June" /><!--6月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.July" /><!--7月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.August" /><!--8月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.September" /><!--9月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.October" /><!--10月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.November" /><!--11月--></td>
				<td class="td_title"><spring:message
					code="hrm.empinfo.December" /><!--12月--></td>
	...
	<c:if test="${menu.MENU_NO eq '14014297'}">
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewSpecialMatter?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }"
	...
	<c:if test="${menu.MENU_NO eq '14014298'}">
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewPassportPerson?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }&flag=1"
		...
		<ul class="toolBar">
			<c:if test="${isSuperUser == 1 or isSuperHrUser == 1 or isHrUser == 1}">
			<li id="editLi"><a class="edit"
				href="/ess/viewDept/viewPassportFamily?PERSON_ID=${personInfo.PERSON_ID}&TABS_SELECTED=${tabsSelected }&flag=2"
