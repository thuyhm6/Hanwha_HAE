...
function validateBatchArVacAffirmCallback(form,callback) {	
	var $form = $(form);	
	if (!$form.valid()) {
		return false;
	}
...
//注意input的id和tr的id要一样
function addRowByIDArVacTwoBatch(currentRowID){
	var count = parseInt($("#affirmListCnt").val());
	str = '<tr id = "rowIdApplyBatchArVac'+count+'">'
			...
			+'<td style="text-align: center">'
				...
				+'	onkeydown="submitKeyClick_affirmor_ArVac(this,' + count + ',event)" class="required"/>'
			+'</td>'
			+'<td style="text-align: center">'
				+'<img src="/resources/images/+.gif" style="cursor:hand" title="添加" onclick="addRowByIDArVacTwoBatch(' + count + ');"/>&nbsp;&nbsp;&nbsp;'
				+'<img src="/resources/images/-.gif" style="cursor:hand" title="删除" '
				//先删除，再排序
				+'	onclick="javaScript:document.all.addApplyBatchArVacAffirm_list.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);changeApplyBatchOtLevel();"/>'
			+'</td>'
		+'</tr>';
	//当前行之后插入一行
	$("#rowIdApplyBatchArVac" + currentRowID).after(str);
...
function addRowByIDApplyBatchArVacFirst(){
	var count = parseInt($("#affirmListCnt").val());
	str = '<tr id = "rowIdApplyBatchArVac'+count+'">'
      	+'<td style="text-align: center"></td>'
			+'<td style="text-align: center">'
	...
	var tb2 = document.getElementById("addApplyBatchArVacAffirm_list");

   	if(tb2.rows.length == 1){
   		$("#addApplyBatchArVacAffirm_list").append(str);
   	} else {
   	 	//当前行之后插入一行
...
var keyCodeInit=0;
function submitKeyClick_affirmor_ArVac(obj,index,event){
		...
		<tbody>
			<c:forEach items="${arVacBatchAffirmList}" var="item" varStatus="i">			
				<tr>
					...
					<td style="text-align:left;width:16%" id="arVacBatchUpload_${i.index}">
						...
						&nbsp;&nbsp;<img src="/resources/images/+.gif"  title="上传" border="0" align="absmiddle" style="cursor:hand" onclick="uploadAttWindow('arVacBatchUpload_${i.index}',${item.APPLY_NO },216691)"/>
					</td>
				</tr>			
			</c:forEach>			
		</tbody>
	</table>
	<c:set value="/ess/annualadjustment/viewFullAnnuAffirmInfo1?seach_APPLY_TYPE_NO=216691&seach_APPLY_NO=${annumap.APPLY_NO}" var="pageUrl"/>
	<%@ include file="/WEB-INF/view/inc/initPagination.jsp"%>
</div>
</c:if>
<div class="pageContent" >
	<form id="modifyAffirmorForBatchArVac" method="post" action="/ess/annualadjustment/modifyAffirmorForBatchArVac" class="pageForm required-validate" 
		onsubmit="return validateBatchArVacAffirmCallback(this,navTabAjaxDone);">
	<table class="user_table" width="100%" border="0">
		<tr>
			...
			<td colspan="3" style="text-align: center;width:90%;">
				<table class="user_table" width="100%" id="addApplyBatchArVacAffirm_list">
					<tr>
									<td class="td_title" style="text-align:center;" width="11%">决裁等级</td>
									<td class="td_title" style="text-align:center;" width="44%">决裁者</td>
									<td class="td_title" style="text-align:center;" width="44%">是否新增(<img src="/resources/images/+.gif" title="添加" border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDApplyBatchArVacFirst()"/>)</td>
					</tr>
								<c:forEach items="${affirmorList}" var="affirmor" varStatus="i">			
									<tr id="rowIdApplyBatchArVac${i.index }">
										...
										<td class="td_type" style="text-align: center"><img src="/resources/images/+.gif" title="添加" border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDArVacTwoBatch(${i.index })"/></td>
									</tr>			
								</c:forEach>
				</table>
							...
							<input type="hidden" name="APPLY_TYPE" id="APPLY_TYPE" value="216691">
