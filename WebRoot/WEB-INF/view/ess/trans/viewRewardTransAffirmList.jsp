	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchRewardAffirmForm");
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/trans/viewRewardTransAffirmList" method="post"
	      rel="pagerForm" name="searchRewardAffirmForm" id="searchRewardAffirmForm">
	<div class="searchBar">
		<table class="searchContent">
			...
			<tr>
			    ...
			    <td>
			        <input type="text" name="seach_REWARD_FORM_DATE" class="date required" format="yyyy-MM-dd" readonly="true" value="${REWARD_FORM_DATE }"/>
				...
				<td>
				    <input type="text" name="seach_REWARD_TO_DATE" class="date required" format="yyyy-MM-dd" readonly="true" value="${REWARD_TO_TIME }"/>
				    <a class="inputDateButton" href="javascript:;"></a>
				</td> 
				<td>
				...
				<li>
					<div class="subBar">
						    <div class="buttonActive">
						         <a class="update" onclick="validateRewardCallback('updateRewardTransAffirmForm',DWZ.ajaxDone,'1')" href="#" ><span>
						         <spring:message code="ess.trans.title.passInBatch"/><!--批量通过--></span></a>
                            </div>
						    <div class="buttonActive">
						         <a class="update" onclick="validateRewardCallback('updateRewardTransAffirmForm',DWZ.ajaxDone,'2')" href="#" ><span>
...
<div class="pageContent">      
			                            ...
			                            <c:choose>
									        <c:when test="${affirmer.AFFIRMOR_ID == rewardTrans.CURRENT_AFFIRM_ID && affirmer.AFFIRM_LEVEL == rewardTrans.AFFIRM_LEVEL }">									
										         <c:if test="${affirmer.HREF_FLAG == 1}" >
												    <a href="/ess/trans/approveRewardTrans?HR_AFFIRM_NO=${affirmer.HR_AFFIRM_NO}&&START_DATE=${rewardTrans.START_DATE}&&EMPID=${rewardTrans.EMPID}&&PERSON_ID=${rewardTrans.PERSON_ID}&&AFFIRM_FLAG=1&&AFFIRM_LEVEL=${affirmer.AFFIRM_LEVEL}&&EXP_INSIDE_NO=${rewardTrans.EXP_INSIDE_NO}" target="ajaxTodo">
							                           <span style="color:blue;">&nbsp;<spring:message code="ess.trans.title.pass"/><!--通过--></span>
													</a>
												    <a href="/ess/trans/approveRewardTrans?HR_AFFIRM_NO=${affirmer.HR_AFFIRM_NO}&&START_DATE=${rewardTrans.START_DATE}&&EMPID=${rewardTrans.EMPID}&&PERSON_ID=${rewardTrans.PERSON_ID}&&AFFIRM_FLAG=2&&AFFIRM_LEVEL=${affirmer.AFFIRM_LEVEL}&&EXP_INSIDE_NO=${rewardTrans.EXP_INSIDE_NO}" target="ajaxTodo">
	...
	<c:set value="/ess/trans/viewRewardTransAffirmList" var="pageUrl"/>
