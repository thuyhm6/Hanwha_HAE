...
<script type="text/javascript">
function exportLBatchModel(){
	var url = "/ess/infoApply/exportBatchOtModuleL?navTabId=ess0231";
	document.getElementById("exportBatchExcel").href=encodeURI(url);
}

function importLOtBatch(){
	$("#importExcelDialogUL_ess0231").attr('href','/pa/excelImport/importExcelData?importFunName=/importLOtApplyTemp');
	$("#importExcelDialogUL_ess0231").click();
}
//-->
</script>
...
function F_HR_ShowMore1(personId,name,empid,i,deptname,deptno,glno,glname,dutyno,dutynoname,postno,postname,postgradeno,postgradename,posino,posiname){
	...
	document.getElementById("tableTitleName_L").rows[cloumeNum1-1].childNodes[10].innerHTML=
		"<input id='"+personId+"_APPLY_REMARK_"+cloumeCount+"' name='"+personId+"_APPLY_REMARK_"+cloumeCount+"'  ></input>";                                                               
...
<a id="importExcelDialogUL_ess0231" href="#" target="dialog" mask="true"></a>
<a id="importExcel_ess0231" href="#" target="navTab" mask="true">
	<span style="display:none;">加班申请L导入</span>
</a>
...
<!--<div class="pageHeader">
	<a id="onck" name="onck" href="/hrm/empinfo/viewEmpIdList?pageNum=1" lookupGroup="person" width="950"></a>
  	<form onsubmit="return navTabSearch(this);" action="/ess/infoApply/viewOtAffirmLBatchUpdateList" method="post" 
...
--><div class="pageContent">
	<form style="margin:0px;padding:0px;" name="applyOvertimeBatchFormL" id="applyOvertimeBatchFormL" method="post" action="/ess/infoApply/addLBatchOtApplyUpdate" 
		...
		<input type="hidden" id="type" name="type" value="update"></input>
		<input type="hidden" id="update_apply_no" name="update_apply_no" value="${APPLY_NO}"></input>
		<input type="hidden" id="OT_TIME_TYPE" name="OT_TIME_TYPE" value="L"/>
	
		<div class="formBar">	<div style="color:red">提示：修改时未选中的加班会被删除。</div>
			<ul class="toolBar">
				<li>
					<div class="buttonActive">
				...
				<!--<li>
					<a class="buttonActive" id ="exportBatchExcel" onclick="exportLBatchModel();" href="#">
		...
		<table class="tablea" width="100%" layoutH="183" id="tableTitleName_L">
		    ...
		    <tr>
				...
				<td width="50">
					<input id="otApplyRemark" name="otApplyRemark"  ></input>
				</td><!--				
				<td width="40">&nbsp;</td>
			--></tr>		        
			<tbody>		 
				<c:forEach items="${personList}" var="person" varStatus="i" >			
					<tr target="sid" rel="${person.PERSON_ID}"  >
						...
						<td>
						    <input type="text" id="${person.PERSON_ID}_APPLY_OT_DATE_${i.count}" name="${person.PERSON_ID}_APPLY_OT_DATE_${i.count}"   value="${person.APPLY_OT_DATE}"
					    ...
					    <td>
						    <select id="${person.PERSON_ID}_OT_APPLY_HOUR_${i.count}" name="${person.PERSON_ID}_OT_APPLY_HOUR_${i.count}">
								...
								<c:forEach var="h" begin="1" end="8" step="1">
									<option value="${h}" <c:if test="${h eq person.OT_APPLY_HOUR}">selected</c:if>>
										${h}小时
									</option>
								</c:forEach>
							</select>
							<select id="${person.PERSON_ID}_OT_APPLY_MINUTE_${i.count}" name="${person.PERSON_ID}_OT_APPLY_MINUTE_${i.count}">
								...
								<c:forEach var="m" begin="0" end="59" step="30">
									<option value="${m}" <c:if test="${m eq person.OT_APPLY_MINUTE}">selected</c:if>>
										${m}分
									</option>
								</c:forEach>
							</select>
						...
						<td>
							<ait:SelectSyCodeByCpnyID parentNo="31" cnpyID="${defaultCpny}" name="${person.PERSON_ID}_OT_APPLY_TYPE_CODE_${i.count}"
						    	selected="${person.OT_TYPE_CODE}" limit="all"/>
						</td>
						<td>
					    	<select id="${person.PERSON_ID}_ADJUST_YN_${i.count}" name="${person.PERSON_ID}_ADJUST_YN_${i.count}" >
								<option value="0" <c:if test="${person.ADJUST_YN eq '0' }">selected="selected"</c:if>>
									否
		                   		</option>
		                   		<option value="1" <c:if test="${person.ADJUST_YN eq '1' }">selected="selected"</c:if>>
									是
		                   		</option>
							</select>
					    </td>
					    <td>
					    	<select id="${person.PERSON_ID}_OT_PLACE_TYPE_${i.count}" name="${person.PERSON_ID}_OT_PLACE_TYPE_${i.count}">
								<option value="INSIDE" <c:if test="${person.OT_PLACE_TYPE eq 'INSIDE' }">selected="selected"</c:if>>
									社内
		                   		</option>
		                   		<option value="OUTSIDE" <c:if test="${person.OT_PLACE_TYPE eq 'OUTSIDE' }">selected="selected"</c:if>>
									社外
		                   		</option>
							</select>
					    </td>
						<td>
							<input id="${person.PERSON_ID}_APPLY_REMARK_${i.count}" name="${person.PERSON_ID}_APPLY_REMARK_${i.count}" 
							value="${person.APPLY_OT_REMARK }"	 ></input>
						</td>		
						<!--<td>
	...
	<c:set value="/ess/infoApply/viewOtAffirmLBatchUpdateList" var="pageUrl"/>
