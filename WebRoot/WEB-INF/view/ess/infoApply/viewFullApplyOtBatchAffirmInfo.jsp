...
<div class="pageContent">
		...
		<thead>
			<tr>
				...
				<th>加班时长</th>
				<th>加班原因</th>
				<c:if test="${CPNY_ID eq 'LGEHZ'}">
					<th>本月平日加班（付薪）累计时间</th>
		...
		<tbody>
			<c:forEach items="${OtBatchAffirmList}" var="item" varStatus="i">			
				<tr>
					...
					<c:if test="${CPNY_ID eq 'LGEHZ'}">
				<td style="text-align:center">${item.WEEKDAY}</td>
				
				</c:if>
				<c:if test="${CPNY_ID eq 'LGEYT'}">
					<td style="text-align:center">${item.OT_COUNT}</td>
	...
	<c:set value="/ess/infoApply/viewFullApplyOtBatchAffirmInfo?APPLY_NO=${APPLY_NO}" var="pageUrl"/>
