...
$(document).ready(function(){
	...
	$("#viewSSTOtApplyInfoTx_save",navTab.getCurrentPanel()).click(function(){
        ...
        if((parseFloat(OT_TOTAIL_MONTH) + parseFloat(OT_LENGTH) < 40) && (parseFloat(OT_TOTAIL) + parseFloat(OT_LENGTH) < 300) ){
	 ...
	 otToTime   = otToDate + " " + otToTime;
        $.ajax({//人事确认过的会和AR_DETAIL表的考勤冲突
			 ...
			 data:{sql:"select AR_GET_OT_OVER_CLASH('11111111','${LoginUser.adminID}','"+otFromTime+"','"+otToTime+"','"+offset+"') FLAG from dual"},
			 dataType:"json",
		...
		alertMsg.confirm("<spring:message code='hr.viewEvaluate.title.COMMIT_CONFIRM' />",//确定要提交吗？
		  	{okCall:function(){
				$.ajax({
					type:'POST',
					url:"/ess/infoApply/addOtOverApply",
					data:jsonData,
					dataType:"json",
					cache: false,
		...
		success: function(data){
			if(data.affirmorList.length>0){
				for(var i=0;i<data.affirmorList.length;i++){
					var count = $("#applyOtOverCount").val();
...
function addRowByIDApplyOtOverFirst(){
	var count = parseInt($("#applyOtOverCount").val());
				...
				<table class="user_table" width="100%"  border="0" cellpadding="0" cellspacing="0">
					...
					<tr>
					    ...
					    <td width="30%" class="td_type">
							...
							<ait:time name="OT_FROM_TIME" spacing="15" selected="17:00" onChange="getOtLengthSST(0);"/>
							~
							...
							<ait:time name="OT_TO_TIME" spacing="15" selected="20:00" onChange="getOtLengthSST(0);"/>
						</td>
						<td width="20%" class="td_title" style="text-align:right"><!--加班类型-->
							<spring:message code="ess.viewApply.title.overtimeApplyType"/>
							 ...
							 <input type="hidden" id="APPLY_TYPE_NO_OT"  name="APPLY_TYPE_NO" value="310"/>
						...
						<br>
							<table class="user_table" width="100%">	
								<tr>
								    ...
								    <!--是否新增 --> <spring:message code="evs.viewRegPersonalProbation.SHIFOUXINZENG.a" />(<img src="/resources/images/+.gif" title="<spring:message code="button.add"/>" border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDApplyOtOverFirst()"/>)</td>
								    </c:if>
								</tr>
								<tr>
									<td colspan="7">
										...
										<input type="hidden" name="applyOtOverCount" id="applyOtOverCount" value="1">
