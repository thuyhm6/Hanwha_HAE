...
function validateBatchArCwaAffirmCallback(form,callback) {	
	var $form = $(form);	
	if (!$form.valid()) {
		return false;
	}
...
//注意input的id和tr的id要一样
function addRowByIDArCwaTwoBatch(currentRowID){
	var count = parseInt($("#affirmListCnt").val());
	str = '<tr id = "rowIdApplyBatchArCwa'+count+'">'
			...
			+'<td style="text-align: center">'
				...
				+'	onkeydown="submitKeyClick_affirmor_ArCwa(this,' + count + ',event)" class="required"/>'
			+'</td>'
			+'<td style="text-align: center">'
				+'<img src="/resources/images/+.gif" style="cursor:hand" title="添加" onclick="addRowByIDArCwaTwoBatch(' + count + ');"/>&nbsp;&nbsp;&nbsp;'
				+'<img src="/resources/images/-.gif" style="cursor:hand" title="删除" '
				//先删除，再排序
				+'	onclick="javaScript:document.all.addApplyBatchArCwaAffirm_list.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);changeApplyBatchOtLevel();"/>'
			+'</td>'
		+'</tr>';
	//当前行之后插入一行
	$("#rowIdApplyBatchArCwa" + currentRowID).after(str);
...
function addRowByIDApplyBatchArCwaFirst(){
	var count = parseInt($("#affirmListCnt").val());
	str = '<tr id = "rowIdApplyBatchArCwa'+count+'">'
      	+'<td style="text-align: center"></td>'
			+'<td style="text-align: center">'
	...
	var tb2 = document.getElementById("addApplyBatchArCwaAffirm_list");

   	if(tb2.rows.length == 1){
   		$("#addApplyBatchArCwaAffirm_list").append(str);
   	} else {
   	 	//当前行之后插入一行
...
var keyCodeInit=0;
function submitKeyClick_affirmor_ArCwa(obj,index,event){
			...
			<c:forEach items="${arCwaBatchAffirmList}" var="item" varStatus="i">			
				<tr>
					...
					<td style="text-align:left;width:16%" id="arCwaBatchUpload_${i.index}">
						...
						&nbsp;&nbsp;<img src="/resources/images/+.gif" title="上传" border="0" align="absmiddle" style="cursor:hand" onclick="uploadAttWindow('arCwaBatchUpload_${i.index}',${item.APPLY_NO },218197)"/>
					</td>
				</tr>			
			</c:forEach>			
		</tbody>
	</table>
	<c:set value="/ess/infoApply/viewFullCwaApplyAffirmInfo1?seach_APPLY_TYPE_NO=218197&seach_APPLY_NO=${applyorInfo.APPLY_NO}" var="pageUrl"/>
	<%@ include file="/WEB-INF/view/inc/initPagination.jsp"%>
</div>
<div class="pageContent" >
	<form id="modifyAffirmorForBatchArCwa" method="post" action="/ess/infoApply/modifyAffirmorForBatchArCwa" class="pageForm required-validate" 
		onsubmit="return validateBatchArCwaAffirmCallback(this,navTabAjaxDone);">
	<table class="user_table" width="100%" border="0">
		<tr>
			...
			<td colspan="3" style="text-align: center;width:90%;">
				<table class="user_table" width="100%" id="addApplyBatchArCwaAffirm_list">
					<tr>
									<td class="td_title" style="text-align:center;" width="11%">决裁等级</td>
									<td class="td_title" style="text-align:center;" width="44%">决裁者</td>
									<td class="td_title" style="text-align:center;" width="44%">是否新增(<img src="/resources/images/+.gif" title="添加" border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDApplyBatchArCwaFirst()"/>)</td>
					</tr>
								<c:forEach items="${affirmorList}" var="affirmor" varStatus="i">			
									<tr id="rowIdApplyBatchArCwa${i.index }">
										...
										<td class="td_type" style="text-align: center"><img src="/resources/images/+.gif" title="添加" border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDArCwaTwoBatch(${i.index })"/></td>
									</tr>			
								</c:forEach>
				</table>
							...
							<input type="hidden" name="APPLY_TYPE" id="APPLY_TYPE" value="218197">
