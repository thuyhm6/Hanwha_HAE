		...
		if (confirm ('<spring:message code="alert.message.ess.infoApply.areYouSureToApply"/>')){		
			...
			success: function(data){ //请求成功后处理函数。
				if(data.statusCode=="200"){
					navTabSearch("searchOvertimeApplyBatchFormP");
  	...
  	$form.find(":checkbox[id='c1']").each(function(index, checkBoxObj){
  		if(checkBoxObj.checked){
			...
			//计算时长
         	...
         	//adjustYnChange(personId,i);//根据选择进行调整
         	//teshuYnChange(personId,i);
         	//otplaceTypeChange(personId,i);
  		}
  	});
}
...
//获取该日期的加班类型   公共的改变日期 不再使用
function getPOtApplyTypeB(){
	var $form = $("#applyOvertimeBatchFormP");
	var fromDate = $("#otFromDate",navTab.getCurrentPanel()).val();
	if(fromDate!=""){
		$.ajax({
			cache: false,
			type: 'post',
			...
			success: function(data) {
				...
				$form.find("input[name='publicDateType']").attr("value",dateType);
				if(dateType == '1440'){  
					$form.find("select[name='otApplyTypeCode']").attr("value","32");
					$form.find("input[name='otApplyTypeCodeValue']").attr("value","32");
					$form.find("select[name='adjustYn']").attr("value",'0');
			    	$form.find("select[name='adjustYn']").attr("disabled",true);
					$form.find("select[name='otPlaceType']").attr("value",'INSIDE');//社内外
					$form.find("select[name='otPlaceType']").attr("disabled",false);
					$form.find("select[name='teshuYn']").attr("value",'0');//特殊
					$form.find("select[name='teshuYn']").attr("disabled",false);
				}else if(dateType == '1441'){
					$form.find("select[name='otApplyTypeCode']").attr("value","33");
					$form.find("input[name='otApplyTypeCodeValue']").attr("value","33");
					$form.find("select[name='adjustYn']").attr("value",'0');//调休
					$form.find("select[name='adjustYn']").attr("disabled",false);
					$form.find("select[name='otPlaceType']").attr("value",'INSIDE');//社外
					$form.find("select[name='otPlaceType']").attr("disabled",false);
					$form.find("select[name='teshuYn']").attr("value",'0');//特殊
					$form.find("select[name='teshuYn']").attr("disabled",false);
				}else if(dateType == '1442'){
					$form.find("select[name='otApplyTypeCode']").attr("value","34");
					$form.find("input[name='otApplyTypeCodeValue']").attr("value","34");
					$form.find("select[name='adjustYn']").attr("value",'0');
					$form.find("select[name='adjustYn']").attr("disabled",true);
					$form.find("select[name='otPlaceType']").attr("value",'INSIDE');//社外
					$form.find("select[name='otPlaceType']").attr("disabled",false);
	...
	//setTimeout("getPOtApplyTypeB()",10000);
}
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(dateType == '1440'){  
					...
					$form.find("select[name="+allID.get("otPlaceType")+"]").attr("value",'INSIDE');//社内外
					...
					var cpny_id_lgehz = document.getElementById("CPNY_ID").value;
				    if(cpny_id_lgehz == 'LGEHZ'){
...
//获取决裁详细信息
function passOvertimeApplyValue_pbat(personId,i)
{   
	
	 
	var applyTypeCode = $("#"+personId+"_OT_APPLY_TYPE_CODE_"+i).val();
	if(applyTypeCode==null||applyTypeCode==''){
			alert("请先选择加班类型");
			return false;
		}
 	var  otLength =   document.getElementById(personId+"_OT_APPLY_LENGTH_VAL_"+i).value;

	 
    document.getElementById(personId+"123456_viewOvertimeApplyBatchPHref").href = 
    document.getElementById(personId+"123456_viewOvertimeApplyBatchPHref").href+
                            "&&APPLY_TYPE_CODE="+applyTypeCode+"&&APPLY_TYPE_NO=31"+"&&PERSON_ID="+personId+
                            "&&otLength="+otLength;
    document.getElementById(personId+"123456_viewOvertimeApplyBatchPHref").click();			
}
...
function calPoTLengthBatch(personId,i){
	...
	var otfromtime = from_date + " " + fromTime + ":" + "00";
	var ottotime = from_date + " " + toTime + ":" + "00";
	var dateTypeId  = $("#applyOvertimeBatchFormP input[id='"+personId+'_dateType_'+i+"']").val();
	var teshuYn =   $("#applyOvertimeBatchFormP select[name='"+personId+'_TESHU_YN_'+i+"']").val();
	var otPlaceType =   $("#applyOvertimeBatchFormP select[name='"+personId+'_OT_PLACE_TYPE_'+i+"']").val();
	var applytypecode =   $("#applyOvertimeBatchFormP select[name='"+personId+"_OT_APPLY_TYPE_CODE_"+i+"_show").val();
		...
		if(from_date!=null&&from_date!=""){
			...
			$.ajax({
				...
				if(response<0){
				  ...
				  $("#"+personId+"_OT_APPLY_LENGTH_VAL_"+i).val(response);
		...
		if(from_date!=null&&from_date!=""){
			...
			$.ajax({
				...
				if(response<0){
				  ...
				  document.getElementById(personId+"_OT_APPLY_LENGTH_ZONG_"+i).innerHTML = response+" 小时";
				  if(cpnyId == 'LGEQD' && response>=70){
			        document.getElementById(personId+"_OT_APPLY_LENGTH_ZONG_"+i).style.color="red";
			     }
			     if(cpnyId == 'LGEYT' && response>=90){
			        document.getElementById(personId+"_OT_APPLY_LENGTH_ZONG_"+i).style.color="red";
			     }
				  $("#"+personId+"_OT_APPLY_LENGTH_VALZ_"+i).val(response);
				 }
			});
	}
 }
...
function addNewTableP() {
	...
	var td10=tr.insertCell(10);//是否调休
		td10.className="td_left";
		
	var td11=tr.insertCell(11);//社内/外
		td11.className="td_left";
	
	var td12=tr.insertCell(12);//加班类型
	...
	var td15=tr.insertCell(15);//裁决者链接
		td15.className="td_left";
} 
...
function F_HR_ShowMore1(personId,name,empid,i,deptname,deptno,glno,glname,dutyno,dutynoname,postno,postname,postgradeno,postgradename,posino,posiname){
	...
	document.getElementById("tableTitleName_P").rows[cloumeNum1-1].childNodes[10].innerHTML=
		" <select id='"+personId+"_ADJUST_YN_"+cloumeCount+"' name='"+personId+"_ADJUST_YN_"+cloumeCount+"' disabled='true'  onChange='adjustYnChange("+personId+","+cloumeCount+");'>"
		+"   <option value='0'>否</option> "
		+"   <option value='1'>是</option> "
		+" </select> ";

		document.getElementById("tableTitleName_P").rows[cloumeNum1-1].childNodes[11].innerHTML=
			" <select id='"+personId+"_TESHU_YN_"+cloumeCount+"' name='"+personId+"_TESHU_YN_"+cloumeCount+"' disabled='true' onChange='teshuYnChange("+personId+","+cloumeCount+");'>"
			+"   <option value='0'>否</option> "
			+"   <option value='1'>是</option> "
			+" </select> ";
		
	document.getElementById("tableTitleName_P").rows[cloumeNum1-1].childNodes[12].innerHTML=
		" <select id='"+personId+"_OT_PLACE_TYPE_"+cloumeCount+"' name='"+personId+"_OT_PLACE_TYPE_"+cloumeCount+"' disabled='true' onChange='otplaceTypeChange("+personId+","+cloumeCount+");'>"
		+"   <option value='INSIDE'>社内</option> "
		+"   <option value='OUTSIDE'>社外</option> "
		+" </select> ";
	document.getElementById("tableTitleName_P").rows[cloumeNum1-1].childNodes[13].innerHTML=//加班类型
			" <ait:SelectSyCodeByCpnyID parentNo='31' cnpyID='${defaultCpny}' name='"+personId+"_OT_APPLY_TYPE_CODE_"+cloumeCount+"_show' selected=''  limit='all'/>"
			+"<input type='hidden' id='"+personId+"_OT_APPLY_TYPE_CODE_"+cloumeCount+"' name='"+personId+"_OT_APPLY_TYPE_CODE_"+cloumeCount+"' value=''/>";//存放32,33,34类型的加班类型
	var $form = $("#applyOvertimeBatchFormP");
	$form.find("select[name='"+personId+"_OT_APPLY_TYPE_CODE_"+cloumeCount+"_show']").attr("disabled",true);		//将加班类型设置为不可用
	...
	document.getElementById("tableTitleName_P").rows[cloumeNum1-1].childNodes[14].innerHTML=
		...
		"<input type='hidden' id='"+personId+'_dateType_'+cloumeCount+"'  name='"+personId+'_dateType_'+cloumeCount+"' value=''/>"+
		"<input type='hidden' id='"+personId+'_peopleType_'+cloumeCount+"' name ='"+personId+'_peopleType_'+cloumeCount+"'value=''/>";
		                                                               
	document.getElementById("tableTitleName_P").rows[cloumeNum1-1].childNodes[15].innerHTML=
		"<a rel='"+personId+"_overtimeApplyBatchPAffirmView' onclick='passOvertimeApplyValue_pbat("+personId+","+cloumeCount+");'><span style='cursor:pointer;'><spring:message code='ess.infoApply.title.viewDetail'/><!--查看详细--></span></a><a id='"+personId+"123456_viewOvertimeApplyBatchPHref'   href='/ess/infoApply/viewAffirmorByPersonIdAndAppCode?APPLY_TYPE_NO=31&&PERSON_ID="+personId+"' target='dialog' mask='true' width='300' height='300'></a>";
...
$(document).ready(function() {
	var $form = $("#applyOvertimeBatchFormP");
	$form.find("select[name='otApplyTypeCode']").attr("disabled",true);		//将加班类型设置为不可用
...
<div class="pageHeader">
	<a id="onck" name="onck" href="/hrm/empinfo/viewEmpIdList?pageNum=1" lookupGroup="person" width="950"></a>
  	<form onsubmit="return navTabSearch(this);" action="/ess/infoApply/viewOtAffirmPBatchList" method="post" 
    	rel="pagerForm" id="searchOvertimeApplyBatchFormP" name="searchOvertimeApplyBatchFormP">
    	<div class="searchBar">
			<table class="searchContent">
		        <tr>
					...
					<td>
						 ...
						 <ait:deptList name="seach_DEPT_NO" limit="ar" id="viewApplyOtPInfoList_seachDept"/>
						<ait:deptTreeIcon name="seach_DEPT_NO" limit="ar"    id="viewApplyOtPInfoList_seachDept" selected="${DEPTNO}"/>
...
<div class="pageContent">
	<form style="margin:0px;padding:0px;" name="applyOvertimeBatchFormP" id="applyOvertimeBatchFormP" method="post" action="/ess/infoApply/addPBatchOtApply" 
       	class="pageForm required-validate" onsubmit="return validateBatchOvertimeApplyCallback_P(this,navTabAjaxDone);">  
 		<a href="/ess/infoApply/viewAffirmorByPersonIdAndAppCode?test=1"  
			target="dialog" mask="true" width="300" height="300" id="viewOvertimeApplyBatchPHref" ></a>
				...
				<li>
					<div class="buttonActive">
						<div class="buttonContent"><!--点击填充-->
							<button type="button" onclick="fillItemOtP();">
								点击填充
							</button>
						</div>
					</div>
				...
				<li>
					<div class="buttonActive">
						<div class="buttonContent"><!--保存-->
							<button type="button" onclick="submitPFormPre(-1);">
								暂存
							</button>
						</div>
					</div>
		...
		<table class="tablea" width="100%" layoutH="183" id="tableTitleName_P">
        	...
        	<input id="APPLY_TYPE_NO" name="APPLY_TYPE_NO" type="hidden" size="30" value="31" />	
				...
				<td>
				    <input type="text" id="otFromDate" name="otFromDate" class="date" format="yyyy-MM-dd"
				     	readonly="true"  size="10" onpropertychange="getPOtApplyTypeBatch('0','0')"/>

                </td>
                <td>
                    <c:if test="${defaultCpny eq 'LGEPN'}">
                      <ait:time name="otFromTime" spacing="30" selected="17:30" onChange="calPoTLengthBatch();"/>	
                    </c:if>
                     <c:if test="${defaultCpny eq 'LGEQH'}">
                      <select name="otFromTime_h" id="otFromTime_h" onChange="calPoTLengthBatch();">
						 <c:forEach items="${hourParam}" var="vlist" varStatus="i">
						   <option value="${vlist.OT_TIME}" <c:if test="${vlist.OT_TIME eq '17'}">selected</c:if>>${vlist.OT_TIME}</option>
					     </c:forEach>
					  </select>
					  <select name="otFromTime_m" id="otFromTime_m" onChange="calPoTLengthBatch();">
                    ...
                    <c:if test="${defaultCpny ne 'LGEPN' && defaultCpny ne 'LGEQH'}">
                      <ait:time name="otFromTime" spacing="30" selected="18:00" onChange="calPoTLengthBatch();"/>	
                    </c:if>
			    </td>
				<td>
				    <c:if test="${defaultCpny eq 'LGEPN'}">
                     <ait:time name="otToTime" spacing="30" selected="20:30" onChange="calPoTLengthBatch();"/>
                    </c:if>
                     <c:if test="${defaultCpny eq 'LGEQH'}">
                     <select name="otToTime_h" id="otToTime_h" onChange="calPoTLengthBatch();">
						 <c:forEach items="${hourParam}" var="vlist" varStatus="i">
						   <option value="${vlist.OT_TIME}" <c:if test="${vlist.OT_TIME eq '20'}">selected</c:if>>${vlist.OT_TIME}</option>
					     </c:forEach>
					  </select>
					  <select name="otToTime_m" id="otToTime_m" onChange="calPoTLengthBatch();">
						<c:forEach items="${muniteParam}" var="list" varStatus="i">
						  <option value="${list.OT_TIME}" <c:if test="${list.OT_TIME eq '30'}">selected</c:if>>${list.OT_TIME}</option>
					    </c:forEach>
					  </select>		
                    ...
                    <c:if test="${defaultCpny ne 'LGEPN' && defaultCpny ne 'LGEQH'}">
                     <ait:time name="otToTime" spacing="30" selected="20:00" onChange="calPoTLengthBatch();"/>	
                    </c:if>				
			    </td>
			    
			    <td>
			    	<select id="endDayOffset" name="endDayOffset" onchange="calPoTLengthBatch();">
						<option value="0" <c:if test="${endDayOffset eq '0' }">selected="selected"</c:if>>
							否
                   		</option>
                   		<option value="1" <c:if test="${endDayOffset eq '1' }">selected="selected"</c:if>>
							是
                   		</option>
					</select>
			    </td>
			    ...
			    <td>
			    	<select id="adjustYn" name="adjustYn" disabled='true'  >
			     ...
			     <td>
			    	<select id="teshuYn" name="teshuYn" disabled='true' onChange="teshuYnChange('0','0');">
						<option value="0" <c:if test="${teshuYn eq '0' }">selected="selected"</c:if>>
							否
                   		</option>
                   		<option value="1" <c:if test="${teshuYn eq '1' }">selected="selected"</c:if>>
							是
                   		</option>
					</select>
			    </td>
			    <td>
			    	<select id="otPlaceType" name="otPlaceType"disabled='true' onChange="otplaceTypeChange('0','0');">
					...
					<input type="hidden"id="publicDateType" name ="publicDateType" value=""/>
					<input type="hidden"id="publicPeoPleType" name ="publicPeoPleType" value="0"/>
			    </td>
			    <td align="right">
			...
			<tbody>		 
				<c:forEach items="${personList}" var="person" varStatus="i" >			
					<tr target="sid" rel="${person.PERSON_ID}"  >
						...
						<td width="60">
						    <input type="text" id="${person.PERSON_ID}_FROM_DATE_${i.count}" name="${person.PERSON_ID}_FROM_DATE_${i.count}" 
						    	class="date" format="yyyy-MM-dd" readonly="true" size="10" onpropertychange="getPOtApplyTypeBatch(${person.PERSON_ID},${i.count});"/>				    	
					    </td>
					    <td width="45">
					     	
					 <c:if test="${defaultCpny eq 'LGEQH'}">
                     <select name="${person.PERSON_ID}_OT_FROM_TIME_H_${i.count}" id="${person.PERSON_ID}_OT_FROM_TIME_H_${i.count}" onChange="calPoTLengthBatch(${person.PERSON_ID},${i.count});">
						 <c:forEach items="${hourParam}" var="vlist" varStatus="j">
						   <option value="${vlist.OT_TIME}" <c:if test="${vlist.OT_TIME eq '17'}">selected</c:if>>${vlist.OT_TIME}</option>
					     </c:forEach>
					  </select>
					  <select name="${person.PERSON_ID}_OT_FROM_TIME_M_${i.count}" id="${person.PERSON_ID}_OT_FROM_TIME_M_${i.count}" onChange="calPoTLengthBatch(${person.PERSON_ID},${i.count});">
                   ...
                   <ait:time name="${person.PERSON_ID}_OT_FROM_TIME_${i.count}" spacing="30" selected="18:00" onChange="calPoTLengthBatch(${person.PERSON_ID},${i.count});"/>
                   </c:if>
					    </td>
						<td width="45">
                   ...
                   <c:if test="${defaultCpny ne 'LGEQH'}">
                    <ait:time name="${person.PERSON_ID}_OT_TO_TIME_${i.count}" spacing="30" selected="20:00" onChange="calPoTLengthBatch(${person.PERSON_ID},${i.count});"/>
					    ...
					    <td>
					    	<select id="${person.PERSON_ID}_END_DAY_OFFSET_${i.count}" name="${person.PERSON_ID}_END_DAY_OFFSET_${i.count}" onchange="calPoTLengthBatch(${person.PERSON_ID},${i.count});">
								<option value="0" <c:if test="${END_DAY_OFFSET eq '0' }">selected="selected"</c:if>>
									否
		                   		</option>
		                   		<option value="1" <c:if test="${END_DAY_OFFSET eq '1' }">selected="selected"</c:if>>
									是
		                   		</option>
							</select>
					    </td>
					    <td>
					    <input type="hidden"  id="${person.PERSON_ID}_OT_APPLY_LENGTH_VAL_${i.count}" value="0"/>
					    	<div id="${person.PERSON_ID}_OT_APPLY_LENGTH_${i.count}"></div>
						</td>
					    
						
						<c:if test="${defaultCpny eq 'LGEQD'}">
						<td>
					    <input type="hidden"  id="${person.PERSON_ID}_OT_APPLY_LENGTH_VALZ_${i.count}" value="0"/>
					    	<div id="${person.PERSON_ID}_OT_APPLY_LENGTH_ZONG_${i.count}"></div>
						</td>
						</c:if>
						<c:if test="${defaultCpny eq 'LGEYT'}">
						<td>
						...
						<td>
					    	...
					    	disabled = "true" onChange="adjustYnChange(${person.PERSON_ID},${i.count});">
					     ...
					     <td>
					    	<select id="${person.PERSON_ID}_TESHU_YN_${i.count}" name="${person.PERSON_ID}_TESHU_YN_${i.count}"
					    	disabled = "true" onChange="teshuYnChange(${person.PERSON_ID},${i.count});">
							<option value="0" <c:if test="${TESHU_YN eq '0' }">selected="selected"</c:if>>
									否
		                   		</option>
		                   		<option value="1" <c:if test="${TESHU_YN eq '1' }">selected="selected"</c:if>>
									是
		                   		</option>
							</select>
					    </td>
				    <td>
					    ...
					    <td>
							<ait:SelectSyCodeByCpnyID parentNo="31" cnpyID="${defaultCpny}" name="${person.PERSON_ID}_OT_APPLY_TYPE_CODE_${i.count}_show"
						    	disabled="true" selected="${OT_APPLY_TYPE_CODE}" limit="all"/>
						    <!-- 一些隐藏的参数 -->
						    <input type='hidden' id="${person.PERSON_ID}_OT_APPLY_TYPE_CODE_${i.count}" name="${person.PERSON_ID}_OT_APPLY_TYPE_CODE_${i.count}" value=''/>
						    <input type='hidden' id="${person.PERSON_ID}_dateType_${i.count}"  name="${person.PERSON_ID}_dateType_${i.count}" value=''/>
							<input type='hidden' id="${person.PERSON_ID}_peopleType_${i.count}" name ="${person.PERSON_ID}_peopleType_${i.count}"value=''/>
						...
						<td>
							<a rel="${person.PERSON_ID}_overtimeApplyBatchPAffirmView" onclick="passOvertimeApplyValue_pbat(${person.PERSON_ID},${i.count});">
							   <span style="cursor:pointer;"><spring:message code="ess.infoApply.title.viewDetail"/><!--查看详细--></span>
							</a>
						    <a    rel="${person.PERSON_ID}_overtimeApplyBatchPAffirmView" href="/ess/infoApply/viewAffirmorByPersonIdAndAppCode?APPLY_TYPE_NO=31&&PERSON_ID=${person.PERSON_ID}"  
						       target="dialog" mask="true" width="300" height="300" id="${person.PERSON_ID}123456_viewOvertimeApplyBatchPHref" ></a>
					    </td> 	
					</tr>			
				</c:forEach>			
			</tbody>
		</table>
		<div id="overtimeApplyBatchPAffirmView" style="border:10;overflow:auto;bottom:10;right:100;width:300;z-index:1;"></div>	
	</form>
	<c:set value="/ess/infoApply/viewOtAffirmPBatchList" var="pageUrl"/>
	<c:if test="${defaultCpny eq 'SST'}">
	<%@ include file="/WEB-INF/view/inc/initPaginationta.jsp"%>	
	</c:if>
