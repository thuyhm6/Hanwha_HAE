...
function delOtOverApplyCallback(OP_FLAG,form,callback) {
    ...
    $form.attr("action","/ess/infoApply/delOtOverApplyInBatch");
				...
				$.ajax({
					...
					success: function(data){ //请求成功后处理函数。
						if(data.statusCode=="200"){
							navTabSearch(document.viewSSTOtApplyInfoList);
 ...
 function changeURL_ess0237(applyNo){
		var href = "/ess/infoApply/viewApprovaledOtOverInfo?seach_APPLY_NO=" + applyNo;
		$.pdialog.open(href,"ess0237", "<spring:message code='ar.viewApplyAttenanceManagentInfoList.MINGXICHAKAN.b' />", {width:1000,height:600,mask:true});//明细查看
	}
</script>
<div>
...
<div class="pageHeader">                                
	<form onsubmit="return navTabSearch(this);" action="/ess/infoApply/viewSSTOtApplyInfoList?pageNum=1&firstFlag=N" method="post"
		id="viewSSTOtApplyInfoList" name="viewSSTOtApplyInfoList">
		<div class="searchBar">
...
<div class="pageContent" >
	<div class="formBar">
		<ul class="toolBar">
			<li>
				<a class="buttonActive" onclick="delOtOverApplyCallback(3,'delOtOverApplyAffirmForm',DWZ.ajaxDone)"><span><spring:message code="ess.affirmApply.title.quxiaoshenqing" /><!--取消申请--></span></a>
			</li>
			<li>
				<a class="buttonActive" onclick="navTabNum('/ess/infoApply/viewSSTOtApplyInfoTx','pageNum=1&menuNo=526&navTabId=ess0212','ess0212','<spring:message code="ess.infoApply.overtime_over" />');"><span><spring:message code="ess.infoApply.overtime_over" /><!--加班申请--></span></a>
			</li>
			
		</ul>
	</div> 
	<form name="delOtOverApplyAffirmForm" id="delOtOverApplyAffirmForm" method="post" action="/ess/infoApply/delOtOverApplyInBatch" 
	  onsubmit="return delOtOverApplyCallback(this, navTabAjaxDone);"> 
		<table class="list" width="99%" nowrapTD="false">
			<thead>
				<tr>
					...
					<th width="2%">
				    	<input type="checkbox" class="checkboxCtrl" group="c3" />
				    </th>
					...
					<tr target="sid" rel="${personInfo.PERSON_ID}">
					    ...
					    <td style="text-align: center">
					         ...
					         <c:if test="${otApply.AFFIRM_FLAG eq '14014306' or otApply.AFFIRM_FLAG eq '14014307' or otApply.AFFIRM_FLAG eq '14014308'}">
						    	<input type="checkbox" id="c3" name="c3" value="${otApply.APPLY_NO}" />
						    </c:if>
					    </td>
					     <td style="text-align: center;cursor: pointer;" id="codeChange" onclick='javascript:changeURL_ess0237(${otApply.APPLY_NO });'>
