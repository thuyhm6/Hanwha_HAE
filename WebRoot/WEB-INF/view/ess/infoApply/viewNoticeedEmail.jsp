...
$(document).ready(function(){
	$("#viewNoticeedEmail_Serch",navTab.getCurrentPanel()).click(function(){
		$("#viewNoticeedEmailForm",navTab.getCurrentPanel()).submit();
	});
	//删除
	$("#viewNoticeedEmail_delete",navTab.getCurrentPanel()).click(function(){
		//获取页面的值
		var SEQ = "";
		$("input[name='viewNoticeedEmail_checkbox']",navTab.getCurrentPanel()).each(function(i, obj){
			if(obj.checked){
				SEQ += ",'" + obj.value + "'";
			}
		});
	  		...
	  		{okCall:function(){
			  	$.ajax({
					...
					data: [{ name: 'SEQ', value: SEQ.substr(1) },
					       { name: 'FORM_ID', value: "viewNoticeedEmailForm" }],
	  				dataType:"json",
	  				cache: false,
	...
	//标记为已读
	$("#viewNoticeedEmail_readed",navTab.getCurrentPanel()).click(function(){
		//获取页面的值
		var SEQ = "";
		$("input[name='viewNoticeedEmail_checkbox']",navTab.getCurrentPanel()).each(function(i, obj){
			if(obj.checked){
...
<form id="viewNoticeedEmailForm" onsubmit="return navTabSearch(this);" action="/ess/infoApply/viewNoticeedEmail" method="post">
<div class="searchBar">
...
<div class="formBar">
	<ul class="toolBar">
			<li>
				<a class="buttonActive" id="viewNoticeedEmail_Serch" href="#">
					<span><!--查询 --><spring:message code="ess.infoApply.SELECT"/></span>
				</a>
			</li>
			<li>
				<a class="buttonActive" id="viewNoticeedEmail_delete" href="#">
					<span><!--删除 --><spring:message code="org.title.DELETE"/></span>
				</a>
			</li>
			<li>
				<a class="buttonActive" id="viewNoticeedEmail_readed" href="#">
...
<div class="pageContent" style="padding-left:10px;padding-right:10px;">
		<div class="user_table" style="font:bold 12px/20px arial,sans-serif;">Total:${viewNoticeedEmailSize}</div>
		<table class="table" width="100%" layoutH="200">
			<thead>
				<tr>
					...
					<th width="30">
				    	<input type="checkbox" class="checkboxCtrl" group="viewNoticeedEmail_checkbox" />
				    </th>
				    <th width="380" style="text-align: center"><!--日期-->
						<!--标题--><spring:message code="ess.title.Title"/>
					</th>
				    <th width="80" style="text-align: center"><!--星期-->
				...
				<c:forEach items="${viewNoticeedEmail}" var="item" varStatus="i">			
					<tr>
					    ...
					    <td style="text-align: center">
					        <input type="checkbox" name="viewNoticeedEmail_checkbox" value="${item.SEQ}" />
					    </td>
