...
function saveOtApplyAffirm(){
	...
	for(var i=0;i<ids.length;i++){
		if(ids[i].checked){
	        ...
	        if(parseFloat(otTotailMonth) + parseFloat(oTApplyHour) >= 30 && parseFloat(otTotailMonth) + parseFloat(oTApplyHour) < 40){
				alert("<spring:message code='ess.viewSSTOtApplyInfo.JIABANCHAOGUOYUESHANGXIAN.a' />");//加班时长超过月加班上限,无法申请!
			}
	        if(parseFloat(otTotailMonth) + parseFloat(oTApplyHour) - parseFloat(otSaturdayMonth) > 40 && otLimitMonth == 1){
	        	alertMsg.info("<spring:message code='ess.viewSSTOtApplyInfo.JIABANCHAOGUOYUESHANGXIAN.b' />");//加班时长超过月加班上限,无法申请!
	        	return false;
			}
	        if(parseFloat(otTotailMonth) + parseFloat(oTApplyHour) - parseFloat(otSaturdayMonth) > 40 ){
	        	alertMsg.info("<spring:message code='ess.viewSSTOtApplyInfo.JIABANCHAOGUOYUESHANGXIAN.b' />");//加班时长超过月加班上限,无法申请!
	        	return false;
			}
			...
			if(parseFloat(oTApplyHour) > 12 && $("#OT_TYPE_CODE_NO_"+index,navTab.getCurrentPanel()).val()=='33'){
				alertMsg.error("<spring:message code='ess.viewSSTOtApplyInfo.PAINDLEAVE.b' />");//平日加班最多申请4小时,请重新选择!
				return false;
			}
			if(parseFloat(oTApplyHour) > 12 && $("#OT_TYPE_CODE_NO_"+index,navTab.getCurrentPanel()).val()=='218181'){
				alertMsg.error("<spring:message code='ess.viewSSTOtApplyInfo.PAINDLEAVE.b' />");//平日加班最多申请4小时,请重新选择!
				return false;
			}
			
	        if(parseFloat(otTotail) + parseFloat(oTApplyHour) >= 300 && otLimtYear == 1){
...
<div class="formBar">
	<ul class="toolBar">
		...
		<c:if test="${LoginUser.language ne 'ko'}">
			<li><a class="buttonActive" onclick="downloadExcel('viewApplyOtLBatchByAnyApproverList','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=128','/ess/infoApply/viewApplyOtLBatchByAnyApproverList?firstFlag=N')"><span><!--导出到Excel--><spring:message code="hrm.empinfo.EXPORT" /></span></a></li>	
		</c:if>
		<c:if test="${LoginUser.language eq 'ko'}">
			<li><a class="buttonActive" onclick="downloadExcel('viewApplyOtLBatchByAnyApproverList','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=324','/ess/infoApply/viewApplyOtLBatchByAnyApproverList?firstFlag=N')"><span><!--导出到Excel--><spring:message code="hrm.empinfo.EXPORT" /></span></a></li>	
		</c:if>
	</ul>
</div>
	<form name="delOtApplyAffirmForm" id="delOtApplyAffirmForm" method="post" action="/ess/infoApply/delLOvertimeApplyInBatch" 
	  ...
	  <%-- <div class="user_table" style="font:bold 12px/20px arial,sans-serif;">Total:${nullOTTSTOAffirmListCnt}</div>  --%>             
		<table class="orderList" width="3500px" id="otAffirmLBatchTable">
			<thead>
			    <tr>
				    <th rowspan="2" width="5px"><!--NO-->NO
					</th>
					<th rowspan="2" ><input type="checkbox" class="checkboxCtrl" group="c1" />
				    </th>
