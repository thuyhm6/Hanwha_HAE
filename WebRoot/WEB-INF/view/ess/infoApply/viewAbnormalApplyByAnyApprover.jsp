...
$(document).ready(function(){
	...
	$(".list",navTab.getCurrentPanel()).dataTable({
		...
		"bScrollInfinite":true,
	     "orderClasses": false,
	     "order":[],//初始化不用自动排序
	     "scrollY": $(document.body).height() - 570,
	     "scrollX": false,
...
$("#check_all").click(function(){
	...
	$("#viewAbnormalApply_save",navTab.getCurrentPanel()).click(function(){	
	//选中检查
	var checked=false;
	var ids= document.getElementsByName("c1_cwa"); 
	...
	for(var i=0;i<ids.length;i++){
		if(ids[i].checked){
			...
			var reson = $("#REASON_"+index,navTab.getCurrentPanel()).val();
			if (reson == '' || reson == ' ') {
	...
	$.each($("input[name='c1_cwa']"),function(i, obj) {
		if (obj.checked) {
			...
			jsonData += ' "FROM_TIME": "'+ $("#FROM_TIME_"+index,navTab.getCurrentPanel()).val()  + '" ,';
			jsonData += ' "TO_TIME": "'+ $("#TO_TIME_"+index,navTab.getCurrentPanel()).val()  + '" ,';
	...
	var affirmJsonData = '[';
	var tb2 = document.getElementById("addApplyAbnormalAffirm_list");
   	if(tb2.rows.length == 0){
        ...
        for(var i=0;i<affirms.length;i++){
           ...
           var rowId = affirms[i].id.substring(35);
	...
	var $form = $("#viewAbnormalApplyForm",navTab.getCurrentPanel());
		...
		success: function(data){
			if(data.affirmorList.length>0){
				for(var i=0;i<data.affirmorList.length;i++){
					var count = $("#applyAbnormalCount").val();
					htm +='<tr id="rowIdApplyAbnormal'+ count +'"><td class="td_type" style="text-align: center" width="5%" id="rowIndex_'+ count +'"><span name="rowIndex">'+(i+1)+'</span></td>';
			        ...
			        htm +='<input id="dwz.person.AFFIRMOR_IDApplyAbnormal' + count + '" name="ATT_AFFIRMOR_ID" value="'+ data.affirmorList[i].AFFIRMOR_ID +'" type="hidden" />';
					...
					htm +='border="0" align="absmiddle" style="cursor:hand" onclick="javaScript:document.all.addApplyAbnormalAffirm_list.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);changeApplyLeaveLevel();"/></td>';
					</c:if>
					htm +='</tr>';
			  	$("#applyAbnormalCount",navTab.getCurrentPanel()).val(++count) ;
				} 
				$("#addApplyAbnormalAffirm_list",navTab.getCurrentPanel()).html(htm);
			}else{
				$("#addApplyAbnormalAffirm_list",navTab.getCurrentPanel()).html('');
			}
		},
		error: DWZ.ajaxError
	});
...
function addRowByIDApplyLeaveFirst(){
	var count = parseInt($("#applyAbnormalCount").val());
    var htm  ='<tr id="rowIdApplyAbnormal'+ count +'"><td class="td_type" style="text-align: center" width="5%" id="rowIndex_'+ count +'"><span name="rowIndex">'+(i+1)+'</span></td>';
        htm +='<td class="td_type" style="text-align: center" width="20%">';
        //htm +='<input type="hidden" id="approvType' + count + '" name="approvType" value="1" />';
        //htm +='<input type="hidden" name="approvTypeIndex" value="' + count + '" />';
        //htm +='<input id="dwz.person.AFFIRMOR_IDApplyAbnormal' + count + '" name="ATT_AFFIRMOR_ID" value="" type="hidden" />';
        htm +='<input id="dwz.person.EMPNAMEApplyAbnormal' + count + '"  type="text" style="text-align: center"  size="24" disabled="disabled"/>';
        htm +='</td>';        
		htm +='<td class="td_type" style="text-align: center" width="20%">';
		htm +='<input id="dwz.person.AFFIRMOR_IDApplyAbnormal' + count + '" name="ATT_AFFIRMOR_ID" value="" type="hidden"/>';
		htm +='<input id="dwz.person.EMPINFOApplyAbnormal' + count + '" name="empid" alt="<spring:message code="evs.affirm.please_input_enter.e"/>" type="text" style="text-align: center" size="25" class="required" lookupGroup="person" onkeydown="submitKeyClick_applyLeave(this,' + count + ',event)"/>';
		htm +='</td>';
		htm +='<td class="td_type" style="text-align: center" width="20%">';
		htm +='<input id="dwz.person.DEPTNAMEApplyAbnormal' + count + '"  type="text" style="text-align: center"  size="30" disabled="disabled"/>';
		htm +='</td>';
		htm +='<td class="td_type" style="text-align: center" width="20%">';
		htm +='<input id="dwz.person.POSITIONApplyAbnormal' + count + '"  type="text" style="text-align: center"  size="30" disabled="disabled"/>';
		htm +='</td>';
		htm +='<td class="td_type" style="text-align: center" width="5%">';
		htm +='<select id="approvType' + count + '" name="approvType' + count + '">';
		htm +='<option value="1" ><!--审批 --><spring:message code="hr.contract.title.shenpi"/></option>';
		...
		htm +='border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDApplyLeave(' + count + ')"/>&nbsp;&nbsp;&nbsp;';
		htm +='<img src="/resources/images/-.gif" title="<spring:message code="button.delete"/>"';	
		htm +='border="0" align="absmiddle" style="cursor:hand" onclick="javaScript:document.all.addApplyAbnormalAffirm_list.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);changeApplyLeaveLevel();"/></td></tr>';
        
		var tb2 = document.getElementById("addApplyAbnormalAffirm_list");
   	if(tb2.rows.length == 0){
   		$("#addApplyAbnormalAffirm_list").html(htm);
   	} else{
   	   	//当前行之后插入一行
   	   	$("#" + tb2.rows[0].id,navTab.getCurrentPanel()).before(htm);
   	}
  	$("[id='dwz.person.EMPINFOApplyAbnormal" + count + "']",navTab.getCurrentPanel()).inputAlert();
   	changeApplyLeaveLevel();
  	$("#applyAbnormalCount").val(++count);
}
...
function addRowByIDApplyLeave(currentRowID){
	...
	htm +='<td class="td_type" style="text-align: center" width="20%">';
	htm +='<input id="dwz.person.AFFIRMOR_IDApplyAbnormal' + count + '" name="ATT_AFFIRMOR_ID" value="" type="hidden"/>';
	htm +='<input id="dwz.person.EMPINFOApplyAbnormal' + count + '" name="empid" type="text" style="text-align: center" alt="<spring:message code="evs.affirm.please_input_enter.e"/>" size="25" class="required" lookupGroup="person" onkeydown="submitKeyClick_applyLeave(this,' + count + ',event)"/>';
	htm +='</td>';
	htm +='<td class="td_type" style="text-align: center" width="20%">';
	htm +='<input id="dwz.person.DEPTNAMEApplyAbnormal' + count + '"  type="text" style="text-align: center"  size="30" disabled="disabled"/>';
	htm +='</td>';
	...
	htm +='border="0" align="absmiddle" style="cursor:hand" onclick="javaScript:document.all.addApplyAbnormalAffirm_list.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);changeApplyLeaveLevel();"/></td></tr>';
   	...
   	//当前行之后插入一行
   	$("#rowIdApplyAbnormal" + currentRowID,navTab.getCurrentPanel()).after(htm);
  	$("[id='dwz.person.EMPINFOApplyAbnormal" + count + "']",navTab.getCurrentPanel()).inputAlert();
   	changeApplyLeaveLevel();
  	$("#applyAbnormalCount").val(++count) ;
}
	...
	var navTabId = '';
 	...
 	var keyCode = e.which ? e.which : e.keyCode;
   	if(keyCode==13){
		...
		var empNameStr = "EMPNAMEApplyAbnormal"+empIdStr.substring(20);
		var positionIdStr = "POSITIONApplyAbnormal"+empIdStr.substring(20);
		var deptIdStr = "DEPTNAMEApplyAbnormal"+empIdStr.substring(20);
		if(empid == ''){
			obj.value=" ";
			document.getElementById("onckAbnormal").href=encodeURI(encodeURI("/ar/attendanceMintenance/viewAddAffirmList?isEmployeement=1&firstFlag=1&limit=all&pageNum=1"
					+'&seach_KEY='+empid
					+'&empidStr='+empIdStr
	   		...
	   		$.ajax({
				...
				success: function(jsonObject){
							if(jsonObject.perCnt != 1 ){
								document.getElementById("onckAbnormal").href=encodeURI(encodeURI("/ar/attendanceMintenance/viewAddAffirmList?isEmployeement=1&limit=all&pageNum=1"
										+'&seach_KEY='+empid
										+'&empidStr='+empIdStr
										+'&personidStr='+personIdStr
										+'&positionIdStr='+positionIdStr
							...
							if(jsonObject.perCnt==1){
								$("[id='dwz.person.EMPNAMEApplyAbnormal" + index + "']").val(jsonObject.empName);
							  	$("[id='dwz.person.EMPINFOApplyAbnormal" + index + "']").val(jsonObject.empId);
							  	$("[id='dwz.person.AFFIRMOR_IDApplyAbnormal" + index + "']").val( jsonObject.personId);
							  	$("[id='dwz.person.POSITIONApplyAbnormal" + index + "']").val( jsonObject.POSITION_NAME);
							  	$("[id='dwz.person.DEPTNAMEApplyAbnormal" + index + "']").val( jsonObject.deptName);
							}
						},
				error: DWZ.ajaxError
			});
...
<div class="pageHeader">
	<form id="viewShowCwaAbnormalApply"  onsubmit="return navTabSearch(this);" action="/ess/infoApply/viewAbnormalApplyByAnyApprover?firstFlag=N" method="post" >
		<div class="searchBar">
...
<form id="viewAbnormalApplyForm" method="post" action="/ess/infoApply/addAbnormalApplyByAnyApprover">
		<div class="formBar">
			<ul  class="toolBar">
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" id="viewAbnormalApply_save">
						...
						<tr> 
							...
							<td style="text-align: center" width="30%">
								<spring:message code="ess.infoApply.in_door_card" /><!-- 进门卡 --><input type="text" name="IN_DATE_${i.index}" id="IN_DATE_${i.index}" class="Wdate" onClick="WdatePicker({dateFmt:'dd/MM/yyyy',lang:'en'})" value="${list.SHIFT_START_YYYY}"/>
										...
										<input id="IN_MI_${i.index}" name="IN_MI_${i.index}" type="text" class="digits required" size="3" maxlength="2" min="0" max="59" />&nbsp;&nbsp;<br>
			...
			<table class="user_table" width="100%" style="margin-bottom:60px">	
				<tr>
				...
				<tr>
					<td colspan="6">
						<table width="100%" border="0" cellpadding="0" cellspacing="0" id="addApplyAbnormalAffirm_list">
							<tbody id="rowIdApplyAbnormalq">
							</tbody>
						</table>
    				<input type="hidden" name="applyAbnormalCount" id="applyAbnormalCount" value="1">
