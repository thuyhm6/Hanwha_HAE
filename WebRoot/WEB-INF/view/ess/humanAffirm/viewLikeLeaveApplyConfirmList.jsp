...
<!--
function validateLeaveApplyConfirmCallback_ess0225(form,callback,flag) {
	var $form=null;
	if($('#'+form).length>0)
		$form=$('#'+form);
	else
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchLeaveApplyConfirmForm_ess0225");
...
function pageFromSea(a){                         
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", "/ess/humanAffirm/viewLikeLeaveApplyConfirmList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_LEAVE_FROM_TIME="+seach_LEAVE_FROM_TIME+"&seach_LEAVE_TO_TIME="+seach_LEAVE_TO_TIME+"&seach_LEAVE_TYPE_CODE="+seach_LEAVE_TYPE_CODE+"&seach_STATUS_CODE="+seach_STATUS_CODE);
}
//-->
</script>
         
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/humanAffirm/viewLikeLeaveApplyConfirmList" method="post" 
	      name="searchLeaveApplyConfirmForm" id="searchLeaveApplyConfirmForm_ess0225" rel="pagerForm">
				...
				<li>
					<div class="subBar">
						    <div class="buttonActive">
						         <a class="update" onclick="validateLeaveApplyConfirmCallback_ess0225('updateLeaveApplyConfirmForm_ess0225',DWZ.ajaxDone,'1')" href="#" >
						         <span><spring:message code="ess.title.passInBatch"/><!--批量通过--></span></a>
                            </div>
						    <div class="buttonActive">
						         <a class="update" onclick="validateLeaveApplyConfirmCallback_ess0225('updateLeaveApplyConfirmForm_ess0225',DWZ.ajaxDone,'2')" href="#" >
...
<form name="updateLeaveApplyConfirmForm" id="updateLeaveApplyConfirmForm_ess0225" method="post" 
      action="/ess/humanAffirm/confirmLeaveApplyInBatch" class="pageForm required-validate"
	  onsubmit="return validateLeaveApplyConfirmCallback_ess0225(this, navTabAjaxDone);">    
					...
					<td>
						<dt style="padding: 1px;">
							<c:if test="${leaveApply.ACTIVITY == 0}" >
							    <a href="/ess/humanAffirm/confirmLeaveApply?APPLY_NO=${leaveApply.APPLY_NO}&&PERSON_ID=${leaveApply.PERSON_ID}&&CONFIRM_FLAG=1&navTabId=ess0225" target="ajaxTodo">
		                           <span style="color:blue;">&nbsp;<spring:message code="ess.viewApply.title.pass"/><!--通过--></span>
								</a>
							    <a href="/ess/humanAffirm/confirmLeaveApply?APPLY_NO=${leaveApply.APPLY_NO}&&PERSON_ID=${leaveApply.PERSON_ID}&&CONFIRM_FLAG=2&navTabId=ess0225" target="ajaxTodo">
	...
	<c:set value="/ess/humanAffirm/viewLikeLeaveApplyConfirmList" var="pageUrl"/>
