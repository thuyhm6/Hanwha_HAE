...
<!--
function validateLeaveApplyConfirmCallback_ess0507(form,callback,flag) {
	var $form=null;
	if($('#'+form).length>0)
		$form=$('#'+form);
	else
    ...
    $form.attr("action","/ess/humanAffirm/confirmLeaveApplyInBatch?CONFIRM_FLAG="+flag);
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchLeaveApplyConfirmForm_ess0507");
...
function pageFromSea(a){                         
	...
	var seach_APPLY_TYPE_CODE="2454";
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", "/ess/humanAffirm/viewLeaveApplyConfirmList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_LEAVE_FROM_TIME="+seach_LEAVE_FROM_TIME+"&seach_LEAVE_TO_TIME="+seach_LEAVE_TO_TIME+"&seach_LEAVE_TYPE_CODE="+seach_LEAVE_TYPE_CODE+"&seach_STATUS_CODE="+seach_STATUS_CODE+"&seach_APPLY_TYPE_CODE="+seach_APPLY_TYPE_CODE);
}
//-->
</script>
         
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/humanAffirm/viewLeaveApplyConfirmList" method="post" 
	      name="searchLeaveApplyConfirmForm" id="searchLeaveApplyConfirmForm_ess0507" rel="pagerForm">
	      <input type="hidden" id="menuNo" name="menuNo" value="2454"/>
	<div class="searchBar">
		<table class="searchContent">
			...
			<tr>
				...
				<td>
				     <ait:SelectSyCodeByCpnyID name="seach_LEAVE_TYPE_CODE" parentNo="123634" cnpyID="${defaultCpny}"  selected="${LEAVE_TYPE_CODE}" limit="all"/>       
				</td> 	--%>				
				...
				<li>
					<div class="subBar">
						    <div class="buttonActive">
						         <a class="update" onclick="validateLeaveApplyConfirmCallback_ess0507('updateLeaveApplyConfirmForm_ess0507',DWZ.ajaxDone,'1')" href="#" >
						         <span><spring:message code="ess.title.passInBatch"/><!--批量通过--></span></a>
                            </div>
						    <div class="buttonActive">
						         <a class="update" onclick="validateLeaveApplyConfirmCallback_ess0507('updateLeaveApplyConfirmForm_ess0507',DWZ.ajaxDone,'2')" href="#" >
						         <span><spring:message code="ess.title.rejectInBatch"/><!--批量否决--></span></a>
                    </div>									
				</div>	
				</li>
	</div>
<form name="updateLeaveApplyConfirmForm" id="updateLeaveApplyConfirmForm_ess0507" method="post" 
      action="/ess/humanAffirm/confirmLeaveApplyInBatch" class="pageForm required-validate"
	  onsubmit="return validateLeaveApplyConfirmCallback_ess0507(this, navTabAjaxDone);">    
		...
		<tbody>
			<c:forEach items="${leaveApplyList}" var="leaveApply" varStatus="i">			
				<tr target="sid">
					...
					<td>${fn:substring(leaveApply.APPLY_TIME,0,10)}</td>
					...
					<td>
						<c:if test="${!empty leaveApply.FILE_URL }">
							<a href="/ess/infoApply/downloadFile?fileName=${leaveApply.FILE_URL }&file=${leaveApply.FILE_NAME}" ><spring:message code="pa.ins.alert.message.button.dolowdFile"/></a>
						</c:if>
					</td>
					<td>              
					...
					<td>
						<dt style="padding: 1px;">
							<c:if test="${leaveApply.ACTIVITY == 0}" >
							    <a href="/ess/humanAffirm/confirmLeaveApply?APPLY_NO=${leaveApply.APPLY_NO}&&PERSON_ID=${leaveApply.PERSON_ID}&&CONFIRM_FLAG=1" target="ajaxTodo">
		                           <span style="color:blue;">&nbsp;<spring:message code="ess.viewApply.title.pass"/><!--通过--></span>
								</a>
							    <a href="/ess/humanAffirm/confirmLeaveApply?APPLY_NO=${leaveApply.APPLY_NO}&&PERSON_ID=${leaveApply.PERSON_ID}&&CONFIRM_FLAG=2" target="ajaxTodo">
	...
	<c:set value="/ess/humanAffirm/viewLeaveApplyConfirmList" var="pageUrl"/>
