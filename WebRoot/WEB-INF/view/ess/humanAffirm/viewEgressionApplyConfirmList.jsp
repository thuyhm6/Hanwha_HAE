...
<!--
function validateEgressionConfirmApplyCallback(form,callback,flag) {
	var $form=null;
	if($('#'+form).length>0)
		$form=$('#'+form);
	else
	...
	if(!checked){
		alertMsg.error('<spring:message code="alert.message.ess.humanConfirm.choosePersonToConfirmEgressionApply"/>');
		return false;
	}

    $form.attr("action","/ess/humanAffirm/confirmEgressionApplyInBatch?CONFIRM_FLAG="+flag);
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchEgressionApplyConfirmForm");
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/humanAffirm/viewEgressionApplyConfirmList" method="post" 
	      name="searchEgressionApplyConfirmForm" id="searchEgressionApplyConfirmForm" rel="pagerForm">
	<div class="searchBar">
		<table class="searchContent">
			...
			<tr>
			    ...
			    <td>
			        <input type="text" name="seach_LEAVE_FROM_TIME" class="date required" format="yyyy-MM-dd" readonly="true" value="${LEAVE_FROM_TIME}"/>
				...
				<td>
				    <input type="text" name="seach_LEAVE_TO_TIME" class="date required" format="yyyy-MM-dd" readonly="true" value="${LEAVE_TO_TIME}"/>
				    <a class="inputDateButton" href="javascript:;"></a>
				</td> 
				<td>
				...
				<td>
				     <ait:SelectSyCodeByCpnyID name="seach_LEAVE_TYPE_CODE" parentNo="16201" cnpyID="${defaultCpny}"  selected="${LEAVE_TYPE_CODE}" limit="all"/>       
				</td>				
				<td>
				...
				<li>
					<div class="subBar">
						    <div class="buttonActive">
						         <a class="update" onclick="validateEgressionConfirmApplyCallback('updateEgressionApplyConfirmForm',DWZ.ajaxDone,'1')" href="#" >
						         <span><spring:message code="ess.title.passInBatch"/><!--批量通过--></span></a>
                            </div>
						    <div class="buttonActive">
						         <a class="update" onclick="validateEgressionConfirmApplyCallback('updateEgressionApplyConfirmForm',DWZ.ajaxDone,'2')" href="#" >
...
<form name="updateEgressionApplyConfirmForm" id="updateEgressionApplyConfirmForm" method="post" 
      action="/ess/humanAffirm/confirmEgressionApplyInBatch" class="pageForm required-validate"
	  onsubmit="return validateEgressionConfirmApplyCallback(this, navTabAjaxDone);">    
	<table class="table" width="100%" height="80%" layoutH="150" nowrapTD="false">
		<thead>
			<tr>
				...
				<th width="140"><spring:message code="ess.viewApply.title.applyDate"/><!--申请日期--></th>		
		...
		<tbody>
			<c:forEach items="${egressionApplyPersonList}" var="egressionApplyPerson" varStatus="i">			
				<tr target="sid">
				    <td>
				        <input type="checkbox" id="c1" name="c1" value="${egressionApplyPerson.APPLY_NO}" />
				    </td>
					...
					<td>${egressionApplyPerson.LEAVE_TYPE_NAME}</td>
					<td>
					...
					<td>
						<dt style="padding: 1px;">
							<c:if test="${egressionApplyPerson.ACTIVITY == 0}" >
							    <a href="/ess/humanAffirm/confirmEgressionApply?APPLY_NO=${egressionApplyPerson.APPLY_NO}&&PERSON_ID=${egressionApplyPerson.PERSON_ID}&&CONFIRM_FLAG=1" target="ajaxTodo">
		                           <span style="color:blue;">&nbsp;<spring:message code="ess.viewApply.title.pass"/><!--通过--></span>
								</a>
							    <a href="/ess/humanAffirm/confirmEgressionApply?APPLY_NO=${egressionApplyPerson.APPLY_NO}&&PERSON_ID=${egressionApplyPerson.PERSON_ID}&&CONFIRM_FLAG=2" target="ajaxTodo">
		                           <span style="color:blue;">&nbsp;<spring:message code="ess.viewApply.title.reject"/><!--否决 --></span> 
								</a>							
							</c:if>
							<c:if test="${egressionApplyPerson.ACTIVITY==1}" >
								<span style="color:green;">&nbsp;${egressionApplyPerson.LOCAL_NAME}</span>
							    <span style="color:green;">&nbsp;<spring:message code="ess.viewApply.title.pass"/><!--通过--></span>
							</c:if>	
							<c:if test="${egressionApplyPerson.ACTIVITY==2}" >
							    <span style="color:red;">&nbsp;${egressionApplyPerson.LOCAL_NAME}</span>
							    <span style="color:red;">&nbsp;<spring:message code="ess.viewApply.title.reject"/><!--否决 --></span>
							</c:if>								
						</dt>	        
                    </td>		                    														
	...
	<c:set value="/ess/humanAffirm/viewEgressionApplyConfirmList" var="pageUrl"/>
