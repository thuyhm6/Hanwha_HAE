...
<script>
	...
	var seach_APPLY_TYPE_CODE="2441";
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", "/ess/viewApply/viewLeaveInfoList?seach_KEY="+seach_KEY+"&seach_DEPT_NO="+seach_DEPT_NO+"&seach_START_DATE="+seach_START_DATE+"&seach_END_DATE="+seach_END_DATE+"&seach_APPLY_TYPE_CODE="+seach_APPLY_TYPE_CODE+"&seach_JUECAI_ZHUANGTAI="+seach_JUECAI_ZHUANGTAI+"&seach_QUEREN_ZHUANGTAI="+seach_QUEREN_ZHUANGTAI+"&seach_APPLY_TYPE_CODE="+seach_APPLY_TYPE_CODE);
 }
</script>
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ess/viewApply/viewLeaveInfoList" rel="pagerForm" method="post">
	<input type="hidden" id="menuNo" name="menuNo" value="2479"/>
	<div class="searchBar">
...
<div class="pageContent" >
	<table class="table" width="100%" layoutH="196" nowrapTD="false">
		<thead>
			<tr>
		...
		<tbody>
			<c:forEach items="${personLeaveApplyList}" var="personLeaveApply" varStatus="i">			
				<tr target="sid" rel="${personLeaveApply.PERSON_ID}">
					...
					<td>
					    ${personLeaveApply.SHIFT_START_TIME}<br>
					    ${personLeaveApply.SHIFT_END_TIME}                    
                    </td>
					<td>${personLeaveApply.APPLY_TYPE_NAME}</td>
					<td>
					    ${personLeaveApply.LEAVE_FROM_TIME}<br>
					    ${personLeaveApply.LEAVE_TO_TIME}                    
                    </td>
    ...
    <c:set value="/ess/viewApply/viewLeaveInfoList" var="pageUrl"/>
