...
function getShiftTime(flag,dateStr){
	...
	if(dateStr != null && dateStr != "" && CPNY_ID != 'LGESY'){
...
function checkAttachment(){
	...
	if(CPNY_ID == 'LGEYT'){
		if( id == 22 || id == 27 || id == 16415 || id == 24 || id == 25 || id == 482 ){
			var attFlag = $("#fileUrl").val();
			if( attFlag == ''){
				alertMsg.error("请先上传附件");
				return false;
	...
	/*if(CPNY_ID == 'LGEKS'){  //KS法人不需要这限制
		if( id == 22 || id == 27 || id == 16415 || id == 23 || id == 28 ){
			var attFlag = $("#fileUrl").val();
			if( attFlag == ''){
				alertMsg.error("请先上传附件");
				return false;
...
function submitFormPre(flag){
	$("#viewApplyLeaveInfo").attr("action","/ess/infoApplyLeave/addLeaveApply");
							...
							<table  class="user_table" width="100%">
							   ...
							   <tbody>
								   	...
								   	<tr>
								   		...
								   		<td class="td_type td_center" width="11%"><fmt:formatNumber type="number" value="${empVacInfo.USE_VAC }" maxFractionDigits="2"/></td>
								   		<td class="td_type td_center" width="11%"><fmt:formatNumber type="number" value="${empVacInfo.SURPLUS_VAC }" maxFractionDigits="2"/></td>
							...
							<table  class="user_table" width="100%">
									...
									<td width="30%" class="td_type">
										...
										<c:if test="${empty leaveApplyMap.PERSON_ID && authority eq '1'}">
											<input id="empId_apply" name="dwz.person.empId1" value="${personInfo.EMPID}" type="text" lookupGroup="person"
												onkeydown="submitKeyClick_apply_Leave(this,event)" class="required"/>
						                    <input id="empName_apply" name="dwz.person.empName1" value="${personInfo.LOCAL_NAME}" readonly style="border:0;background:transparent;"
						                    	type="text" lookupGroup="person"/>
						                    <input id="PERSON_ID" name="dwz.person.personId1" type="hidden" size="30"
												   value="${personInfo.PERSON_ID}" lookupGroup="person" rel="submitKeyClick_apply_Leave"/>
										</c:if>
								...
								<tr>
									...
									<td width="30%" class="td_type" >
									    ...
									    <c:if test="${defaultCpny eq 'LGEQH'}">
											<ait:time name="fromTime" spacing="5" selected="${leaveApplyMap.FROMTIME}" onChange="callength();"/>
									    </c:if>
									    <c:if test="${defaultCpny eq 'LGESY'}">
									    	<ait:time name="fromTime" special="07:50,12:50" selected="07:50" onChange="callength();"/>
									    </c:if>
									    <c:if test="${defaultCpny eq 'LGEKS'}">
											<ait:time name="fromTime" special="08:00,10:00,11:50,12:20,15:00,20:00,22:00,23:50,00:20,03:00" selected="${leaveApplyMap.FROMTIME}" onChange="callength();"/>
									    </c:if>
									    <c:if test="${defaultCpny ne 'TSTO' and defaultCpny ne 'LGESY' and defaultCpny ne 'LGEKS' and defaultCpny ne 'LGEQH'}">
											<ait:time name="fromTime" spacing="30" selected="${leaveApplyMap.FROMTIME}" onChange="callength();"/>
									    </c:if>
									</td>
									<td width="20%" class="td_title">
									...
									<td width="30%" class="td_type">
									    <c:if test="${defaultCpny eq 'SST'}">
					                    	<ait:SelectSyCodeCombinByCpnyID name="LEAVE_APPLY_TYPE_CODE" combinParentNo="21,18" exclude="218112,27,23619,15822,25,3326,217873" cnpyID="${defaultCpny}" selected="${leaveApplyMap.LEAVE_TYPE_CODE}" limit="all" onChangeName="xiujialeixing(this.value);callength();"/>
									    </c:if>
									    <c:if test="${defaultCpny eq 'LGEQH' or defaultCpny eq 'LGEYT'}">
					                    	<ait:SelectSyCodeCombinByCpnyID name="LEAVE_APPLY_TYPE_CODE" combinParentNo="21,18" exclude="218112,16415" cnpyID="${defaultCpny}" selected="${leaveApplyMap.LEAVE_TYPE_CODE}" limit="all" onChangeName="xiujialeixing(this.value);callength();"/>
									    </c:if>
									    <c:if test="${defaultCpny ne 'LGEQH' and defaultCpny ne 'LGEYT' and defaultCpny ne 'SST'}">
								...
								<tr>
									...
									<td width="30%" class="td_type">
										...
										<c:if test="${defaultCpny eq 'LGEQH'}">
									    	<ait:time name="toTime" spacing="5" selected="${leaveApplyMap.TOTIME}" onChange="callength();"/>
										</c:if>
										<c:if test="${defaultCpny eq 'LGESY'}">
										    <ait:time name="toTime" special="12:00,17:00" selected="17:00" onChange="callength();"/>
										</c:if>
										<c:if test="${defaultCpny eq 'LGEKS'}">
									    	<ait:time name="toTime" special="10:00,11:20,15:00,17:00,22:00,23:20,03:00,05:00" selected="${leaveApplyMap.TOTIME}" onChange="callength();"/>
										</c:if>
										<c:if test="${defaultCpny ne 'TSTO' and defaultCpny ne 'LGESY' and defaultCpny ne 'LGEKS' and defaultCpny ne 'LGEQH'}">
									    	<ait:time name="toTime" spacing="30" selected="${leaveApplyMap.TOTIME}" onChange="callength();"/>
										</c:if>
									...
									<td width="20%" class="td_title">
										<span id="view_buru_sub_title" 
									   		<c:if test="${leaveApplyMap.LEAVE_TYPE_CODE eq '16415' and defaultCpny eq 'TSTO'}">
									   			style="display:block"
									   		</c:if>
									   		<c:if test="${leaveApplyMap.LEAVE_TYPE_CODE ne '16415' or defaultCpny ne 'TSTO'}">
									   			style="display:none"
									   		</c:if>>哺乳假类型</span>
									</td>
									<td width="30%" class="td_type">
