...
<script>
function validateCallbackUpdateBusinessInfo(form, callback) {
	var $form = $("#updateBusinessInfoForm");
	if (!$form.valid()) {
		return false;
	}
	$.ajax({
...
<form id="updateBusinessInfoForm" method="post"
	action="/org/orgManage/addBusinessInfo"
	class="pageForm required-validate"
	onsubmit="return validateCallbackUpdateBusinessInfo(this, dialogAjaxDone);">
<table class="table" width="103%" layoutH="56">
		<tr>
			<th colspan="4" style="text-align:center;height:30px;font-size:14px;font-weight:bold;"><spring:message code="org.title.DEPT_BUSSINESS_SETTINGS" /><!-- 部门业务设置 -->
			<input type="hidden" name="DEPTNO" value="${DEPTNO }"/>
			</th>
		</tr>
		<c:forEach items="${codeList}" var="item" varStatus="i">
			<c:if test="${i.count%4 eq 1}">
				<tr><td width="80"><input type="checkbox" name="CODE_NO"
					value="${item.CODE_NO}" <c:if test="${item.SELECTED eq 1}">Checked = "true"</c:if>/>&nbsp;&nbsp;${item.CODENAME }</td> 
			</c:if>
			<c:if test="${i.count%4 eq 0}">
				<td width="80"><input type="checkbox" name="CODE_NO"
