...
function addRowByIDObjectTarget(currentRowID){
    ...
    htm +='<td class="td_type" style="text-align:center"><input name="ITEM_SCORE" value="0" type="text" size="5" onblur="sumObjectTargetScore()" class="required number" min="0" max="100"></td>';
...
function addRowByIDOperationalTarget(currentRowID) {
	...
	htm += '<td class="td_type" style="text-align:center"><input name="OP_ITEM_SCORE" value="0" type="text" size="5" onblur="sumOperationalTargetScore()" class="required number" min="0" max="100"></td>';
	htm += '<td class="td_type" style="text-align: center">';
	...
	htm += '<img src="/resources/images/-.gif" title="Xóa" border="0" align="absmiddle" style="cursor:hand" onclick="document.all.viewRegOperationalTarget_table.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);sumOperationalTargetScore();"/></td></tr>';

	// Thêm dòng mới sau dòng hiện tại
	$("#rowIdOperationalTarget_" + currentRowID).after(htm);

	// Gắn editor cho textarea
	var op = {html5Upload:false, skin: 'default',tools: 'Cut,Copy,Paste,|,Fullscreen'};
...
function saveObjectTarget(flag){
	...
	if(flag == 1){
		if($("#objectTargetSum",navTab.getCurrentPanel()).html() > 40){
			alertMsg.error("<spring:message code='evs.objectiveShallNotExceed40.a'/>");//比率不是40%,请确认
			return false;
		}
		if(parseInt($("#operationalTargetSum",navTab.getCurrentPanel()).html()) + parseInt($("#objectTargetSum",navTab.getCurrentPanel()).html()) != 100){
...
<div class="pageHeader">
	<form id="viewRegPersonalTargetForm" onsubmit="return navTabSearch(this);" action="/evs/manage/viewRegPersonalTarget" method="post" >
		<div class="searchBar">
	...
	<%@ include file="/WEB-INF/view/evs/manage/viewPersonalInfoHead_evs.jsp"%>
		...
		<table class="user_table" width="100%" id="viewRegPersonalTarget_table">	
			<tr id="rowIdObjectTarget_100">
				...
				<td class="td_title" style="text-align:center;" width="40%"><spring:message code="inct.salesman.eval.personal.target"/><!--目标--></td>
				<td class="td_title" style="text-align:center;" width="3%"><spring:message code="inct.salesman.ratio"/><!--比率-->(%)</td>
				<c:if test="${viewEvsObjectInfo.ACTIVITY eq '14015354' or viewEvsObjectInfo.ACTIVITY eq '14015362'}">
				<td class="td_title" style="text-align:center;" width="1%">
				<%-- <spring:message code="org.title.ADD"/> --%><!--新增--> &nbsp; <img src="/resources/images/+.gif" title="<spring:message code="ess.empInfo.insert"/>" border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDObjectTarget(100)"/>&nbsp;</td><!--添加
				--></c:if>
			</tr>
			<c:if test="${viewEvsObjectInfo.ACTIVITY eq '14015354' or viewEvsObjectInfo.ACTIVITY eq '14015362'}">
			<c:forEach items="${viewSSTEvsItem}" var="item" varStatus="i">
				<tr id="rowIdObjectTarget_${i.index }">
			    	...
			    	<td class="td_type" style="text-align:center"><input name="ITEM_NAME" value="${item.ITEM_NAME }" type="text" size="50" class="required"></td>
			    	...
			    	<td class="td_type" style="text-align:center"><input name="ITEM_SCORE" value="${item.ITEM_SCORE }" type="text" size="5" onblur="sumObjectTargetScore()" class="required number" min="0" max="100"></td>
					<td class="td_type" style="text-align: center">
						...
						--><img src="/resources/images/-.gif" title="<spring:message code="ess.empInfo.Delete"/>"	
							border="0" align="absmiddle" style="cursor:hand" onclick="javaScript:document.all.viewRegPersonalTarget_table.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);changeObjectTargetLevel();sumObjectTargetScore();"/><!--删除
					--></td>
				</tr>
			</c:forEach>
			</c:if>
		...
		<table class="user_table" width="100%" id="viewRegOperationalTarget_table">
			<tr id="rowIdOperationalTarget_100">
				<td class="td_title"  style="text-align:center;" width="1%">No</td>
				...
				<td class="td_title" style="text-align:center;" width="40%">
					<spring:message code="inct.salesman.eval.personal.target"/>
				</td>
				<td class="td_title" style="text-align:center;" width="3%">
					<spring:message code="inct.salesman.ratio"/>(%)
				</td>
				<c:if test="${viewEvsObjectInfo.ACTIVITY eq '14015354' or viewEvsObjectInfo.ACTIVITY eq '14015362'}">
					<td class="td_title" style="text-align:center;" width="1%">
						<img src="/resources/images/+.gif" title="Thêm" border="0" align="absmiddle" style="cursor:hand"
						onclick="addRowByIDOperationalTarget(100)"/>
					</td>
				</c:if>
			</tr>
			...
			<c:if test="${viewEvsObjectInfo.ACTIVITY eq '14015354' or viewEvsObjectInfo.ACTIVITY eq '14015362'}">
				<c:forEach items="${viewOpEvsItem}" var="item" varStatus="i">
					<tr id="rowIdOperationalTarget_${i.index}">
						<td class="td_type" style="text-align:center">${i.count}</td>
						<td class="td_type" style="text-align:center">
							<input name="OP_ITEM_NAME" value="${item.ITEM_NAME}" type="text" size="50" class="required">
						</td>
						<td class="td_type" style="text-align:center">
							<textarea style="width:99.5%;height:100px" name="OP_ITEM_CONTENT" class="editor required">${item.ITEM_CONTENT}</textarea>
						</td>
						<td class="td_type" style="text-align:center">
							<input name="OP_ITEM_SCORE" value="${item.ITEM_SCORE}" type="text" size="5" onblur="sumOperationalTargetScore()" class="required number" min="0" max="100">
						</td>
						<td class="td_type" style="text-align:center">
							<img src="/resources/images/+.gif" title="Thêm" style="cursor:hand"
							onclick="addRowByIDOperationalTarget('${i.index}')"/>
							&nbsp;&nbsp;&nbsp;
							<img src="/resources/images/-.gif" title="Xoá" style="cursor:hand"
							onclick="document.all.viewRegOperationalTarget_table.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);sumOperationalTargetScore();"/>
						</td>
					</tr>
				</c:forEach>
			</c:if>
			...
			<tr id="rowIdOperationalTarget_101">
				<td class="td_title"></td>
				<td class="td_title"></td>
				<td class="td_title"></td>
		...
		<input type="hidden" id="operationalTargetCnt" name="operationalTargetCnt" value="${operationalTargetCnt}">
