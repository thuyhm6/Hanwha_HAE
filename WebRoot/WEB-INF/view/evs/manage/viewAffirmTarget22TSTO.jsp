...
<div class="pageHeader">
	<form id="viewAffirmTargetForm" onsubmit="return navTabSearch(this);" action="/evs/manage/viewAffirmTarget22TSTO" method="post" >
		<div class="searchBar">
			...
			<div class="subBar">
				<ul>
					...
					<li>
						<a class="buttonActive"  onclick="downloadExcel('viewAffirmTargetForm','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=181','/evs/manage/viewAffirmTarget22TSTO')">
...
<%@ include file="/WEB-INF/view/evs/manage/viewEvsAffirmRat.jsp"%>
		...
		<table class="evsList" id="viewaffirmTarget_evsList" width="1540px;">
			<thead>
				<tr>
					...
					<th width="110px" rowspan="2"><spring:message code="hrm.empinfo.MAIN_BUSINESS_NAME"/><!--主要业务--></th>
					<th width="80px" rowspan="2"><spring:message code="hrm.empinfo.ATTEND_DATE"/><!--入社日--></th>
					<th width="360px" colspan="4">Evaluation Result</th>
			...
			<c:forEach items="${viewConfirmTarget}" var="item" varStatus="i">
				<tr>
					...
					<td>
						<c:if test="${item.LIST_TYPE ne 'POT'}">
							<a href="/evs/manage/viewConfirmTargetInfoTSTO?EVS_PERSON_ID=${item.PERSON_ID }&RESUME_SEQ=${RESUME_SEQ }&EVS_OBJECT_SEQ=${item.SEQ}&ACTIVITY=14015358" target="dialog" width="800" height="600" mask="true" style="color:blue;">${item.LOCAL_NAME }</a>
						</c:if>
						<c:if test="${item.LIST_TYPE eq 'POT'}">
							${item.LOCAL_NAME }
						</c:if>
					</td>
					<td>${item.EMPID }</td>
					...
					<td style="text-align:center">
						...
						<c:if test="${item.ACTIVITY ne '14015358' and not empty item.AFFIRM_CONTENT2}">
							<img src="/resources/images/newImages/view_en.gif" style="vertical-align:middle;cursor:pointer;" title="${item.AFFIRM_CONTENT2 }" onclick="openEvsCommentWindow('${item.ACTIVITY}','14015358',${i.index},'${item.AFFIRM_CONTENT2 }');"></img>
						</c:if>
