...
function changeURL_person(personId,resumeSEQ,seq,activity,localName){ 
	var href = "/evs/manage/viewConfirmTargetInfoSST?APPLY_PERSON_ID="+ personId + "&EVS_PERSON_ID="+ personId +"&RESUME_SEQ="+ resumeSEQ +"&EVS_OBJECT_SEQ="+ seq +"&ACTIVITY="+ 14015358 +"";
...
<div class="pageHeader">
	<form id="viewAffirmTargetForm" onsubmit="return navTabSearch(this);" action="/evs/manage/viewAffirmTarget2" method="post" >
		<div class="searchBar">
		...
		<table class="evsList" id="viewaffirmTarget_evsList" width="1700px;">
			<thead>
				<tr>
					...
					<th  width="5%" rowspan="2"><spring:message code="evs.viewEvsResult.BENREN.a"/><!--本人--></th>
					<th  width="15%" colspan="2"><spring:message code="evs.viewAffirmTarget1.YICI.a"/><!--1次--></th>
			...
			<c:forEach items="${viewConfirmTarget}" var="item" varStatus="i">
				<tr>
					...
					<%-- <td><a href="/evs/manage/viewConfirmTargetInfoSST?APPLY_PERSON_ID=${item.PERSON_ID }&EVS_PERSON_ID=${item.PERSON_ID }&RESUME_SEQ=${RESUME_SEQ }&EVS_OBJECT_SEQ=${item.SEQ}&ACTIVITY=14015358" 
					...
					<td style="text-align:center">
						<input type="text" id="evsPoint_${i.index}" sysIndex="${i.index}" name="EVS_POINT" size="7" onblur="changeEvsGrade2('${i.index}')" value="${item.EVS_POINT2 }" <c:if test="${item.ACTIVITY ne '14015358'}">disabled="disabled"</c:if>/>
