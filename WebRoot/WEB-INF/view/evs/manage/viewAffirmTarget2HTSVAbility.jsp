...
$(document).ready(function(){
	...
	$("#viewaffirmTarget_evsList",navTab.getCurrentPanel()).dataTable({"bPaginate": true,    //关闭分页
		...
		"bAutoWidth": false,    //表格宽度不自动变化
		"scrollY": $(document.body).height() - 430,
        "scrollX": true,
        "orderClasses": false,
        "oLanguage": {//多语言配置
...
function changeURL_person(personId,resumeSEQ,seq,activity,localName){ 
	...
	$.pdialog.open(href,"ess0518", localName, {width:1000,height:600,mask:true});
}
</script>
<c:if test="${not empty resumeList}">
<div class="pageHeader">
	<form id="viewAffirmTargetForm" onsubmit="return navTabSearch(this);" action="/evs/manage/viewAffirmTarget2HTSVAbility" method="post" >
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					...
					<td>
						...
						<ait:deptList name="seach_DEPTNO" limit="hr" id="viewAffirmTarget2HTSVAbility_seachDept"/>
						<ait:deptTreeIcon name="seach_DEPTNO" limit="hr" id="viewAffirmTarget2HTSVAbility_seachDept" selected="${DEPTNO}"/>
...
<div class="pageContent">
	<div style="padding-left:10px;padding-right:10px;padding-top:5px;">
		...
		<table class="evsList" id="viewaffirmTarget_evsList" width="1800px;">
			<thead>
				<tr>
					...
					<th width="140px" rowspan="2"><spring:message code="hrm.empinfo.MAIN_BUSINESS_NAME"/><!--主要业务--></th>
					<th width="80px" rowspan="2"><spring:message code="hrm.empinfo.ATTEND_DATE"/><!--入社日--></th>
					<th width="360px" colspan="4"><spring:message code="inct.salesman.eval.result"/><!-- Evaluation Result --></th>
				...
				<tr>
					...
					<th width="120px"><spring:message code="evs.viewAffirmTarget11HTSV.PINGJIADENGJIERCI.a"/><!--评价等级2次--></th>
					<th width="90px"><spring:message code="evs.viewAffirmTarget22HTSV.SANCIPINGYU.a"/><!--2次 评语--></th>
				</tr>
			</thead>
			<tbody>
			<c:forEach items="${viewConfirmTarget}" var="item" varStatus="i">
				<tr>
					...
					<%-- <td style="text-align:center"><a href="/evs/manage/viewConfirmTargetInfoHTSVAbility?APPLY_PERSON_ID=${item.PERSON_ID }&RESUME_SEQ=${RESUME_SEQ }&EVS_OBJECT_SEQ=${item.SEQ}&ACTIVITY=14015358" 
