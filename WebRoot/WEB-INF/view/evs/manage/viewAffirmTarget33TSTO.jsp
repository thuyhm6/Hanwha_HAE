...
<div class="pageHeader">
	<form id="viewAffirmTargetForm" onsubmit="return navTabSearch(this);" action="/evs/manage/viewAffirmTarget33TSTO" method="post" >
		<div class="searchBar">
			...
			<div class="subBar">
				<ul>
					...
					<li>
						<a class="buttonActive"  onclick="downloadExcel('viewAffirmTargetForm','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=180','/evs/manage/viewAffirmTarget33TSTO')">
...
<div class="pageContent">
	<div style="padding-left:10px;padding-right:10px;padding-top:5px;">
		...
		<table class="evsList" id="viewaffirmTarget_evsList" width="1620px;">
			<thead>
				<tr>
				...
				<tr>
					...
					<th width="90px"><spring:message code="evs.viewAffirmTarget22HTSV.SANCIPINGYU.a"/><!--3 次 评语--></th>
				</tr>
			</thead>
			<tbody>
			<c:forEach items="${viewConfirmTarget}" var="item" varStatus="i">
				<tr>
					...
					<td>
						...
						<a href="/evs/manage/viewConfirmTargetInfoTSTO?EVS_PERSON_ID=${item.PERSON_ID }&RESUME_SEQ=${RESUME_SEQ }&EVS_OBJECT_SEQ=${item.SEQ}&ACTIVITY=14015359" target="dialog" width="800" height="600" mask="true" style="color:blue;">${item.LOCAL_NAME }</a>
						</c:if>
					...
					<td style="text-align:center" name="${item.EVS_GRADE2 }">
					<%-- ${item.EVS_GRADE2 } --%>
						<c:choose>
					...
					<td style="text-align:center">
						<select id="evsGrade_${i.index}" name="EVS_GRADE" style="width:70px" <c:if test="${item.ACTIVITY ne '14015359'}">disabled="disabled"</c:if>>
							...
							<c:forEach items="${viewGradeList}" var="item2" varStatus="j">
								<option value="${item2.START_SCORE }" name="${item2.EVS_GRADE_NAME }" <c:if test="${item2.EVS_GRADE_NAME eq item.EVS_GRADE3}">selected="selected"</c:if>>
