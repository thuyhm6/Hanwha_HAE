...
function changeEvsGrade1(index){
...
function changeURL_person(personId,resumeSEQ,seq,activity,localName){ 
	var href = "/evs/manage/viewConfirmTargetInfoSST?APPLY_PERSON_ID="+ personId + "&EVS_PERSON_ID="+ personId +"&RESUME_SEQ="+ resumeSEQ +"&EVS_OBJECT_SEQ="+ seq +"&ACTIVITY="+ 14015357 +"";
	var localName = localName;
	$.pdialog.open(href,"ess0505", localName, {width:1000,height:600,mask:true});
}
</script>
<c:if test="${not empty resumeList}">
<div class="pageHeader">
	<form id="viewAffirmTargetForm" onsubmit="return navTabSearch(this);" action="/evs/manage/viewAffirmTarget1" method="post" >
		<div class="searchBar">
		...
		<table class="evsList" id="viewaffirmTarget_evsList" width="1700px;">
			<thead>
				<tr>
					...
					<th width="7%" rowspan="2"><spring:message code="alert.pa.pasalarycanshu.xingming"/><!--姓名--></th>
					<th width="7%" rowspan="2"><spring:message code="ess.infoApply.EMP_ID"/><!--工号--></th>
					<th width="13%" rowspan="2"><spring:message code="org.title.dept"/><!--部门--></th>
					<th width="7%" rowspan="2"><spring:message code="org.title.POST_GRADE_NAME"/><!--职级--></th>
					<th width="8%" rowspan="2"><spring:message code="evs.viewEvsParamInfoList.DUIXIANGLEIXING.a"/><!--对象类型--></th>
					<th width="8%" rowspan="2"><spring:message code="hrm.empinfo.ATTEND_DATE"/><!--入社日--></th>
					<th  width="14%" colspan="1"><spring:message code="evs.viewEvsResult.BENREN.a"/><!--本人--></th>
					<th  width="23%" colspan="2"><spring:message code="evs.viewAffirmTarget1.YICI.a"/><!--1次--></th>
					<th width="8%" rowspan="2"><spring:message code="ess.empInfo.conduct_state"/><!--进行状态--></th>
				</tr>
				<tr>
					<th width="7%"><spring:message code="inct.salesman.evalPoint"/><!-- Point --></th>
					<th width="7%" style="display: none"><spring:message code="hr.viewEvaluate.title.EV_GRADE_NAME"/><!--评价等级--></th>
					<th width="7%"><spring:message code="inct.salesman.evalPoint"/><!-- Point --></th>
					<th width="7%" style="display: none"><spring:message code="hr.viewEvaluate.title.EV_GRADE_NAME"/><!--评价等级--></th>
					<th width="9%"><spring:message code="evs.viewAffirmTarget1.KAOHEYIJIAN.a"/><!--考核意见--></th>
				</tr>
			</thead>
			<tbody>
			<c:forEach items="${viewConfirmTarget}" var="item" varStatus="i">
				<tr>
					...
					<%-- <td><a href="/evs/manage/viewConfirmTargetInfoSST?APPLY_PERSON_ID=${item.PERSON_ID }&EVS_PERSON_ID=${item.PERSON_ID }&RESUME_SEQ=${RESUME_SEQ }&EVS_OBJECT_SEQ=${item.SEQ}&ACTIVITY=14015357" 
					...
					<td style="text-align:center">
						<input type="text" id="evsPoint_${i.index}" sysIndex="${i.index}" name="EVS_POINT" size="7" onblur="changeEvsGrade1('${i.index}')" value="${item.EVS_POINT1 }" <c:if test="${item.ACTIVITY ne '14015357'}">disabled="disabled"</c:if>/>
						<input type="hidden" id="evsSeq_${i.index}" value="${item.SEQ1 }"/>
					</td>
					<td style="text-align:center;display: none">
						<select id="evsGrade_${i.index}" name="EVS_GRADE" style="width:70px" disabled="disabled">
							...
							<c:forEach items="${viewGradeList}" var="item2" varStatus="j">
								<option value="${item2.START_SCORE }" name="${item2.EVS_GRADE_NAME}" <c:if test="${item2.EVS_GRADE_NAME eq item.EVS_GRADE1}">selected="selected"</c:if>>
									<c:choose>
