							...
							<span>
								<c:choose>
								   <c:when test="${add_name eq '' || add_name eq null}">
								     	<spring:message code="ess.empInfo.insert" />
								   </c:when>
								   <c:otherwise>
								   		${add_name}
								   </c:otherwise>
			...
			<c:if test="${delete_Url ne '' && delete_Url ne null}">
				<li id="deleteLi">
					<a class="delete" href="${delete_Url}"
						...
						<c:if test="${delete_callback ne '' && delete_callback ne null }">
						...
						<span>
							...
							<c:choose>
							   <c:when test="${delete_name eq '' || delete_name eq null}">
							     	<spring:message code="org.title.DELETE" />
							   </c:when>
							   <c:otherwise>
							   		${delete_name}
							   </c:otherwise>
							...
							><span>
								<c:choose>
								   ...
								   <c:when test="${edit_name eq '' || edit_name eq null}">
								     	<spring:message code="ess.empInfo.modify" />
								   </c:when>
								   <c:otherwise>
								   		${edit_name}
								   </c:otherwise>
