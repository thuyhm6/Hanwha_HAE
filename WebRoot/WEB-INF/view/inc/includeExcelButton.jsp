...
<div class="formBar">
	<ul class="toolBar">
		...
		<c:if test="${toolbarInfo.UPDATER == '1'}">
			<c:if test="${excelT_Url ne '' && excelT_Url ne null}">
				<li id="downloadExelTemplate">
					<a class="downloadExel"
						href="${excelT_Url}">						
							<span>
		...
		<c:if test="${toolbarInfo.UPDATER == '1'}">
			<c:if test="${excelU_Url ne '' && excelU_Url ne null}">
				<li id="uploadExel">
					<a class="downloadExel"
						href="${excelU_Url}">						
							<span>
		...
		<c:if test="${excelD_Url ne '' && excelD_Url ne null}">
			<li id="downloadToExel">
				<a class="downloadExel"
					href="${excelD_Url}">						
						<span>
		...
		<c:if test="${toolbarInfo.INSERTR == '1'}">
			 <c:if test="${req_Url ne '' && req_Url ne null}">
				<li id="reqToConfirm">
					<a class="add" href="${req_Url}"
						<c:if test="${req_target_exit eq '' || req_target_exit eq null }">
							target="${req_tab eq '' || req_tab eq null ? 'dialog' : req_tab}" 
						</c:if>
							mask="${req_mask eq '' || req_mask eq null ? 'true' : req_mask }" 
							width="${req_width eq '' || req_width eq null ? '800' : req_width}" 
							height="${req_height eq '' || req_height eq null ? '400' : req_height}"
							<c:if test="${req_rel ne '' && req_rel ne null}">
								rel="${req_rel}"
							</c:if>
							>
							<span>
		...
		<c:if test="${toolbarInfo.UPDATER == '1'}">
			<c:if test="${addReq_Url ne '' && addReq_Url ne null}">
				<li id="addReq_Url">
					<a class="add" href="${addReq_Url}">
						<span>
							<c:choose>
							   <c:when test="${addReq_name eq '' || addReq_name eq null}">
							     	申请
							   </c:when>
							   <c:otherwise>
							   		${addReq_name}
