...
<div class="pageHeader">
	<form id="viewJobTypeExcelImportResult" name="viewJobTypeExcelImportResult"
			action="/hrm/jobType/viewImportExcelTempJobTypeDataList" 
...
<div class="pageContent">	
	...
	<div class="formBar">
		<ul>			
			...
			href="/hrm/jobType/downloadJobTypelateByExcelData"><span>
			...
			href="/hrm/jobType/submitImportExcelJobTypeData"  title="确定要提交吗?"><span>
	...
	<table class="table" width="100%" layoutH="220" nowrapTD="false">
		...
		<tbody>
			<c:forEach items="${jobTypeTempList}" var="item" varStatus="i">			
				<tr>
				    <td class='td_center'>${item.LINE_ID}</td>
					<td class='td_center'>${item.CPNY_ID}</td>
					<td class='td_center'>${item.CPNY_LOCATION}</td>
					<td class='td_center'>${item.JOBTYPE_GROUP_NO}</td>
					<td class='td_center'>${item.JOBTYPE_GROUP_NAME}</td>
					<td class='td_center'>${item.JOBTYPE_NO}</td>
					<td class='td_center'>${item.JOBTYPE_NAME}</td>
	...
	<c:set value="/hrm/jobType/viewImportExcelTempJobTypeDataList" var="pageUrl"/>
