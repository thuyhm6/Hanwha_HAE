...
<!--
function cancelTransValidateCallback(form, callback,flag) {
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchTransactionTransForm");
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/hrm/searchTransferOrder/viewTransactionTransViewList"
	rel="pagerForm" method="post" id="searchTransactionTransForm" name="searchTransactionTransForm">
	<div class="searchBar">
				...
				<li>
					<div class="subBar">
						    ...
						    <div class="buttonActive">
						         <a class="update" onclick="return cancelTransValidateCallback('updateTransForm',DWZ.ajaxDone,'');" href="#" ><span><spring:message code="hr.viewTransactionTransViewList.title.CANCLETRANS"/><!-- 取消发令 --></span></a>
                            </div>
				    </div>	
				</li>
		</tr>
...
<form name="updateTransForm" id="updateTransForm" method="post" action="/hrm/searchTransferOrder/cancelTransactionTransInBatch"
	  onsubmit="return cancelTransValidateCallback(this, navTabAjaxDone);" rel="pagerForm">
 	<table class="table" width="120%" layoutH="150" nowrapTD="false">      
		<thead>
			<tr>
				...
				<th width="80" orderField="EMPID" class="${orderDirection}">
					<spring:message code="hr.viewPersonalInfo.title.EMPID"/>
					<!--社号-->
				</th>
				<th width="80" orderField="nlssort(LOCAL_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewPersonalInfo.title.LOCAL_NAME"/>
					<!--姓名-->
				</th>
				<th width="100" orderField="nlssort(EMP_DEPT_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewTransactionTransViewList.title.OLD_DEPTNO"/>
					<!--原部门-->
				</th>
				<th width="80" orderField="nlssort(EMP_POSITION_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewTransactionTransViewList.title.OLD_POSITIONNO"/>
					<!--原职(岗)位-->
				</th>
				<th width="140" orderField="nlssort(EMP_POST_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewTransactionTransViewList.title.OLD_POST_NAME"/>
					<!--原职级名称(职务)-->
				</th>
			    <th width="40" orderField="nlssort(EMP_POST_GRADE_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
			    	<spring:message code="hr.viewTransactionTransViewList.title.OLD_POST_GRADE_NAME"/>
			    	<!--原职级-->
			    </th>
			    <th width="100" orderField="nlssort(EMP_DUTY_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
			    	<spring:message code="hr.viewTransactionTransViewList.title.OLD_DUTY_NAME"/>
			    	<!--原职责-->
			    </th>
			    <th width="80" orderField="nlssort(EMP_WORK_AREA_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
			    	<spring:message code="hr.viewTransactionTransViewList.title.OLD_WORK_AREA"/>
			    	<!--原工作地-->
			    </th>
				<th width="100" orderField="nlssort(NEW_DEPT_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewPersonalInfo.title.XIANBUMEN"/>
					<!--现部门-->
				</th>
				<th width="80" orderField="nlssort(NEW_POSITION_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewPersonalInfo.title.XIANZHIWEI"/>
					<!--现职位-->
				</th>
				<th width="120" orderField="nlssort(NEW_POST_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewPersonalInfo.title.POST_NAME"/>
					<!--职级名称(职务)-->
				</th>
			    <th width="40" orderField="nlssort(NEW_POST_GRADE_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
			    	<spring:message code="hr.viewPersonalInfo.title.XIANZHIJI"/>
					<!--现职级-->
			    </th>		
			    <th width="100" orderField="nlssort(NEW_DUTY_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
			    	<spring:message code="hr.viewPersonalInfo.title.XIANZHIZE"/>
					<!--现职责-->
			    </th>		    
				<th width="80" orderField="nlssort(NEW_WORK_AR_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewPersonalInfo.title.WORK_AREA_NAME"/>
					<!--工作地-->
				</th>
				<th width="160" orderField="nlssort(TRANS_TYPE_NAME,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
					<spring:message code="hr.viewUpgrade.title.UPGRADETYPE"/>
					<!--调动类型-->
				</th>	
				<th width="140" orderField="nlssort(START_DATE,'NLS_SORT=SCHINESE_PINYIN_M')" class="${orderDirection}">
