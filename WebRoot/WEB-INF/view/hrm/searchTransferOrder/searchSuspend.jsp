...
function cancelSuspendTransValidateCallback(form, callback) {
    ...
    var checked=false;
	var ids= document.getElementsByName("searchSus");
    ...
    $form.attr("action","/hrm/searchTransferOrder/cancelSuspendInBatch");
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchSuspendForm");
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/hrm/searchTransferOrder/searchSuspend" 
	rel="pagerForm" method="post" name="searchSuspendForm" id="searchSuspendForm">
	<div class="searchBar">
		...
		<div class="formBar">
			...
			<label style="float: left">
				<input type="checkbox" class="checkboxCtrl" group="searchSus" />
				...
				<li>
					<div class="subBar">
						    ...
						    <div class="buttonActive">
						         <a class="update" onclick="return cancelSuspendTransValidateCallback('updateSuspendForm',DWZ.ajaxDone);" href="#" ><span><spring:message code="hr.viewTransactionTransViewList.title.CANCLETRANS"/><!-- 取消发令 --></span></a>
                            </div>
				    </div>	
				</li>
		</tr>
...
<form name="updateSuspendForm" id="updateSuspendForm" method="post" action="/hrm/searchTransferOrder/cancelSuspendInBatch"
	  onsubmit="return cancelSuspendTransValidateCallback(this, navTabAjaxDone);">
 	<table class="table" width="120%" layoutH="150" nowrapTD="false">      
		...
		<tbody>
			<c:forEach items="${hrSuspendList}" var="suspend" varStatus="i">			
				<tr target="sid" rel="${suspend.EXP_INSIDE_NO}">
				    <td>
				        <input type="checkbox" id="searchSus" name="searchSus" value="${suspend.EXP_INSIDE_NO}" />
				    </td>
					<td>${suspend.EMPID}</td>
					...
					<td> <a href="/hrm/searchTransferOrder/viewEmployeeSuspendHistoryList?EXP_INSIDE_NO=${suspend.EXP_INSIDE_NO}&&PERSON_ID=${suspend.PERSON_ID}" target="navTab">
