...
<div class="pageContent">
	<form id="updateFamilyInfo" method="post" action="/hrm/empinfo/editFamilyInfo" class="pageForm required-validate" onsubmit="return validateCallbackUpdateFamilyInfo(this, dialogAjaxDone);">
<input type="hidden" name="PERSON_ID" size="13"  value="${PERSON_ID }"/>
<input type="hidden" name="isEssSystem" size="13"  value="${isEssSystem }"/>
	<div class="panelBar">
		<ul class="toolBar">
			<li id="addLi">
				<span>&nbsp;</span>
		...
		<tbody>
			<c:forEach items="${familyList}" var="item" varStatus="i">
				...
				<tr>
					...
					<td><ait:SelectSyCodeByCpnyID onClickName="setCheckboxChecked(${i.count})" name="FAM_TYPE_CODE_${item.FAMILY_NO}" parentNo="1693" cnpyID="${defaultCpny}" selected="${item.FAM_TYPE_CODE }" /></td>
					
					<td><input   type="text" name="FAM_NAME_${item.FAMILY_NO}" class="textInput" size="13" maxlength="13" value="${item.FAM_NAME}" onclick="setCheckboxChecked(${i.count})"/></td>
					
					<%--<td><input type="text" name="FAM_IDCARD_${item.FAMILY_NO}"  class="textInput" maxlength="30" value="${item.FAM_IDCARD}"/></td>
					
					<td>
						<input type="text"  name="FAM_BORNDATE_${item.FAMILY_NO}" class="date required" value="${item.FAM_BORNDATE}" size="12" readonly="true" format="yyyy-MM-dd" yearstart="-50" yearend="5" onClick="setdate(this);"/>
					</td>
					
					<td><input type="text" name="FAM_ADDRESS_${item.FAMILY_NO}"  class="textInput" maxlength="100" value="${item.FAM_ADDRESS }"/></td>
					
					--%><td><input onclick="setCheckboxChecked(${i.count})" type="text" name="FAM_PHONE_${item.FAMILY_NO}"  class="textInput" maxlength="20" value="${item.FAM_PHONE }"/></td>
