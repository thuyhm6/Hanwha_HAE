...
function fangdajingTRAINING(flag) {
	var name = encodeURI(encodeURI($('#seach_KEYTRAINING').val()));
	...
	$('#fangdaTRAINING')
			.attr(
					...
					'/hrm/empinfo/viewEmpInfoListTanchu?dataSearch='
							...
							+ name);
	if (flag == 'onkeyup')
		$('#fangdaTRAINING').click();
}

function chaxunTRAINING(aa) {
	$('#TrainingProcessSearch').submit();
}
function chooseLowerTRAINING(aa) {
...
function shanchuTRAINING() {
	$('#titlenameTRAINING').html('');
	$('#PERSON_IDTRAINING').attr('value', '');
	$('#LOCAL_TITLETRAINING').attr('value', '');
}
...
function changeTRAININGINFOR(no, status, id, aid) {
	var idvalue = $('#' + id).val();
...
function changeZhijiTRAININGINFOR(status, id, aid) {
	var idvalue = $('#' + id).val();
	var parentnoTRAININGINFOR = $('#POST_FAMILY_TRAININGINFOR').val();
	var idhref = '/hrm/empinfo/searchTanchu?PARENT_CODE_NO='+ parentnoTRAININGINFOR + '&firstFlag=N&status='+status+'&nameid=' + id + '&typeFlag=Y&idvalue=' + idvalue;
	$('#' + aid).attr('href', idhref);
}
</script>
<form id="TrainingProcessSearch" method="post"
		action="/hrm/empinfo/TrainingProcessSearch" class="pageForm required-validate"
		onsubmit="return navTabSearch(this);">
		
		<input type="hidden" id="PERSON_IDTRAINING" name="PERSON_ID"
				value="${PERSON_ID }">
			<input type="hidden" id="LOCAL_TITLETRAINING" name="LOCAL_TITLE"
				value="${LOCAL_TITLE }">
			...
			<td style="height:0px; width:1px;">
				...
				<td style="border: 0px">
                    ...
                    <input type="checkbox" name="lowerDepart" id="lowerDepart_family"
						value="N" onclick="chooseLowerTRAINING('training')">
					<spring:message code="hrm.empinfo.Department_include" /><!--下位部门包括--></td>
		</tr>
			<tr>
				 <td width="5%" style="padding-top: 4px">
					<spring:message code="ar.viewcycleparameter.title.qujian" /><!--入职日期-->
				</td>
			<td width="5%" style="padding-top: 4px">
...
<div id="family" style="width: 100%;margin-left:auto;margin-right:auto;">
		...
		<div class="formBar">
			...
			<ul class="toolBar">
				...
				<li>
					<a class="buttonActive" onclick="chaxunTRAINING('training')" href="#">
					<span><spring:message code="button.search" /><!--查询--></span>
					</a>
				</li>
					<%-- <li>
					<a class="buttonActive" href="/pa/excelExport/downloadExcelTemplateHAE?file=TrainingProcess_Info_Temp">
					<span><spring:message code="ar.addempshift.title.downloadmodule" /><!-- 下载导入模板 --></span>
				</a>
				</li>
				<li>
					<a class="add" href="/pa/excelImport/importExcelData?importFunName=/importTrainingProcess" target="dialog" mask="true">
						<span><spring:message code="ar.addempshift.title.excelimport" /><!-- EXCEL导入 --></span>
					</a>
				</li> --%>
				<li>
					<a class="buttonActive"
						href="/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=361">
		...
		<thead>
				<tr>
					...
					<th width="5%">
						<spring:message code="hrm.empinfo.TRAIN_ADDRESS"/><!--trainingprogram -->
					</th>
					<th width="5%">
						<spring:message code="hrm.empinfo.training_distinction"/><!-- startdate -->
					</th>
					<th width="5%">
						<spring:message code="hrm.empinfo.TRAIN_curriculum"/><!--enddate -->
					</th>
					<th width="5%">
						<spring:message code="hrm.empinfo.Training_form" /><!--年龄-->  
					</th>
					<th width="5%">
						<spring:message code="hrm.empinfo.START_DATE" /><!--出生日期-->  
					</th>
					<th width="5%">
						<spring:message code="hrm.empinfo.END_DATE" /><!--家庭电话-->
					</th>
					<th width="5%">
						<spring:message code="hrm.empinfo.TRAINING_RESULT" /><!--家庭电话-->
					</th>
					<th width="5%">
						<spring:message code="org.title.UPDATED_IP" /><!--家庭电话-->
					</th>
					<th width="5%">
						<spring:message code="org.title.UPDATE_DATE" /><!--家庭电话-->
					</th>
				</tr>
			</thead>
			<c:forEach items="${TrainingProcessSearch }" var="a" varStatus="i">
				<tr>
					<td class="td_type">${i.count }</td>
					<td class="td_type">${a.EMPID }</td>
					<td style="text-align: center">${a.LOCAL_NAME}</td>
					<td class="td_type">${a.DEPTNO_NAME}</td>
					<td class="td_type">${a.POST_GRADE_NO_NAME }</td>
					<td class="td_type">${a.TRAIN_ADDRESS }</td>
					<td class="td_type">${a.TRAIN_DIFF_CODE_NAME }</td>
					<td class="td_type">${a.COURSE_NAME_CODE}</td>
					<td class="td_type">${a.TRAIN_FORM_CODE_NAME}</td>
					<td class="td_type">${a.IMPLE_START_DATE }</td>	
					<td class="td_type">${a.IMPLE_END_DATE }</td>
					<td class="td_type">${a.EVA_RESULT }</td>
					<td class="td_type">${a.UPDATE_BY_NAME }</td>
					<td class="td_type">${a.UPDATE_DATE }</td>
				</tr>
