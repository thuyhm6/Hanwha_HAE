...
<div class="pageHeader">
	<form id="viewEvalDataExcelImportResult" name="viewEvalDataExcelImportResult"
			action="/hrm/empinfo/viewImportExcelTempDisableDataList" 
...
<div class="pageContent">	
	...
	<div class="formBar">
		<ul>			
			...
			href="/hrm/empinfo/downloadInfoByExcelData11?type=2"><span>
			...
			href="/hrm/empinfo/submitImportInfoExcelTempData?accrual=disable"  title="确定要提交吗?"><span>
	...
	<table class="table" width="100%" layoutH="220" nowrapTD="false">
		...
		<tbody>
			<c:forEach items="${paTempDisabledTempList}" var="item" varStatus="i">			
				<tr>
					<td class='td_center'>${item.LINE_ID}</td>
					<td class='td_center'>${item.T_ID}</td>
					<td class='td_center'>${item.PERSON_ID}</td>
					<td class='td_center'>${item.DISABILITY_TYPE}</td>
					<td class='td_center'>${item.ADDDATE}</td>
					<td class='td_center'>${item.DISABILITY_VALIDITY}</td>
	...
	<c:set value="/hrm/empinfo/viewImportExcelTempDisableDataList" var="pageUrl"/>
