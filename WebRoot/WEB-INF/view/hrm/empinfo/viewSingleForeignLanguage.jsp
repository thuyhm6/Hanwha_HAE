...
function validateAddResumeInfoCallback(form,callback) {
	...
	if($("#LANGUAGE_LEVEL").val() ==='' || $("#LANGUAGE_LEVEL").val()===null){
    	alertMsg.error("<spring:message code="hrm.alert.empinfo.LANGUAGE_LEVEL" />");//等级不能为空
      	return  false;
    }
...
function validateDeleteResumeInfoCallback(form,callback) {
	...
	alertMsg.confirm("<spring:message code="hrm.alert.empinfo.Sure.delete"/>",//确定要删除吗？
		  	...
		  	$.ajax({
  				type: form.method || 'POST',
  				url:'/hrm/empinfo/deleteForeignLanguage',
  				data:$form.serializeArray(),
  				dataType:"json",
  				cache: false,
  				success: callback || DWZ.ajaxDone,
...
function blanguage(){
	var check=$('#blanguage').prop('checked');
	if(check==true){
		$('#BASIC_LANGUAGE').attr('value','Y');
	}else{
		$('#BASIC_LANGUAGE').attr('value','N');
	}
}

//附件上传
function uploadifySuccess_ForeignLanguagefile(file, data, response){
...
<div>
<form id="editForeignLanguage" method="post"
	action="/hrm/empinfo/editForeignLanguage"
...
<input TYPE="hidden" NAME="SINGLE_PERSON_ID" id="SINGLE_PERSON_ID"
	VALUE="${PERSON_ID}"> <input type="hidden" name="LANGUAGE_NO"
	id="LANGUAGE_NO" value="${personInfo.LANGUAGE_NO}">
<div>
				...
				<table class="user_table" width="100%">
					<tr>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.LANGUAGE" /><!--语言--></td>
						<td width="35%" class="td_type">
							<input type="text"  id="LANGUAGE_TYPE" name="LANGUAGE_TYPE" value="${personInfo.LANGUAGE_TYPE }">
						</td>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.Whether_basic_language" /><!--是否为基本语言--></td>
						<td><c:if test="${personInfo.BASIC_LANGUAGE=='Y' }">
							<input type="checkbox" id="BASIC_LANGUAGE" checked="checked"
								name='BASIC_LANGUAGE' value="Y">
						</c:if> <c:if test="${personInfo.BASIC_LANGUAGE!='Y' }">
							<input type="checkbox" id="BASIC_LANGUAGE" name='BASIC_LANGUAGE'
								value="Y">
						</c:if></td>
					</tr>
					...
					<tr>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.SAY" /><!--说--></td>
						<td width="35%" class="td_type"><ait:SelectSyCodeByCpnyID
							name="SAY" id="SAY" parentNo="14014324"
							cnpyID="${defaultCpny}" selected="${personInfo.SAY}"
							limit="all" /></td>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.READ" /><!--读--></td>
						<td width="35%" class="td_type"><ait:SelectSyCodeByCpnyID
							name="READ" id="READ" parentNo="14014324"
							cnpyID="${defaultCpny}" selected="${personInfo.READ}"
							limit="all" /></td>
					</tr>
					<tr>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.WRITE" /><!--写--></td>
						<td width="35%" class="td_type"><ait:SelectSyCodeByCpnyID
							name="WRITE" id="WRITE" parentNo="14014324"
							cnpyID="${defaultCpny}" selected="${personInfo.WRITE}"
							limit="all" /></td>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.comprehensive" /><!--综合--></td>
						<td width="35%" class="td_type"><ait:SelectSyCodeByCpnyID
							name="COMBINED" id="COMBINED" parentNo="14014324"
							cnpyID="${defaultCpny}" selected="${personInfo.COMBINED}"
							limit="all" /></td>
					</tr>
					</c:if>
					...
					<tr>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.LISTEN.Z" /><!--听--></td>
						<td width="35%" class="td_type"><ait:SelectSyCodeByCpnyID
							name="LISTEN" id="LISTEN" parentNo="14014324"
							cnpyID="${defaultCpny}" selected="${personInfo.LISTEN}"
							limit="all" /></td>
						<td width="15%" class="td_title"><spring:message
							code="hrm.empinfo.SAY" /><!--说--></td>
						<td width="35%" class="td_type"><ait:SelectSyCodeByCpnyID
					...
					<tr>
						...
						<td width="85%" class="td_type" colspan="3"><textarea name="REMARK"
							id="REMARK" style="width: 600px; height: 80px">${personInfo.REMARK}</textarea>
						</td>
					</tr>
				</table>
				</td>
...
</c:if> <c:if test="${language_no!='0' }">
	<div>
	<div style="font: 10px; float: left; height: 20px; line-height: 20px;">
	<a class="w_button" href="#"
		onclick="uploadAttDialog_new('viewResumeList_viewForeignLanguageunit','/hrm/empinfo/viewSingleForeignLanguage?PERSON_ID=${PERSON_ID}$LANGUAGE_NO=${language_no }','${language_no}','hrLanguageLevel','<spring:message code="js.upload.msg.saveAndUpload" />','<spring:message code="js.upload.msg.fileToUpload" />')"><span><spring:message
		code="hrm.recruitManage.ATTACHED_FILE" /><!--附加文件--></span></a> <a class="w_button" href="#"
		onclick="deleteAttList_new('viewResumeList_viewForeignLanguageunit','/hrm/empinfo/viewSingleForeignLanguage?PERSON_ID=${PERSON_ID}&LANGUAGE_NO=${language_no }',divAjaxDone,'<spring:message code="js.upload.msg.firstSelectDelete" />','<spring:message code="js.upload.msg.confirmToDelete" />')"><span><spring:message
		code="button.delete" /><!--删除--></span></a></div>
