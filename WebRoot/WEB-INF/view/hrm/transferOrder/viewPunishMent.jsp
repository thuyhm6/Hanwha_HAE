...
function validateCallbackPunishMent(form,callback) {	
	var $form = $("#punishMent");
	...
	$form.find(":checkbox[id='punishMent']").each(function(index, checkBoxObj){
...
function navTabSearchPunishMent(form, navTabId){
	var $form = $("#searchpunishMent");
...
<div class="pageHeader">
	<form id="searchpunishMent" onsubmit="return navTabSearchPunishMent(this);" action="/hrm/transferOrder/viewPunishMent" method="post" rel="pagerForm">
	<div class="searchBar">
...
<div class="pageContent" style="padding:5px;">
	<form style="margin:0px;padding:0px;" id="punishMent" onsubmit="return validateCallbackPunishMent(this,navTabAjaxDone);" action="/hrm/transferOrder/savePunishMent" method="post"  class="pageForm required-validate">
	<div class="formBar">
			<tr>	
	...
	<table width="100%" class="tablea" layoutH="145">
		...
		<tbody>
			<c:forEach items="${punishMentList}" var="punishMent">
				<tr target="sid" rel="${punishMent.EMPID}">
					<td>
						<input type="checkbox" id="punishMent" name="punishMent" value="${punishMent.EMPID}" />
						<input type="hidden" name="PERSON_ID_${punishMent.EMPID}" value="${punishMent.PERSON_ID}"/>
						<input type="hidden" name="DEPTNO_${punishMent.EMPID}" value="${punishMent.DEPTNO}"/>
						<input type="hidden" name="POSITION_NO_${punishMent.EMPID}" value="${punishMent.POSITION_NO}"/>
						<input type="hidden" name="POST_NO_${punishMent.EMPID}" value="${punishMent.POST_NO}"/>
					</td>
					<td>${punishMent.EMPID}</td>
					<td>${punishMent.LOCAL_NAME}</td>
					<td>
						${punishMent.DEPT_NAME}
					</td>
					<td>
						${punishMent.POSITION_NAME}
					</td>
					<td>						
						${punishMent.POST_NAME}
					</td>
					<td>
						<input id="DATE_PUNISHED" name="DATE_PUNISHED_${punishMent.EMPID}" size="10" class="date" >
					</td>
					<td>
						<ait:SelectSyCodeByCpnyID name="TRANS_CODE_${punishMent.EMPID}" parentNo="642" cnpyID="${defaultCpny}"/>
					</td>
					<td>
						<input id="PUN_BONUS" name="PUN_BONUS_${punishMent.EMPID}" size="10" class="number" maxlength="7">
					</td>
					<td>
						<input id="PUN_REASON" name="PUN_REASON_${punishMent.EMPID}" size="10" maxlength="60">
					</td>						
				</tr>
			</c:forEach>
		</tbody>
			...
			<c:set value="/hrm/transferOrder/viewPunishMent" var="pageUrl"/>
