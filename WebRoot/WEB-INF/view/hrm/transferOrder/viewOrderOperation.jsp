...
<%--		document.diaoling.submit();--%>
			...
			if(type1=="1"){
				$("input[name='chexkbox']:checkbox:checked").each(function(){ 
					checkValue+=$(this).val()+"-" ;
				}) 
			}
			if(type1=="2"){
				$("input[name='chexkbox']:checkbox:").each(function(){ 
					checkValue+=$(this).val()+"-" ;
			...
			var nameValue=document.getElementsByName("cloumeNumValue");
			
			var checkValues1=checkValue.split("-");
			
			var checkValues2="";
			for(var i=0;i<checkValues1.length;i++){
				
				var checkValues3=checkValues1[i].split(",");
				if(checkValues3[1]!=null&&checkValues3[1]!=""){
					checkValues2+=checkValues3[1]+"-";
				}
			}
			
			var checkValues=checkValues2.split("-");
			for(var i=0; i<checkValues.length;i++){
				if(checkValues[i]==null||checkValues[i]==""){
					continue;
				}
				var empidValue=document.getElementById("EMPID_"+checkValues[i]).value;
				if(empidValue==""){
					document.getElementById("EMPID_"+checkValues[i]).focus();
					alertMsg.error("请填写社号");
					return false;
				}
			}
			for(var i=0; i<checkValues.length;i++){
				...
				var empidValue=document.getElementById("TRANS_ORDER_DATE_"+checkValues[i]).value;
				
				if(empidValue==""){
					document.getElementById("TRANS_ORDER_DATE_"+checkValues[i]).focus();
			...
			if (confirm ('<spring:message code="hr.viewEvaluate.title.COMMIT_CONFIRM"/>')){
				var $form = $("#diaolingForm");	
			  	$.ajax({
					type: 'POST',
					url:"/hrm/transferOrder/SaveHrExperienceInside",
	...
	function downloadImportTemplate(){
		...
		if(selectedType != ''){
			var url = "/pa/excelExport/exportDiaoLingInputItemDataExcelIsNullModuleType?code="+selectedType+"&navTabId=hr0998";
	...
	function getCurrentTransferOrderType(){
		var selectedType=$("#OrderType :selected").val();
		if(selectedType != ''){
			$("#importExcel").attr('href','/pa/excelImport/importExcelData?importFunName=/importOrderOperationExcel&transferOrder_type='+selectedType);
			$("#importExcel").click();
...
<input type="hidden" value="" id="diaolingType" name="type"/>
	<div class="panel">
		...
		<div>
			<%@ include file="/WEB-INF/view/hrm/empinfo/viewPersonalInfoHead4.jsp"%>
		</div>
	</div>
		<table width="100%" >
			<tr>
				...
				<td align="right">
					...
					<a style="float:right; " class="buttonActive"  onclick="validateCallbackViewOrderOperation(this,navTabAjaxDone,2)" href="#">
						<span>
