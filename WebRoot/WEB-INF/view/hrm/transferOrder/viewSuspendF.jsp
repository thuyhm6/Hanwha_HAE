 ...
 <head>   
   <meta http-equiv="X-UA-Compatible" content="IE=edge" >
    ...
    $(function ()
    {
		$("#mainBody").hide();
        f_initGrid();
        $("#BandhType").ligerComboBox({width:"100",selectBoxWidth:"100",
        	onSelected:function(val,text){
        		var type='';
        		if(val == 'BandhType0020'){
            		type='F';	
        		}else if(val == 'BandhType0030'){
        			type='D';
        		}else if(val == 'BandhType0040'){
        			type='Z';
        		}

        		location.href = 'viewSuspend'+type;
        	}
    	});          
    ...
    function f_initGrid()
    {
    	$grid = $("#transfer").ligerGrid({
    		...
    		{ display: '复职日期', name: 'STARTDATE',width:110 },
    		{ display: '复职事由', name: 'REASON',width:270 }
            ],
            enabledEdit: true,
            dataAction: 'server', 
            sortName: 'EMPID',
            pageSize: 10,
            url: '/hrm/transferOrder/getTransferOrderList?suspendf=1',
            width: '100%', 
            height: '400'
        });
    }
    ...
    function search(){
		$("#l-search").hide();
		$.ligerWindow.show( { url:'/hrm/transferOrder/searchEmp?type=suspendf',
    ...
    function getSelected(obj,rowindex){				
		...
		if(obj.checked){
			...
			$(rowobj).children("TD[columnname='REASON']").children("div").html("<input id='REASON_"+rowindex
					+"' name='REASON_"+rowindex+"' type='text' value=''>");
			...
			$("#REASON_"+rowindex).ligerTextBox({width:"260"});
			...
			$(rowobj).children("TD[columnname='REASON']").children("div").html('');
		}	 
    }
	...
	function save(){
		...
		if(flag == 0){
            ...
            var options = {	                        
            url:'/hrm/transferOrder/saveTransferOrder?type=suspendf',
            type:'POST',	                        
            success: function(){
...
<body style="padding:5px" >
		...
		<div id="mainBody">
			<div class="l-page-top">
		     	...
		     	<input type="button" id="l-search" class="l-button" value="搜索" onclick="search();">	     
		  		<div>
			  		<table><tr>
			  		<td>发令类型：&nbsp;&nbsp;<td>
					<td><ait:selectSyCode parentNo="BandhType" name="BandhType"
						selected="BandhType0020"/></td>
			  		</tr></table>
		  		</div>
		  	</div>
			...
			<form id="applyForm">
			   	...
			   	<input type="hidden" name="TRANSCODE" value="BandhType0020">
		   	</form>       
      	</div>
</body>
</html>
