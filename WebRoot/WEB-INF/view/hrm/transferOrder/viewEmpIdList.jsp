...
<div class="pageHeader">
	...
	<form method="post" action="/hrm/transferOrder/viewEmpIdList?navTabId=${param.navTabId}&seach_NAVID=${seach_NAVID}&empid=${EMP}&viewEmpIdListColnum=${EMP}" onsubmit="return dwzSearch(this,'dialog')" rel="pagerForm">
		...
		<table class="searchContent">
			<tr>
				...
				<td>
					...
					<input type="hidden" name="seach_EMPID" value="${LOCAL_NAME}"/>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			<c:forEach items="${empList}" var="item">
				...
				<tr id="EMPID1" target="EMPID" rel="${item.EMPID}" ondblclick="F_HR_ShowMore1('${item.PERSON_ID}','${item.LOCAL_NAME}','${item.EMPID}','${EMP}','${item.DEPTNAME}','${item.DEPTNO}','${item.GRADELEVEL}','${item.GRADELEVELNAME}','${item.DUTYNO}','${item.DUTYNONAME}','${item.POSTNO}','${item.POSTNONAME}','${item.POSTGRADENO}','${item.POSTGRADENONAME}','${item.POSITIONNO}','${item.POSITIONNONAME}')">
					<td>${item.EMPID}</td>
					...
					<td>
					<spring:message code="heran.viewEmp.title.dbclickOk"/><!-- 双击确定 -->
<%--						<img src="/resources/images/1.gif" onclick="F_HR_ShowMore(${item.PERSON_ID},'${param.navTabId}')" style="cursor:hand">--%>
	...
	<form id="pagerForm" method="post" action="/hrm/transferOrder/viewEmpIdList?navTabId=${param.navTabId}&seach_NAVID=${seach_NAVID}&empid=${EMP}&viewEmpIdListColnum=${EMP}">
	<div class="panelBar">
		<div class="pages">
