...
<div class="pageHeader">
	...
	<form method="post" action="/hrm/transferOrder/viewTransferOrderEmpList?navTabId=${param.navTabId}" onsubmit="return dwzSearch(this,'dialog')" rel="pagerForm">
...
<div class="pageContent">
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			<c:forEach items="${empList}" var="item">
				<tr target="EMPID" rel="${item.EMPID}" ondblclick="grabSelectedEmpBack('${item.EMPID}','${item.PERSON_ID }','${item.LOCAL_NAME }','${item.DEPTNO }','${item.DEPTNAME }','${item.POSITION_NO }','${item.POSITION_NAME }','${item.POST_NO }','${item.POST_NAME }','${FLAG }','${ROW }')">
					<td>${item.EMPID}</td>
					<td>${item.LOCAL_NAME}</td>
					<td>${item.IDCARD_NO}</td>
					<td>${item.DEPTNAME}</td>
					<td>${item.DEPT_DISTINGUISH_NO}</td>
					<td  ondblclick="grabSelectedEmpBack('${item.EMPID}','${item.PERSON_ID }','${item.LOCAL_NAME }','${item.DEPTNO }','${item.DEPTNAME }','${item.POSITION_NO }','${item.POSITION_NAME }','${item.POST_NO }','${item.POST_NAME }','${FLAG }','${ROW }')">
						双击确定<%--<img src="/resources/images/1.gif" style="cursor:hand" title="双击确定">--%>
					</td>
				</tr>
	...
	<form id="pagerForm" method="post" action="/hrm/transferOrder/viewTransferOrderEmpList?navTabId=${param.navTabId}">
	<div class="panelBar">
		<div class="pages">
