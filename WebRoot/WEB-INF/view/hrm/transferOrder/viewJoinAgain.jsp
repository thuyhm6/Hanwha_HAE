   ...
   <meta http-equiv="X-UA-Compatible" content="IE=edge" >
    ...
    function f_initGrid()
    {
    	$grid = $("#transfer").ligerGrid({
    		...
    		{ display: '离职日期', name: 'DATELEFT',width:90 },
    		{ display: '再入职日期', name: 'STARTDATE',width:110 },
    		{ display: '部门', name: 'DEPTFULLNAME',width:250 },
    		{ display: '职群', name: 'POSTGROUP',width:110 },
            ...
            sortName: 'EMPID',
            pageSize: 10,
            url: '/hrm/transferOrder/getTransferOrderList?joinAgain=1',
            width: '100%', 
            height: '400'
        });
    }
    ...
    function search(){
		$("#l-search").hide();
		$.ligerWindow.show( { url:'/hrm/transferOrder/searchEmp?type=joinAgain',
			name:'upgrade',width:800,height:500,left:90,top:10,showMax:false });
		     
	}

    function getDept(obj){
    	$.ligerDialog.open({ id:'getDept',url: '/hrm/transferOrder/searchDept', 
    ...
    function getSelected(obj,rowindex){				
		...
		if(obj.checked){
			...
			$(rowobj).children("TD[columnname='DEPTFULLNAME']").children("div").html("<div><ait:selectDept name='DEPTFULLNAME_"
					+rowindex+"'/>&nbsp;<img style='cursor:hand;' src='/resources/images/button/modify.gif' onclick=\"getDept('DEPTFULLNAME_"+rowindex+"')\"/></div>");
			$("#DEPTFULLNAME_"+rowindex+" option[value='"+rowdata.DEPTID+"']").attr("selected","true");
			$("#DEPTFULLNAME_"+rowindex).width("220");
			...
			$(rowobj).children("TD[columnname='STATUS']").children("div").html("<div><ait:selectSyCode parentCode='EmpStatus' name='STATUS_"
			...
			$(rowobj).children("TD[columnname='DEPTFULLNAME']").children("div").html(rowdata.DEPTFULLNAME);
			...
			$(rowobj).children("TD[columnname='STATUS']").children("div").html(rowdata.STATUS);
		}	 
    }
	...
	function save(){
		...
		if(flag == 0){
            ...
            var options = {	                        
            url:'/hrm/transferOrder/saveTransferOrder?type=joinAgain',
            type:'POST',	                        
            success: function(){
...
<body style="padding:5px" >
		...
		<input type="hidden" id="tempDept" value="">
</body>
</html>
