...
function f_checkAll_insuranceTwo(checkBoxFlag){
...
function f_checkAntiAll_insuranceTwo(checkBoxFlag){
	$("#InsuranceResultCenter :checkbox").each(function ()
	{	
		if(this.checked){
			this.checked = false;
...
function addUrlInsuranceParamTwo(name,value,flag,exFlag){
	var hrItem=document.getElementById("in32"+name);
	var isSort=document.getElementById("isSort32"+name);
	...
	<div class="formBar">
		<label style="float: left">
			<input type="checkbox" name="checkbox"
				onclick="f_checkAll_insuranceTwo(this.checked)" />
			<spring:message code="pa.salary.title.allChecked"/><!--全选-->
		</label>
		<label style="float: left">
			<input type="checkbox" name="checkbox"
				onclick="f_checkAntiAll_insuranceTwo(this.checked)" />
		...
		<ul>
			...
			<li>
				<a class="buttonActive" onclick="excelExportInsuranceResultTwo(this);">
							...
							<c:choose>
								<c:when test="${item.DISTINCT_FIELD eq 'IS_MONTH'}">
									<input type="checkBox" value="${item.FIELD_NAME}" name="ishrItem" 
											id="in32${item.DISTINCT_FIELD}"checked="checked" group=""/>
									<input type="text" id="isSort32${item.DISTINCT_FIELD}" name="ishrItem" style="width:15px;" 
								...
								<c:otherwise>
									<input type="checkBox" value="${item.FIELD_NAME}" name="ishrItem" 
											id="in32${item.DISTINCT_FIELD}" ${item.ISCHECKED } group=""/>
									<input type="text" id="isSort32${item.DISTINCT_FIELD}" name="ishrItem" style="width:15px;" 
											maxlength="2"
											<c:choose>
	...
	<div class="panel">
		...
		<table>
			<c:forEach items="${insuranceInputItemList}" var="item" varStatus="i">
				...
				<td>
					<input type="checkBox" name="isParamItem" value="${item.ALIAS_NAME}" id="in32${item.PARAM_ID}" ${item.ISCHECKED} group=""/>
					<input type="text" id="isSort32${item.PARAM_ID}" name="isParamItem" style="width:15px;" 
									maxlength="2"
									<c:choose>
										<c:when test="${item.ORDERNO ne null && item.ORDERNO ne ''}">
											class="number textInput required"
	...
	<div class="panel">
		...
		<table>
			<c:forEach items="${insuranceComputeItemList}" var="item"
				...
				<td>
					<input type="checkBox" name="isItem" value="${item.ALIAS_NAME}" id="in32${item.ITEM_ID}" ${item.ISCHECKED} group=""/>
					<input type="text" id="isSort32${item.ITEM_ID}" name="isItem" style="width:15px;" 
									maxlength="2"
									<c:choose>
										<c:when test="${item.ORDERNO ne null && item.ORDERNO ne ''}">
											class="number textInput required"
