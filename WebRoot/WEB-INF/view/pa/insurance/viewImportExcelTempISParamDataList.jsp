...
//导出
function exportISParamImportInfo(a) {
	var $from = $("#viewISParamExcelImportResult");
	alertMsg.confirm("确定导出么?", {
		okCall : function() {
			doISParamImportInfoExport($from);
		}
	});
}
function doISParamImportInfoExport(from) {
	var $from = $("#viewISParamExcelImportResult");
	//var url = "/pa/insurance/viewImportISParamListExcel";
	var url = "/pa/insurance/viewImportISParamDataListExcel";
	window.location = url + (url.indexOf('?') == -1 ? "?" : "&")
			+ $from.serialize();
}
//提交导入数据
function doISParamDataImport() {
	var params = $("#viewISParamExcelImportResult").serialize();
	alertMsg.confirm("确认导入项目数据吗?", {
		okCall : function() {
			$.ajax( {
				...
				url : "/pa/insurance/createImportISParamDataListExcel?"
						+ params,
				success : function(result) {
					...
					//页面重载
				navTabSearch($("#viewISParamExcelImportResult"));
			}
			});
		}
	});
...
<div class="pageHeader">
	<form id="viewISParamExcelImportResult" name="viewISParamExcelImportResult"
		action="/pa/insurance/viewImportExcelTempISParamDataList"
...
<div class="pageContent">
	...
	<div class="formBar">
		<ul>
			<li>
				<div class="buttonActive">
					<div class="buttonContent">
						<button type="button" onclick="exportISParamImportInfo(this)">
			...
			<li>
				<div class="buttonActive">
					<div class="buttonContent">
						<button type="submit" onclick="doISParamDataImport()">
	...
	<c:set value="/pa/insurance/viewImportExcelTempISParamDataList" var="pageUrl" />
