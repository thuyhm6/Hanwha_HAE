		...
		$.ajax({
			...
			success: function(data){ //请求成功后处理函数。
				if(data.statusCode=="200"){
					$("#seach_PARAM_NO").val(data.PARAM_NO);
					navTabSearch('viewInsuranceInputItemDataView');
...
function submitFormViewBonus_pa0412(){
	if($("#seach_PARAM_NO_pa0412",navTab.getCurrentPanel()).val()==""||$("#seach_CPNY_ID",navTab.getCurrentPanel()).val()==""){
		alertMsg.info('<spring:message code="liang.pa.insuranceApply.title.shenqingxiangmuNotNull"/>');//("申请项目不能为空");
		return false;
	}
	 var $from = $("#seachInsuranceInputItemDataViewList");
  	$from.submit();
	
}
function cpnyIdProject_pa0412(a){
				 
	var sel = $("#seach_PARAM_NO_pa0412",navTab.getCurrentPanel());
...
<div id="viewInsuranceInputItemDataView">
	<div class="pageHeader" style="border:1px #B8D0D6 solid">
		<form onsubmit="return navTabSearch(this);" 
		    id="seachInsuranceInputItemDataViewList" name="seachInsuranceInputItemDataViewList"
			action="/pa/insurance/viewInsuranceInputItemDataView?pageNum=1" 
			...
			<div class="searchBar">
				<table class="searchContent">
					<tr>
						...
						<td>
							...
							<c:if test="${CPNY_ID eq 'C02' }"><!-- 只有C02HQ 才能查看到其他公司的申请-->
							<ait:SyCompany target="login" name="seach_CPNY_ID" language="${LAN }" limit="ALL" activity="1" selected="${CPNY_ID}" onChangeName="cpnyIdProject_pa0412(this)" />
							</c:if>
							<c:if test="${CPNY_ID ne 'C02' }">
							<ait:SyCompany target="login" name="seach_CPNY_ID" language="${LAN }" limit="ALL" activity="1" selected="${CPNY_ID}"  disabled="true"/>
							</c:if>
						</td>
						<td><!-- 申请项目 --><spring:message code="liang.pa.insuranceApply.title.shenqingxiangmu"/>：</td>
						<td><select name="seach_PARAM_NO" id="seach_PARAM_NO_pa0412">
						...
						<td>
							<select id="seach_AFFIRM_FLAGA" name="seach_AFFIRM_FLAGA">
								<option value="-1" <c:if test="${AFFIRM_FLAGA eq '-1' }">selected</c:if>><spring:message code="ar.viewarcardrecord.title.quanbu"/><!-- 全部 --></option>
		           	     		<option value="0" <c:if test="${AFFIRM_FLAGA eq '0' }">selected</c:if>><spring:message code="liang.pa.insuranceApply.title.weicaijue"/><!--未决裁--></option>
		           	     		<option value="1" <c:if test="${AFFIRM_FLAGA eq '1' }">selected</c:if>><spring:message code="hr.viewTransactionTransViewList.title.PASS"/><!--已决裁--></option>
		           	     		<option value="2" <c:if test="${AFFIRM_FLAGA eq '2' }">selected</c:if>><spring:message code="hr.viewTransactionTransViewList.title.VOTE_DOWN"/><!-- 已决裁 --></option>
				...
				<div class="subBar">
					<ul>
						<li><div class="buttonActive"><div class="buttonContent"><button type="button" onclick="submitFormViewBonus_pa0412()">
...
<div class="pageContent" style="border-left:1px #B8D0D6 solid;border-right:1px #B8D0D6 solid">
				...
				<li>
					<div class="subBar">
			            <div class="buttonActive">
						    <a class="update" onclick="deleteInsDataApplyCallback('viewInsDataViewList',DWZ.ajaxDone,'1')" href="#" ><span>
					...
					<c:forEach items="${insuranceItemDataList}" var="itemData">
						<tr target="PARAM_DATA_NO" rel="${itemData.PARAM_DATA_NO}">
							...
							<!--<td style="text-align: center">点击查看
								<a  href="/pa/insurance/viewUnifySuitCompany?PARAM_NO=${PARAM_NO }&PARAM_DATA_NO=${itemData.PARAM_DATA_NO}&AFFIRM_FLAG=${itemData.AFFIRM_FLAG}" target="dialog" mask="true" width="1100" ><spring:message code="pa.ins.alert.message.title.clickForDetail"/></a>
							</td>
							--><td style="text-align: center">
								<c:if test="${itemData.AFFIRM_FLAG eq '0'}">
									<b><font color="blue" style="line-height:21px"><!--未决裁--><spring:message code="ess.trans.title.notAffirmed"/></font></b>
								</c:if>
								<c:if test="${itemData.AFFIRM_FLAG eq '1'}">
									<b><font color="green" style="line-height:21px"><!--通过--><spring:message code="ess.trans.title.pass"/></font></b>
								</c:if>
								<c:if test="${itemData.AFFIRM_FLAG eq '2'}">
									<b><font color="red" style="line-height:21px"><!--否决--><spring:message code="ess.trans.title.reject"/></font></b>
								</c:if>
							</td>
							<td style="text-align: center">
								<c:if test="${itemData.AFFIRM_FLAG eq '0'}">
	...
	<c:set value="/pa/insurance/viewInsuranceInputItemDataView?seach_PARAM_NO=${PARAM_NO}&seach_CPNY_ID=${CPNY_ID}" var="pageUrl" />
