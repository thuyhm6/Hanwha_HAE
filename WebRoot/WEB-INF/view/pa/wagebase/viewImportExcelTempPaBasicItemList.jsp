...
//导出
function exportPaBasicItemImportInfo(a) {
	var $from = $("#viewPaBasicItemExcelImportResult");
	alertMsg.confirm("确定导出么?", {
		okCall : function() {
			doPaBasicItemImportInfoExport($from);
		}
	});
}
function doPaBasicItemImportInfoExport(from) {
	var $from = $("#viewPaBasicItemExcelImportResult");
	var url = "/pa/wagebase/viewImportPaBasicItemListExcel";
	window.location = url + (url.indexOf('?') == -1 ? "?" : "&")
			+ $from.serialize();
}
//提交导入数据
function doPaAccountDataImport() {
	var params = $("#viewPaBasicItemExcelImportResult").serialize();
	alertMsg.confirm("确认导入项目数据吗?", {
		okCall : function() {
			$.ajax( {
				...
				url : "/pa/wagebase/createImportPaBasicItemListExcel?"
						+ params,
				success : function(result) {
					if (result == 1) {
						alertMsg.info("基础项目数据导入成功！");
						navTab.closeCurrentTab();
					} else {
						alertMsg.info("基础项目数据导入失败！");
					}
					//页面重载
				navTabSearch($("#viewPaBasicItemExcelImportResult"));
			}
			});
		}
	});
...
<div class="pageHeader">
	<form id="viewPaBasicItemExcelImportResult" name="viewPaBasicItemExcelImportResult"
		action="/pa/wagebase/viewImportExcelTempPaBasicItemList"
...
<div class="pageContent">
	...
	<div class="formBar">
		<ul>
			<li>
				<div class="buttonActive">
					<div class="buttonContent">
						<button type="button" onclick="exportPaBasicItemImportInfo(this)">
	...
	<c:set value="/pa/wagebase/viewImportExcelTempPaBasicItemList" var="pageUrl" />
