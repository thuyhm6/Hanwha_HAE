...
function updateFeiCuLowSalaryValidateCallback(form, callback,flag) {
    ...
    $form.attr("action","/pa/salary/updateFeiCuLowSalaryBatch");
	...
	$.ajax({
		...
		success: function(data){ //请求成功后处理函数。
			if(data.statusCode=="200"){
				navTabSearch("searchFeiCuLowSalaryForm");
...
function f_cancelFeiCulowadjust(callback, pa_month,person_id,adjust_bef){
	
	if (confirm ('<spring:message code="pa.low.adjust.chexiaotiaozheng"/>')){	
		$.ajax({
			type: 'POST',
			url: '/pa/salary/cancelFeiCulowadjust',
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);"
		action="/pa/salary/viewFeiCuLowSalaryList" method="post"
		id="searchFeiCuLowSalaryForm" name="searchFeiCuLowSalaryForm" rel="pagerForm">
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					...
					<td>
						<ait:deptList name="seach_DEPT_NO" id="viewFeiCuLowSalaryList_seachDept" limit="pa"/>
						<ait:deptTreeIcon name="seach_DEPT_NO" limit="pa" id="viewFeiCuLowSalaryList_seachDept" selected="${DEPT_NO}"/>
						<%--
				...
				<ul>
					<li>
						<div class="subBar">
							...
							<div class="buttonActive">
								<a class="update"
									onclick="return updateFeiCuLowSalaryValidateCallback('updateFeiCuLowSalaryForm',DWZ.ajaxDone,'');"
									href="#"><span><spring:message
											code="public.title.submit" />
										<!-- 提交 --> </span> </a>
							</div>
						</div>
					</li>
...
<div class="pageContent">
	<form name="updateFeiCuLowSalaryForm" id="updateFeiCuLowSalaryForm" method="post"
		action="/pa/salary/updateFeiCuLowSalary"
		onsubmit="return updateFeiCuLowSalaryValidateCallback(this, navTabAjaxDone);">
			...
			<thead>
				<tr>
					<th width="40"><spring:message
							code="hr.viewUpgrade.title.CHECKALL" /> <!--全选-->
					</th>
					...
					<th width="80"><!--是否撤销:--> 
						<spring:message code="pa.low.adjust.shifouchexiao" />
					</th>
					<th width="80"><!-- 备注 -->
					...
					<tr target="PERSON_ID" rel="${paLowAdjust.PERSON_ID}">
						...
						<!-- 未提交才显示checkbox -->
							<c:if
								...
								<input type="checkbox" id="c1" name="c1" value="${paLowAdjust.PERSON_ID}" />
								<input name="${paLowAdjust.PERSON_ID}_PA_MONTH" type="hidden" 
									id="${paLowAdjust.PERSON_ID}_PA_MONTH" value="${paLowAdjust.PA_MONTH}"/>
								<!-- 调整前最低工资标准，即系统原有值 -->
								<input name="${paLowAdjust.PERSON_ID}_ZUIDIGONGZIBIAOZHUN" type="hidden" 
									id="${paLowAdjust.PERSON_ID}_ZUIDIGONGZIBIAOZHUN" value="${paLowAdjust.ZUIDIGONGZIBIAOZHUN}"/>
								<input name="${paLowAdjust.PERSON_ID}_QUFEN" type="hidden" 
									id="${paLowAdjust.PERSON_ID}_QUFEN" value="${paLowAdjust.QUFEN}"/>
							</c:if>
						</td>
						...
						<td>
							<c:if test="${paLowAdjust.QUFEN eq 'SQ'}">
								<font color="red">${paLowAdjust.BENYUEYINGFAGONGZI}</font>
							</c:if>
							<c:if test="${paLowAdjust.QUFEN ne 'SQ'}">
								${paLowAdjust.BENYUEYINGFAGONGZI}
							</c:if>
						</td>
						<td>
							<c:if test="${paLowAdjust.QUFEN eq 'SH'}">
								<font color="red">${paLowAdjust.BENYUESHIFAGONGZI}</font>
							</c:if>
							<c:if test="${paLowAdjust.QUFEN ne 'SH'}">
								${paLowAdjust.BENYUESHIFAGONGZI}
							</c:if>
						</td>
						...
						<td>
							...
							<c:if
								test="${paLowAdjust.ACTIVITY != '1' }">
								<input name="${paLowAdjust.PERSON_ID}_SHIJITIAOZHENGJINE" type="text" 
									id="${paLowAdjust.PERSON_ID}_SHIJITIAOZHENGJINE" value="${paLowAdjust.XITONGJISUANTIAOZHENGJINE}"/>
							</c:if>
						</td>
						<td>
							<c:if
						...
						<!-- 已提交才显示 -->
							<c:if
								test="${paLowAdjust.ACTIVITY == '1' }">
								<%-- 
								<a href="/hrm/searchTransferOrder/viewEntryTrans?EXP_INSIDE_NO=${hrExpInside.EXP_INSIDE_NO}&&PERSON_ID=${hrExpInside.PERSON_ID}"
									target="navTab"> <span><spring:message
												code="pa.low.adjust.chexiao" />
											<!-- 撤销 --> </span> </a>
								 --%>
								 <!-- 参数:pa_month,person_id,调整前工资——即最低工资标准 -->
								<a href="#" onclick="javascript:f_cancelFeiCulowadjust(navTabAjaxDone,'${paLowAdjust.PA_MONTH}','${paLowAdjust.PERSON_ID}','${paLowAdjust.ADJUST_BEF}');">
									<spring:message code="pa.low.adjust.chexiao" />
									<!-- 撤销 -->
								</a>
							</c:if>
						</td>
						<td>
							...
							<c:if
								test="${paLowAdjust.ACTIVITY != '1' }">
								<textarea style="overflow-x:hidden;overflow-y:hidden" id="${paLowAdjust.PERSON_ID}_CONTENT" name="${paLowAdjust.PERSON_ID}_CONTENT">
									${paLowAdjust.CONTENT}
								</textarea>
							</c:if>
						</td>
	...
	<c:set value="/pa/salary/viewFeiCuLowSalaryList"
		var="pageUrl" />
