...
//导出
function exportPaParamMonthImportInfo(a) {
	var $from = $("#viewPaParamMonthExcelImportResult");
...
function doPaParamImportMonthInfoExport(from) {
	var $from = $("#viewPaParamMonthExcelImportResult");
	var url = "/pa/salary/viewImportPaParamMonthListExcel";
	window.location = url + (url.indexOf('?') == -1 ? "?" : "&")
			+ $from.serialize();
}
//提交导入数据
function doPaParamMonthDataImport() {
	var params = $("#viewPaParamMonthExcelImportResult").serialize();
	alertMsg.confirm("<spring:message code='alert.message.confirm_import_item_data.b' />", {//确认导入项目数据吗?
		okCall : function() {
			$.ajax( {
				type : 'post',
				cache : false,
				url : "/pa/salary/createImportPaParamMonthListExcel?"
						+ params,
				success : function(result) {
					...
					//页面重载
				navTabSearch($("#viewPaParamMonthExcelImportResult"));
			}
			});
		}
	});
...
<div class="pageHeader">
	<form id="viewPaParamMonthExcelImportResult" name="viewPaParamMonthExcelImportResult"
		action="/pa/salary/viewImportExcelTempPaParamMonthList"
...
<div class="pageContent">
	...
	<div class="formBar">
		<ul>
			<li>
				<div class="buttonActive">
					<div class="buttonContent">
						<button type="button" onclick="exportPaParamMonthImportInfo(this)">
			...
			<li>
				<div class="buttonActive">
					<div class="buttonContent">
						<button type="submit" onclick="doPaParamMonthDataImport()">
	...
	<c:set value="/pa/salary/viewImportExcelTempPaParamMonthList" var="pageUrl" />
