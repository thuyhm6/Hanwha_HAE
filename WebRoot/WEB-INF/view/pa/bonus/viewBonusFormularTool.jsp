...
<script type="text/javascript">
	function filltext(values){		
		if (parent){
			if (parent.bonusFormularForm){
				if($(":radio:checked").val() == 'condition'){
					var target = parent.bonusFormularForm.CONDITION;
				}else{
					var target = parent.bonusFormularForm.FORMULAR;
				}
				target.value+=values+' ';
			}
		}
	...
	<div class="pageFormContent" layoutH="60">
		<table width="100%" height="300" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
			<tr align="center">
				...
				<td><spring:message code="pa.bonus.title.caculateParmeterlist"/><!--计算项目参数列表--></td>
			...
			<tr align="center" >
				<td align="center"  style="padding:5 5 5 5 " valign="top">
					<select name="select" id="param_no" size="17" style="width:170px " onClick="filltext(' '+this.value)">
						<c:forEach items="${bonusComputeItemList}" var="ci">
							<option value="${ci.ITEM_ID}">${ci.ITEM_NAME}</option>
						</c:forEach>
					</select>
				</td>
				<td  align="center"  style="padding:5 5 5 5 " valign="top">
					<select name="select" id="param_no" size="17" style="width:170px " onClick="filltext(''+this.value)">
						<c:forEach items="${bonusInputItemList}" var="cip">
							<option value="${cip.PARAM_ITEM_ID}">${cip.PARAM_NAME}</option>
						</c:forEach>
					</select>
				</td>
				<td  align="center"  style="padding:5 5 5 5 " valign="top">
				...
				<td style="padding:5 5 5 5 ">
					<table width="100" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td><input type="radio" name="radiobutton" value="condition" checked><spring:message code="pa.insurance.title.condition"/><!--条件--></td>
