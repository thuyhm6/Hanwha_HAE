...
function f_checkAll_bonusTwo(checkBoxFlag){
...
function f_checkAntiAll_bonusTwo(checkBoxFlag){
	$("#BonusResultCenter :checkbox").each(function ()
	{	
		if(this.checked){
			this.checked = false;
...
function addUrlBonusParamTwo(name,value,flag,exFlag){
	var hrItem=document.getElementById("in22"+name);
	var bnItem=document.getElementById("bnSort22"+name);
	...
	<div class="formBar">
		<label style="float: left">
			<input type="checkbox" name="checkbox"
				onclick="f_checkAll_bonusTwo(this.checked)" />
			<spring:message code="pa.salary.title.allChecked"/><!--全选-->
		</label>
		<label style="float: left">
			<input type="checkbox" name="checkbox"
				onclick="f_checkAntiAll_bonusTwo(this.checked)" />
		...
		<ul>
			...
			<li>
				<a class="buttonActive" onclick="excelExportBonusResultTwo(this);"><span>
					...
					<c:choose>
						<c:when test="${item.DISTINCT_FIELD eq 'BN_MONTH'}">
							<input type="checkBox" value="${item.FIELD_NAME}"
								name="ishrItem" id="in22${item.DISTINCT_FIELD}" 
								checked="checked" group=""
								/>
							<input type="text" id="bnSort22${item.DISTINCT_FIELD}" name="ishrItem" style="width:15px;" 
						...
						<c:otherwise>
							<input type="checkBox" value="${item.FIELD_NAME}"
								name="ishrItem" id="in22${item.DISTINCT_FIELD}" ${item.ISCHECKED } group=""/>
							<input type="text" id="bnSort22${item.DISTINCT_FIELD}" name="ishrItem" style="width:15px;" 
											maxlength="2"
											<c:choose>
			...
			<c:forEach items="${bonusInputItemList}" var="item" varStatus="i">
				...
				<td>
					<input type="checkBox" name="bnParamItem"
						value="${item.PARAM_NAME}" id="in22${item.PARAM_ITEM_ID}" 
						 ${item.ISCHECKED } group=""
						/>
					<input type="text" id="bnSort22${item.PARAM_ITEM_ID}" name="bnParamItem" style="width:15px;" 
										maxlength="2"
										<c:choose>
											<c:when test="${item.ORDERNO ne null && item.ORDERNO ne ''}">
												class="number textInput required"
			...
			<c:forEach items="${bonusComputeItemList}" var="item"
				...
				<td>
					<input type="checkBox" name="bnItem"
						...
						id="in22${item.ITEM_ID}" 
						 ${item.ISCHECKED } group=""
						/>
					<input type="text" id="bnSort22${item.ITEM_ID}" name="bnItem" style="width:15px;" 
									maxlength="2"
									<c:choose>
										<c:when test="${item.ORDERNO ne null && item.ORDERNO ne ''}">
											class="number textInput required"
