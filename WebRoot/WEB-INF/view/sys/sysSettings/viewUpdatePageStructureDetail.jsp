	...
	*/
	 function f_save(){
       ...
       var manager = $("#viewInfo").ligerGetGridManager();
               ...
               $.each($("input[name='RI_NO']"),function(index,obj){
                    if(obj.checked){ 
                    ...
                    var col=manager.getRowByRowIndex(index);
                      ...
                      if (jsonData.length > 1){
	                   ...
	                   jsonData += ' "RI_NO": "' + obj.value + '" ,' ;
	                   jsonData += ' "ITEM_NAME": "' + $.StringUtil.checkNull(col.ITEM_NAME) + '" ,' ;
	                   jsonData += ' "ITEM_KOR_NAME": "' + $.StringUtil.checkNull(col.ITEM_KOR_NAME)  + '",' ;
	                   jsonData += ' "ITEM_EN_NAME": "' + $.StringUtil.checkNull(col.ITEM_EN_NAME)  + '",' ;
	                   jsonData += ' "ORDER_NO": "' + $.StringUtil.checkNull(col.ORDER_NO)  + '"' ;
                       jsonData += '}' ;
        ...
        $.post('/sys/sysSettings/updatePageStructureDetailInfo', 
                [	
    ...
    function f_initGrid()
    {
       ...
       var REPORT_TYPE=${REPORT_TYPE};
    	...
    	$grid = $("#viewInfo").ligerGrid({
    		...
    		columns: [
    		{ display: '选择', name: 'RI_NO' ,minWidth: 40, render: function (item)
	                    {
	                        return "<input type='checkbox' name='RI_NO' id='RI_NO' value='"+item.RI_NO+"' />"
	                    } },
    		{ display: '名称', name: 'ITEM_NAME',minWidth: 200 ,editor:{type : 'string'}},
	        ...
	        { display: '显示排序',  name:'ORDER_NO', minWidth:80 ,editor:{type : 'int'}} 
            ...
            dataAction: 'server', 
            sortName: 'ORDER_NO',
            url: '/sys/sysSettings/getUpdatePageStructureDetail',
            parms: [	
	          	{ name: 'RT_NO', value: RT_NO } 
	          ], 
            width: '99%', height: '410'
            ...
            if(yes){
               ...
               $.each($("input[name='RI_NO']"),function(index,obj){
                ...
                if(jsonData.length == 2){
                  ...
                  $.post('/sys/sysSettings/deletePageStructureDetail', 
		                [	
		                 	{ name: 'jsonData', value: jsonData },
		                ]
	       ...
	       <table cellpadding="0" cellspacing="0" class="l-table-edit" >
	           <tr>
	               ...
	               <spring:message code="button.sys.affirm.save"/><!--保存--></a>
	                <a id="deleteButton" class="l-button" style="width:60px;float:left; margin-left:10px;" onclick="f_delete()">
	                <spring:message code="button.delete"/><!--删除--></a>
	                </td>
	            </tr>
	       </table>
