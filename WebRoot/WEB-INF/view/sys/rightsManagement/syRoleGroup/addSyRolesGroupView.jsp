...
var cpnyInitId=document.getElementById("CPNY_ID").value;
$.ajax({  
    async : false,  
    cache:false,  
    type: 'POST',  
    dataType : "json",  
    url: "/sys/rightsManagement/getRolesIfChecked?cpnyInitId="+cpnyInitId+"&SYS_TYPE="+document.getElementById("SYS_TYPE").value,//请求的action路径  
    ...
    success:function(data){ //请求成功后处理函数。    
    	...
    	for(var i=0;i<data.length;i++){
			...
			str+='<input type="checkbox" name="ROLE_NOS"  value="'+data[i].ROLE_NO+'"'+
			'/>'+data[i].ROLENAME+"&nbsp;&nbsp;";
        }
...
function addCpnyIdRoles(obj){
	$.ajax({  
	    ...
	    type: 'POST',  
	    dataType : "json",  
	    url: "/sys/rightsManagement/getRolesIfChecked?cpnyInitId="+obj.value+"&SYS_TYPE="+document.getElementById("SYS_TYPE").value,//请求的action路径  
	    error: function () {//请求失败处理函数  
	        alertMsg.error('<spring:message code="alert.message.sys.arAffirm.qequestFailed"/>');   
	    },  
	    success:function(data){ //请求成功后处理函数。    
...
<form method="post" action="/sys/rightsManagement/syRoleGroup/saveOrUpdateSyRolesGroupInfo" class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone);">
			...
			<dl>
				...
				<dd>
					...
					<c:forEach items="${cpnyList}" var="cpny">
						<option value="${cpny.CPNY_ID}">${cpny.CONTENT}</option>
					</c:forEach>
				</select>
				</dd>
			</dl>			 
