    ...
    alert(seach_APPOINT_NAME);
    	if (seach_AFFIRMOR_NAME==''||seach_APPOINT_NAME==''){
    	 ...
    	 $.ajax( {
				...
				url : '/sys/affirm/updateaffirmSpecialAppoints?seach_AFFIRMOR_NAME=' + seach_AFFIRMOR_NAME + '&seach_APPOINT_NAME='+seach_APPOINT_NAME,
			    dataType : "json",
				success : function() {
                alert("委任成功");
				}
...
function search(){
	...
	alert(seach_AFFIRMOR_NAME_BYNAME);
	 $.ajax( {
			...
			cache : false,
			contentType : 'application/json',
			url : '/sys/affirm/affirmSpecialAppoint?seach_AFFIRMOR_NAME_BYNAME=' + seach_AFFIRMOR_NAME_BYNAME,
		    dataType : "json",
			success : function() {
            alert("委任成功");
			}
...
function searchempid(){
	...
	navTabSearch(document.affirmSpecialAppoint);
}
...
<div class="pageHeader">
	<form id='affirmSpecialAppoint' name='affirmSpecialAppoint' onsubmit="return navTabSearch(this);" action="/sys/affirmSpecial/affirmSpecialAppoint" method="post" rel="pagerForm">
		<div class="searchBar">
...
<div class="pageContent">
	...
	<table class="table" style="overflow: auto" width="100%"  layoutH="206" nowrapTD="false">
		...
		<tbody>
			<c:forEach items="${affirmSpecialList}" var="affirmSpecialList" varStatus="i">
			    <tr>
