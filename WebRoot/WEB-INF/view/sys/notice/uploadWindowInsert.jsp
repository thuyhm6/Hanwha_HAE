...
<script>
function validateCallbackuploadWindow(form) {	

	var fileTable = $("#fileTable tbody");
	...
	for (i=0;i<fileUrlStrs.length ;i++ ) {
		fileTable.append("<tr><td class='td_center'><input type='checkbox' name='FILE_NO'/><input type='hidden' name='fileUrl' value='" + fileUrlStrs[i] + "'/><input type='hidden' name='fileName' value='" + fileNameStrs[i] + "'/></td><td><a href=\"/ess/infoApplyLeave/downloadFile?fileName=/resources/temp/apply/applyleave/${LoginUser.personId}/" + fileUrlStrs[i] + "&file=" + fileNameStrs[i] + "\" >" + fileNameStrs[i] + "</a></td></tr>");
	}

	$("#fileUrl_upload_insert").val(fileUrl);
	$("#fileName_upload_insert").val(fileName);
...
<div class="pageContent" layoutH="10">
	<form id="uploadWindowForm" method="post" action="/sys/notice/uploadAtt" class="pageForm required-validate" onsubmit="return validateCallbackuploadWindow(this)">
