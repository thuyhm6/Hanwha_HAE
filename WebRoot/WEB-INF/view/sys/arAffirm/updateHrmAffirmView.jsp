...
var affirmLevel=document.getElementById('operateTable').rows.length-1;
...
<div >
	<form method="post" action="/sys/hrmAffirm/updateHrmAffirmInfo" class="pageForm required-validate" 
		...
		<spring:message code="sys.affirm.title.affirmDuty"/><!--申请者职责--></span></div>
				 ...
				 <tr>
				    <td class="info_content_01">
				     <select name="DUTY_NO">
					         ...
					         <option  value="${result.DUTY_NO}" <c:if test="${result.DUTY_NO eq leaveApply.DUTY_NO}">selected</c:if>>${result.DUTYNAME}</option>
					         </c:forEach>
					         </select>
				    </td>
				 </tr>    
			...
			<input type="hidden" name="TRANS_PARAM_NO" value="${leaveApply.TRANS_PARAM_NO}"/>
				 ...
				 <tr>
				    ...
				    <td class="info_content_01">
				     <select name="APPLY_TYPE">
					         ...
					         <option  value="${result.CODE_NO}" <c:if test="${leaveApply.APPLY_TYPE eq result.CODE_NO}">selected="selected"</c:if>>${result.CODENAME}</option>
					         </c:forEach>
					         </select>
				    </td>
				   ...
				   <tr>
						 ...
						 <td align="left" class="info_content_01" width="25%" >
						  ...
						  <td align="left" class="info_title_01" colspan="1" >
							 <span onclick="appendRow('add');"><spring:message code="button.add"/>
							 <!--添加--></span> &nbsp;|&nbsp;
				    ...
				    <c:forEach items="${paramList}" var="post" varStatus="var">
					     <tr>
						  	<td class="info_title_01" width="25%" style="text-align: center"><input type='radio' name='rowNum' /></td>
							<td colspan="1" style="text-align: center">${post.DUTY_NAME}<input type='hidden' name='DUTY_NO' value='${post.AFFIRMOR_NO}'/></td>
							<td width="25%" style="text-align: center">${var.count}</td> 
					     </tr>
				    </c:forEach>
			</table>
		</div>
