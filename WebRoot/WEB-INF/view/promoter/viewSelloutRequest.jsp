...
<script>
//添加决裁者
function addRowByIDCx1601(currentRowID){
	var count = parseInt($("#affirmCount").val());
    var htm  ='<tr id="rowIdCx1601'+ count +'"><td class="td_type" style="text-align: center" width="25%"><span name="rowIndex"></span></td>';
		htm +='<td class="td_type" style="text-align: center" width="37.5%">';
		htm +='<input id="AFFIRMOR_IDCx1601' + count + '" name="AFFIRMOR_ID" value="" type="hidden"/>';
		htm +='<input id="EMPINFOCx1601' + count + '" name="empid" type="text" class="required" lookupGroup="person" onkeydown="submitKeyClick_affirmor(this,' + count + ',event)" class="required"/>';
		htm +='</td>';
		htm +='<td class="td_type" style="text-align: center" width="37.5%">';
		htm +='<img src="/resources/images/+.gif" title="添加"';	
		htm +='border="0" align="absmiddle" style="cursor:hand" onclick="addRowByIDCx1601(' + count + ')"/>&nbsp;&nbsp;&nbsp;';
		htm +='<img src="/resources/images/-.gif" title="删除"';	
		htm +='border="0" align="absmiddle" style="cursor:hand" onclick="javaScript:document.all.addAffirm_listCx1601.deleteRow(event.srcElement.parentElement.parentElement.rowIndex);changeAffirmLevel();"/></td></tr>';

   	//当前行之后插入一行
   	$("#rowIdCx1601" + currentRowID).after(htm);
   	changeAffirmLevel();
  	$("#affirmCount").val(++count) ;
}
function addRowByID1601First(){
	var count = parseInt($("#affirmCount").val());
    var htm  ='<tr id="rowIdCx1601'+ count +'"><td class="td_type" style="text-align: center" width="25%"><span name="rowIndex"></span></td>';
		htm +='<td class="td_type" style="text-align: center" width="37.5%">';
		htm +='<input id="AFFIRMOR_IDCx1601' + count + '" name="AFFIRMOR_ID" value="" type="hidden"/>';
	...
	var tb2 = document.getElementById("addAffirm_listCx1601");
   	if(tb2.rows.length == 0){
   		$("#addAffirm_listCx1601:last tbody").html(htm);
   	} else {
   	   	//当前行之后插入一行
   	   	$("#" + tb2.rows[0].id).before(htm);
   	}
  	$("[id='EMPINFOCx1601" + count + "']").attr("alt","请输入关键字按回车检索").attr("size","30").inputAlert();
  	changeAffirmLevel();
  	$("#affirmCount").val(++count) ;
}
//修改决裁者等级
		...
		var empNameStr="empName"+empIdStr.substring(7);
   		$.ajax({
			...
			success: function(jsonObject){
						...
						if(jsonObject.perCnt==1){
							$("#EMPINFOCx1601" + index).val('['+jsonObject.empId + ']-'+jsonObject.empName);
							$("#AFFIRMOR_IDCx1601" + index).val( jsonObject.personId);
						}
					},
			error: DWZ.ajaxError
		});
...
document.onkeydown = function(event) {  
	...
	function saveAddSelloutReq(flag){
		if(document.getElementById("viewSelloutReqAdd").INVOICE_QTY.value.trim() == null || document.getElementById("viewSelloutReqAdd").INVOICE_QTY.value.trim() == 0)
		{
           alert("请录入发票销量!");
           return;
		}

		if(document.getElementById("viewSelloutReqAdd").SCREEN_QTY.value.trim() == null || document.getElementById("viewSelloutReqAdd").SCREEN_QTY.value.trim() == 0)
		{
           alert("请录入截屏及凭证销量!");
           return;
		} 
		$("#viewAddSelloutReqFLAG").val(flag);
		$("#viewSelloutReqAdd").submit();
	}
	...
	var ajaxGet_add_Cx1601_add;
	function ajaxAdd_add_Cx1601_add() {
		alert($(":input[sysLong='viewAddPaTempSales_seachDept']").val());
		if (ajaxGet_add_Cx1601_add != null) {
			ajaxGet_add_Cx1601_add.abort();
		}
		$.ajaxSettings.global = false;
		ajaxGet_add_Cx1601_add = $.ajax( {
				type : "POST",
				url : "/pa/tempsale/getAffirmorInfo",
				data : {DEPT_NO : $(":input[sysLong='viewAddPaTempSales_seachDept']").val()},
				dataType : "json",
				success : function(data) {
					$('#addAffirm_listCx1601 tbody').html("");
					var html = "";
					if (typeof (data['affirmList']) != "undefined") {
						$.each(data['affirmList'],
										function(commentIndex, comment) {
											html += '<tr id="rowIdCx1601' + commentIndex  + '">';
					...
					$('#addAffirm_listCx1601 tbody').html(html);
				}
		});
		$.ajaxSettings.global = true;
	}
	...
	function expReqInfo(expType){
		var sform = document.getElementById("viewSelloutReqAdd");
		var eForm = document.getElementById("excelForm_cx1601");
	...
	function uploadSuccess_Sell(file, data, response){
...
<div class="pageContent">
	<form id="excelForm_cx1601" name="excelForm_cx1601" action="/promoter/expSelloutRequestExcel" method="post">
...
<c:forEach items="${menuThirdList}" var="menu" varStatus="i">
	<c:if test="${menu.MENU_CODE eq 'cx1601'}">
	  <div style="display: block;" id="displaySellPanel_1">
		<form id="viewSelloutReqAdd" method="post" action="/promoter/addSelloutReq" class="pageForm required-validate" onsubmit="return validateCallbackViewSelloutReq(this, dialogAjaxDone)">
		<div class="pageFormContent">
			...
			<input type="hidden" name="PAY_AREA_CD" id="PAY_AREA_CD" value="${PAY_AREA_CD}">
		    ...
		    <table class="table" width="100%" layoutH="456">
		    <thead>
		      <tr>
		        <th width="10%">大区 </th>
		        <th width="10%">支社</th>
			...
			<tr>
				...
				<td class="td_title" style="text-align: center" width="30%">是否新增(<img src="/resources/images/+.gif" title="添加" border="0" align="absmiddle" style="cursor:hand" onclick="addRowByID1601First()"/>)</td>
			</tr>
			<tr>
				<td colspan="3">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" id="addAffirm_listCx1601">
						<tbody>
							<c:forEach items="${affirmorList}" var="affirmor" varStatus="j">
								<tr id="rowIdCx1601${j.index}">
									...
									<td class="td_type" style="text-align: center" width="37.5%">
									 <!-- c:if test="${fn:length(affirmorList) gt j.count}"> -->
										<img src="/resources/images/+.gif" title="添加" border="0"
											align="absmiddle" style="cursor: hand" onclick="addRowByIDCx1601(${j.index})" />
									 <!-- /c:if> -->
									</td>
								</tr>
							</c:forEach>
				...
				<td width="80%" class="td_type">
				    <input id="reqSell_file" type="file" name="file" 
							uploaderOption="{
								...
								width:50,
								auto:false,
								onUploadSuccess:uploadSuccess_Sell,
								removeTimeout:1
							}"
						/><span id="Sell_fileNmae"></span>
					  <div id="fileQueue_Sell" class="fileQueue" style="height:30px"></div>
					  <input type="hidden" id="Sell_fileUrl" name="Sell_fileUrl" value=""/>
					  <input type="hidden" id="Sell_fileName" name="Sell_fileName" value=""/>
		...
		<div class="pageFormContent">
		    ...
		    <c:forEach items="${reqOver10List}" var="item2" varStatus="i">
			  <tr>
				...
				<td class='td_center'>${item2.PAY_RATE}
	...
	<c:if test="${menu.MENU_CODE eq 'cx1606'}">
	  <div style="display: block;" id="displaySellPanel_6">
		<div class="pageFormContent">
		  <form>
		    <table class="table" width="300%" layoutH="150">
		    <thead>
		      <tr>
		        <th>COME_CODE</th>
		        <th>社号</th>
...
<div class="tabsFooterContent"></div>
		...
		<div class="formBar">
			<ul>
				<li><div class="buttonActive"><div class="buttonContent"><button type="button" onclick="saveAddSelloutReq(1);"><spring:message code="public.title.submit"/><!-- 提交 --></button></div></div></li>
				<li><div class="buttonActive"><div class="buttonContent"><button type="button" onclick="saveAddSelloutReq(0);">暂存</button></div></div></li>
				<li><div class="buttonActive"><div class="buttonContent"><button type="button" onclick="expReqInfo(0);">下载</button></div></div></li>
				<li>
