...
function submitAffirmPreEss0252(flag){
	...
	if(pro_flag == 0){
		$("#pro_flag").val("1");
		$.ajax({
			type:$("#addHireAffirmInfo").method || 'POST',
			url:$("#addHireAffirmInfo").attr("action"),
			data:$("#addHireAffirmInfo").serializeArray(),
			dataType:"json",
			cache: false,
...
function addCheck(essAffirmNo,AFFIRMOR_ID){
	var returnValue =  window.showModalDialog("/LGEP/affirm/addCheckInfo?APPLY_TYPE=21&APPLY_NO=${infoApplyLeave.APPLY_NO}&ESS_AFFIRM_NO=" + essAffirmNo + "&AFFIRMOR_ID=" + AFFIRMOR_ID,window,"dialogWidth=500px;dialogHeight=300px;help=no;center=yes;resizable=no;status=no;scroll=no");
	if(returnValue == "true"){
...
<body>
	<form id="addHireAffirmInfo" method="post" action="/LGEP/affirm/affirmReqHire" class="pageForm required-validate" onsubmit="return validateContractAffirmCallback(this,navTabAjaxDone);">
			...
			<div class="pageFormContent">
				<table class="user_table" layoutH="60" width="100%">
					<tr>
						...
						<input type="hidden" name="APPLY_NO" value="${essAffirmReq.REQ_ID}"/>
						<input type="hidden" name="ESS_AFFIRM_NO" value="${essAffirmNo}"/>
						<input type="hidden" name="AFFIRMOR_ID" value="${PERSON_ID}"/>
						<input id="AFFIRM_FLAG_HIRE_APPLY" name="AFFIRM_FLAG" type="hidden" value="" />
...
<div class="pageContent">
	...
	<c:set value="/LGEP/affirm/viewReqHireAffirm" var="pageUrl"/>
					...
					<li>
						<div class="button">
							<div class="buttonContent"><!-- 通过 -->
								<button type="button" onclick="submitAffirmPreEss0252(1)">
									通过
								</button>
							</div>
						</div>
					...
					<li>
						<div class="button">
							<div class="buttonContent"><!-- 否决 -->
								<button type="button" onclick="submitAffirmPreEss0252(2)">
									否决
								</button>
							</div>
						</div>
