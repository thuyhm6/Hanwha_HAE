...
function subCheck_Dimission(){
    var $form = $("#checkApplyInfo_dimission");
...
<div class="pageContent">
	<form id="checkApplyInfo_dimission" method="post"
		action="/LGEP/affirm/checkLeave"
		onsubmit="return subCheck_Dimission()">
		<table width="100%" border="0" layoutH="200" cellpadding="0"
			cellspacing="0" class="user_table margin_b">
			<tr>
				<c:if test="${affirmorListCnt > 0}">
					<td class="td_title" width="10%" style="text-align: center"
						rowspan="${affirmorListCnt+1 }">
						<!-- 决裁线 --> 决裁线</td>
				</c:if>
				<c:if test="${affirmorListCnt == 0}">
					<td class="td_title" width="10%" style="text-align: center"
				...
				<td class="td_title" width="7%" style="text-align: center">
					<!-- 决裁等级 --> 决裁等级</td>
				<td class="td_title" width="20%" style="text-align: center">
					<!-- 决裁者 --> 决裁者</td>
				<td class="td_title" width="18%" style="text-align: center">
					<!-- 决裁情况 --> 决裁情况</td>
				<td class="td_title" width="15%" style="text-align: center">
					<!-- 审批时间 --> 审批时间</td>
			...
			<c:forEach items="${affirmList}" var="affirmor" varStatus="i">
				<tr>
					...
					<td class="td_type" style="text-align: center"><c:if
							test="${affirmor.AFFIRM_FLAG eq '0'}">
							<!--<font color="blue">未决裁</font>-->
							未决裁
						</c:if> <c:if test="${affirmor.AFFIRM_FLAG eq '1'}">
							<!--<font color="green">已通过</font>-->
							已通过
						</c:if> <c:if test="${affirmor.AFFIRM_FLAG eq '2'}">
			...
			<tr>
				...
				<td class="td_title" width="7%" style="text-align: center">
					<!-- Type --> Type</td>
				<td class="td_title" width="38%" style="text-align: center"
					colspan="2">
					<!-- Requests --> Requests</td>
				<td class="td_title" width="45%" style="text-align: center"
					colspan="2">
					<!-- Reviewed --> Reviewed</td>
			</tr>
			...
			<c:forEach items="${checkList}" var="checkor" varStatus="i">
				<tr>
					<td class="td_type" width="10%" style="text-align: center"
						>Public</td>
					<td class="td_type" width="35%" style="text-align: left"
						colspan="2">
					...
					<td class="td_type" width="45%" style="text-align: left"
						colspan="2">
						[${checkor.CHECK_EMPID}]${checkor.CHECK_NAME}&nbsp;&nbsp;&nbsp;${checkor.CHECK_POSITION}
						&nbsp;&nbsp;(${checkor.CHECK_DEPT_NAME})&nbsp;/&nbsp;${checkor.CHECKED_DATE}&nbsp;
						...
						<c:if test="${checkor.ESS_CHECK_NO eq check_no}">
							<c:if test="${checkor.CHECKOR_ID eq personId}">
								<input type="hidden" id="ESS_CHECK_NO" name="ESS_CHECK_NO"
									value="${check_no}" /> 
								<input type="hidden" id="ESS_AFFIRM_NO"
									name="ESS_AFFIRM_NO" value="${checkor.ESS_AFFIRM_NO }" /> 
								<input id="pro_flag" name="pro_flag" type="hidden" value="0" /> 
								<input id="ADMIN_ID" name="ADMIN_ID" type="hidden" value="${ADMIN_ID }" />
								...
								<input id="APPLY_NO" name="APPLY_NO" type="hidden" value="${applyorInfo.APPLY_NO}" />
			...
			<c:if test="${checkorListCnt == 0}">
				<tr>
					<td class="td_type" width="15%" style="text-align: center"
						>Public</td>
					<td class="td_type" width="30%" style="text-align: left"
						colspan="2">无</td>
					<td class="td_type" width="45%" style="text-align: left"
						colspan="2">无</td>
				</tr>
