...
function exportByDeptJV(a, navTabId) {
	var suffix = $('input[name="suffix"]:checked').val();
	var report = $('input[name="reportType"]:checked').val();
	if(suffix == 'xls' && report == 'save'){
		document.getElementById("rpPayExpenseByDeptJVLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		var eForm = document.getElementById("exportPayExpenseByDeptJV"); 
		...
		$("#rp_PayExpenseByDeptJV").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac04/exportDatilyReport2"
				+"&navTabId=rpt0102"
				+"&formId=exportPayExpenseByDeptJV");
		$("#rp_PayExpenseByDeptJV").attr('width', "300");
		$("#rp_PayExpenseByDeptJV").attr('height', "150");
		$("#rp_PayExpenseByDeptJV").click();
		...
		$.pdialog.open("/report/common/showPDFPop?"
				...
				, "showPDFPop", "工厂部门费用报表",options);
	}
}
</script>
<div class="pageHeader">
	<a id="rp_PayExpenseByDeptJV" href="#" target="dialog" mask="true"><span
		id="rpPayExpenseByDeptJVLink" style="display: none"></span></a> 
	<form action="/report/pac04/exportDatilyReport2" id="onlyForm" rel="htmlReport" method="post">
		<input type="hidden" id="reportName" name="reportName" value="payExpensebyDepartmentJV${defaultCpny}" />
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" onClick="exportByDeptJV(this,'${param.navTabId}')">
									查询
								</button>
							</div>
						</div>
	...
	<form id="exportPayExpenseByDeptJV" name="exportPayExpenseByDeptJV" method="post">
		...
		<input type="hidden" id="reportName" name="reportName" value="payExpensebyDepartmentJV" />
