...
function exportWelTypeDetailLGEHZ(a, navTabId) {
	var suffix = $('input[name="suffix"]:checked').val();
	var report = $('input[name="reportType"]:checked').val();
	if(suffix == 'xls' && report == 'save'){
		
		document.getElementById("rpwelTypeDetailListLGEHZLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		var eForm = document.getElementById("exportWelTypeDetailListLGEHZ"); 
		...
		$("#rp_welTypeDetailListLGEHZ").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac08/exportDatilyReport2"
				+"&navTabId=rpt0102"
				+"&formId=exportWelTypeDetailListLGEHZ");
		$("#rp_welTypeDetailListLGEHZ").attr('width', "300");
		$("#rp_welTypeDetailListLGEHZ").attr('height', "150");
		$("#rp_welTypeDetailListLGEHZ").click();
	}else if(suffix == 'pdf'  && report == 'save'){
		//$("#result").attr("src","");
		$("#onlyForm").attr("target","");
		$("#onlyForm").attr("action","/report/pac08/exportDatilyReport2");
		$("#onlyForm").submit();
		...
		$.pdialog.open("/report/common/showPDFPop?"
				+"params="+param
                +"&actionUrl="+"/report/pac08/exportDatilyHtmlReport2"
				, "showPDFPop", "部门工资明细",options);
	}
}
</script>
<div class="pageHeader">
	<a id="rp_welTypeDetailListLGEHZ" href="#" target="dialog" mask="true"><span
		id="rpwelTypeDetailListLGEHZLink" style="display: none"></span></a> 
	<form action="/report/pac08/exportDatilyReport2" id="onlyForm" rel="htmlReport" method="post">
		...
		<div class="searchBar">
			<table class="searchContent">
				...
				<td>人员类型 </td>
						<td>
		 	<ait:SelectEmpTypeCode id="ess0204_seach_EmpTypeCodeNo" name="seach_JOB_TP" selected="${EmpTypeCodeNo}" limit="pa"/>
						</td>
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" onClick="exportWelTypeDetailLGEHZ(this,'${param.navTabId}')">
									查询
								</button>
							</div>
						</div>
	...
	<form id="exportWelTypeDetailListLGEHZ" name="exportWelTypeDetailListLGEHZ" method="post">
