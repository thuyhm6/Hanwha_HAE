...
var sform = document.getElementById("onlyFormYuTiPromtr");
...
$("#PA_MONTH").attr('value',sform.seach_YEAR.value+''+sform.seach_MONTH.value);
		...
		alertMsg.confirm("确定要导出数据吗?", {
			okCall : function() {
				//用于excel导出的表单参数处理
				document.getElementById("rpYTPromtrLink").innerHTML = "EXCEL密码设置";
				$("#rp_YuTiPayDetailListPromtr").attr('href',"/disc/autoExcel/encryptExcelForExcel"
						+"?exportFunName=/disc/autoExcel/exportLOtImportExcel"
						+"&navTabId=disc0101"
						+"&formId=onlyFormYuTiPromtr");
				$("#rp_YuTiPayDetailListPromtr").attr('width', "300");
				$("#rp_YuTiPayDetailListPromtr").attr('height', "150");
				$("#rp_YuTiPayDetailListPromtr").click();
			}
		});
	}
</script>
<div class="pageHeader">
	<a id="rp_YuTiPayDetailListPromtr" href="#" target="dialog" mask="true"><span
		id="rpYTPromtrLink" style="display: none"></span></a> 
	<form id="onlyFormYuTiPromtr" name="onlyFormYuTiPromtr" method="post" 
		action="/disc/autoExcel/testExcel">
	<!-- action="/report/pac04/exportDatilyReport" id="onlyFormYuTiPromtr" rel="htmlReport" method="post" -->
		<input type="hidden" id="reportName" name="reportName" value="payPayDetailListPromtr" />
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					<input  type= "hidden" id="password" name="password" value=""/>
					<input readonly type="hidden" name="SQL_SEQMEAN" id="SQL_SEQMEAN"   value="161"/>
					<input  readonly type="hidden" name="PGM_NM" id="PGM_NM"   value="PAY"/>
					<input readonly  type="hidden" name="SQL_NM" id="SQL_NM"     value="家电促销职预提工资下载"/>
				...
				<%-- <tr>
					...
					<td colspan="3"><a class="buttonActive" id ="exportLOtImportExcel" onclick="exportPAYtImportExcel('onlyFormYuTiPromtr',DWZ.ajaxDone);" href="#">
					<span><spring:message code="ar.addempshift.title.excelexport"/>导出Excel</span>
				</a>&nbsp;</td>
				</tr> --%>
			</table>
