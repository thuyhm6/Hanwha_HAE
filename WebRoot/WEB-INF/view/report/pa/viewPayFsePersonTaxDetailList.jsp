...
function exportpayPayFSE(a, navTabId) {
	...
	if(suffix == 'xls'){
		document.getElementById("rpPayFsePersonTaxDetailListLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		document.getElementById("onlyForm").reportName.value = 'payPayFsePersonTaxDetailList';
		var eForm = document.getElementById("exportPayFsePersonTaxDetailList"); 
		...
		$("#rp_PayFsePersonTaxDetailList").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac04/exportDatilyReport2"
				+"&navTabId=rpt0102"
				+"&formId=exportPayFsePersonTaxDetailList");
		$("#rp_PayFsePersonTaxDetailList").attr('width', "300");
		$("#rp_PayFsePersonTaxDetailList").attr('height', "150");
		$("#rp_PayFsePersonTaxDetailList").click();
		...
		$.pdialog.open("/report/common/showPDFPop?"
				...
				, "showPDFPop", "FSE工资个人所得税汇总表",options);
		
	}
}

function exportpayPayFSEOnlyLgech(a, navTabId) {
	var suffix = $('input[name="suffix"]:checked').val();
	var report = $('input[name="reportType"]:checked').val();
	if(suffix == 'xls'){
		document.getElementById("rpPayFsePersonTaxDetailListLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		document.getElementById("onlyForm").reportName.value = 'payPayFsePersonTaxDetailListLGECHOnly';
		var eForm = document.getElementById("exportPayFsePersonTaxDetailList"); 
		eForm.CPNY_ID.value	= sform.seach_CPNY_ID.value;
		eForm.YEAR.value 		= sform.seach_YEAR.value;
		eForm.MONTH.value 		= sform.seach_MONTH.value;
...
<div class="pageHeader">
	<a id="rp_PayFsePersonTaxDetailList" href="#" target="dialog" mask="true"><span
		id="rpPayFsePersonTaxDetailListLink" style="display: none"></span></a> 
	<form action="/report/pac04/exportDatilyReport2" id="onlyForm" rel="htmlReport" method="post">
		<input type="hidden" id="reportName" name="reportName" value="payPayFsePersonTaxDetailList" />
		<div class="searchBar">
			<table class="searchContent">
				...
				<tr>
					...
					<td style="text-align:right" >
										<input type="button" onClick="exportpayPayFSEOnlyLgech(this,'${param.navTabId}')" value='TSTO'/>
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
										<input type="button" onClick="exportpayPayFSE(this,'${param.navTabId}')" value="查询"/>
					</td>
	...
	<form id="exportPayFsePersonTaxDetailList" name="exportPayFsePersonTaxDetailList" method="post">
