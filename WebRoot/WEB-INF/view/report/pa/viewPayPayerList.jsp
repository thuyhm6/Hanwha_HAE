...
function exportPayPayerList(a, navTabId) {
	...
	if(suffix == 'xls'){
		...
		$("#importExcelDialog_rp0103Payer").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac04/exportDatilyReport"
				+"&navTabId=rpt0103"
				+"&formId=exportPayPayer_rp0103Payer");
		$("#importExcelDialog_rp0103Payer").attr('width', "300");
		$("#importExcelDialog_rp0103Payer").attr('height', "150");
		$("#importExcelDialog_rp0103Payer").click();
	}else if(suffix == 'pdf'  && report == 'save'){
		$("#onlyForm_Payer").attr("target","");
		$("#onlyForm_Payer").attr("action","/report/pac04/exportDatilyReport");
		...
		$.pdialog.open("/report/common/showPDFPop?"
				...
				, "showPDFPop", "实发工资列表报表",options);
	}
}
</script>
<div class="pageHeader">
	<a id="importExcelDialog_rp0103Payer" href="#" target="dialog" mask="true"><span
		id="rp0103Link" style="display: none"></span></a> 
	<form action="/report/pac04/exportDatilyReport" id="onlyForm_Payer" rel="htmlReport"  method="post">
		<input type="hidden" id="reportName" name="reportName" value="payPayerList" />
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					...
					<td>部门：</td>
					<td>
						<ait:deptList name="seach_ORG_ID" cpnyId="${defaultCpny}" id="viewEmpInfoList_PayerP"/>
			            <ait:deptTreeIcon name="seach_ORG_ID" cpnyId="${defaultCpny}" limit="pa" id="viewEmpInfoList_PayerP" selected="${DEPTNO_TYPE}"/>
					</td>
					<td>人员类型组： </td>
						<td>
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" onClick="exportPayPayerList(this,'${param.navTabId}')">
									查询
								</button>
							</div>
						</div>
	...
	<form id="exportPayPayer_rp0103Payer" name="exportPayPayer_rp0103Payer" method="post">
