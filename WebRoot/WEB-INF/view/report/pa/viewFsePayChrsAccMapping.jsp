...
function exportFsePay(a, navTabId) {
	var suffix = $('input[name="suffix"]:checked').val();
	var report = $('input[name="reportType"]:checked').val();
	if(suffix == 'xls'){
		document.getElementById("rpFsePayChrsAccMappingLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		var eForm = document.getElementById("exportFsePayChrsAccMapping"); 
		...
		$("#rp_FsePayChrsAccMapping").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac04/exportDatilyReport2"
				+"&navTabId=rpt0102"
				+"&formId=exportFsePayChrsAccMapping");
		$("#rp_FsePayChrsAccMapping").attr('width', "300");
		$("#rp_FsePayChrsAccMapping").attr('height', "150");
		$("#rp_FsePayChrsAccMapping").click();
		...
		$.pdialog.open("/report/common/showPDFPop?"
				...
				, "showPDFPop", "FSE人事与财务工资核对表",options);
	}
}
</script>
<div class="pageHeader">
	<a id="rp_FsePayChrsAccMapping" href="#" target="dialog" mask="true"><span
		id="rpFsePayChrsAccMappingLink" style="display: none"></span></a> 
	<form action="/report/pac04/exportDatilyReport2" id="onlyForm" rel="htmlReport" method="post">
		<input type="hidden" id="reportName" name="reportName" value="FsePayChrsAccMapping${defaultCpny}" />
		<div class="searchBar">
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" onClick="exportFsePay(this,'${param.navTabId}')">
									查询
								</button>
							</div>
						</div>
	...
	<form id="exportFsePayChrsAccMapping" name="exportFsePayChrsAccMapping" method="post">
		...
		<input type="hidden" id="reportName" name="reportName" value="FsePayChrsAccMapping" />
