...
<div class="pageHeader">
	...
	<input type="hidden" id="reportName" name="reportName" value="payPayTeamSumLGETRT1" />
		<div class="searchBar">
		    <table class="searchContent">
				<tr>
					...
					<td><c:if test="${defaultCpny ne'LGEND' and defaultCpny ne'LGEHZ'  and defaultCpny ne'LGETR'}">
						人员类型组：
						</c:if>
						<c:if test="${defaultCpny eq'LGEND' or defaultCpny eq'LGEHZ' or defaultCpny eq 'LGETR'}">
						&nbsp;人员类型：
						</c:if></td>
					<td>
						<c:if test="${defaultCpny eq'LGETR'}">
						<ait:SelectEmpTypeCodeCheckBox name="seach_JOB_TP" selected="${EMP_TYPE_GROUP}" limit="pa" />
						</c:if>
