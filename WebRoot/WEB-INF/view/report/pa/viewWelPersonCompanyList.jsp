...
function exportWelTypeDetail(a, navTabId) {
	var suffix = $('input[name="suffix"]:checked').val();
	var report = $('input[name="reportType"]:checked').val();
	if(suffix == 'xls' && report == 'save'){
		document.getElementById("rpwelTypeDetailListLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		var eForm = document.getElementById("exportWelTypeDetailList"); 
		...
		eForm.TITLE.value 		= sform.seach_TITLE.value;
		eForm.reportName.value 		= sform.reportName.value;
		eForm.suffix.value 		= $('input[name="suffix"]:checked').val();
		$("#rp_welTypeDetailList").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac09/exportDatilyReport2"
				+"&navTabId=rpt0102"
				+"&formId=exportWelTypeDetailList");
		$("#rp_welTypeDetailList").attr('width', "300");
		$("#rp_welTypeDetailList").attr('height', "150");
		$("#rp_welTypeDetailList").click();
	}else if(suffix == 'pdf'  && report == 'save'){
		var sform = document.getElementById("onlyForm");
		//$("#result").attr("src","");
		$("#onlyForm").attr("target","");
		$("#onlyForm").attr("action","/report/pac09/exportDatilyReport2"+"?TITLE="+sform.seach_TITLE.value);
		$("#onlyForm").submit();
        ...
        +"@TITLE="+sform.seach_TITLE.value
        +"@suffix="+$('input[name="suffix"]:checked').val();
		//打开dialog 需要根据各自的页面更换actionUrl和报表名称 其它值固定
		$.pdialog.open("/report/common/showPDFPop?"
				+"params="+param
                +"&actionUrl="+"/report/pac09/exportDatilyHtmlReport2"
				, "showPDFPop", "福利年度公司个人",options);
	}
}
</script>
<div class="pageHeader">
	<a id="rp_welTypeDetailList" href="#" target="dialog" mask="true"><span
		id="rpwelTypeDetailListLink" style="display: none"></span></a> 
	<form action="/report/pac09/exportDatilyReport2" id="onlyForm" rel="htmlReport" method="post">
		<input type="hidden" id="reportName" name="reportName" value="reportType" />
		...
		<div class="searchBar">
			<table class="searchContent">
				...
				<td width="10%" style="text-align:center">展示类型：</td>
					<td>
					    <select id="seach_TITLE" name="seach_TITLE" style="width:110px">
					         <option value="101010" <c:if test="${TITLE eq '101010' }">selected</c:if> >
					                                  公司承担
					         </option>
					         <option value="121212" <c:if test="${TITLE eq '121212' }">selected</c:if> >
					                                  个人承担
					         </option>
					    </select>
					</td>
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" onClick="exportWelTypeDetail(this,'${param.navTabId}')">
									查询
								</button>
							</div>
						</div>
	...
	<form id="exportWelTypeDetailList" name="exportWelTypeDetailList" method="post">
		...
		<input type="hidden" id="reportName" name="reportName" value="welTypeDetailList" />
