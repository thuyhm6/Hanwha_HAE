...
function exportFsePayDetail(a, navTabId) {
	var suffix = $('input[name="suffix"]:checked').val();
	var report = $('input[name="reportType"]:checked').val();
	if(suffix == 'xls'){
		document.getElementById("rpFsePayDetailChrsAccMappingLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		var eForm = document.getElementById("exportFsePayDetailChrsAccMapping"); 
		...
		$("#rp_FsePayDetailChrsAccMapping").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac04/exportDatilyReport2"
				+"&navTabId=rpt0102"
				+"&formId=exportFsePayDetailChrsAccMapping");
		$("#rp_FsePayDetailChrsAccMapping").attr('width', "300");
		$("#rp_FsePayDetailChrsAccMapping").attr('height', "150");
		$("#rp_FsePayDetailChrsAccMapping").click();
		...
		$.pdialog.open("/report/common/showPDFPop?"
				...
				, "showPDFPop", "FSE工资明细人事财务核对表",options);
	}
}
 
</script>
<div class="pageHeader">
	<a id="rp_FsePayDetailChrsAccMapping" href="#" target="dialog" mask="true"><span
		id="rpFsePayDetailChrsAccMappingLink" style="display: none"></span></a> 
	<form action="/report/pac04/exportDatilyReport2" id="onlyForm" rel="htmlReport" method="post">
		<input type="hidden" id="reportName" name="reportName" value="FsePayDetailChrsAccMapping${defaultCpny}" />
		<div class="searchBar">
			<table class="searchContent">
				<tr>
					...
					<td width="20%">
						...
						<input type="hidden" id="seach_CPNY_ID" name="seach_CPNY_ID" value="${item.CPNY_ID}" />
					</td>
					...
					<td style="text-align:right">人员类型组：</td>
					<td>
						<ait:SelectEmpTypeCode name="seach_JOB_TP" id="seach_JOB_TP" cnpyID="${defaultCpny}" selected="${EMP_TYPE_GROUP}"  limit="pa" type="group" />
					</td>
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" onClick="exportFsePayDetail(this,'${param.navTabId}')">
									查询
								</button>
							</div>
						</div>
	...
	<form id="exportFsePayDetailChrsAccMapping" name="exportFsePayDetailChrsAccMapping" method="post">
		...
		<input type="hidden" id="reportName" name="reportName" value="FsePayDetailChrsAccMapping" />
