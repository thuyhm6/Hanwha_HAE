...
function exportPayDetailListLGECH(a, navTabId) {
	var suffix = $('input[name="suffix"]:checked').val();
	var report = $('input[name="reportType"]:checked').val();
	if(suffix == 'xls' && report == 'save'){
		document.getElementById("payPayDetailListLGECHLink").innerHTML = "设置导出文件密码";
		var sform = document.getElementById("onlyForm");
		var eForm = document.getElementById("exportPayPayDetailListLGECH"); 
		...
		$("#rp_PayPayDetailListLGECH").attr('href', "/sys/encryptExcel"
				+"?exportFunName=/report/pac04/exportDatilyReport2"
				+"&navTabId=rpt0102"
				+"&formId=exportPayPayDetailListLGECH");
		$("#rp_PayPayDetailListLGECH").attr('width', "300");
		$("#rp_PayPayDetailListLGECH").attr('height', "150");
		$("#rp_PayPayDetailListLGECH").click();
...
<div class="pageHeader">
	<a id="rp_PayPayDetailListLGECH" href="#" target="dialog" mask="true"><span
		id="rpPayPayDetailListLGECHLink" style="display: none"></span></a> 
	<form action="/report/pac04/exportDatilyReport2" id="onlyForm" rel="htmlReport" method="post">
		<input type="hidden" id="reportName" name="reportName" value="payPayDetailListLGECH" />
		<div class="searchBar">
					...
					<li>
						<div class="buttonActive">
							<div class="buttonContent">
								<button type="button" onClick="exportPayDetailListLGECH(this,'${param.navTabId}')">
									查询
								</button>
							</div>
						</div>
	...
	<form id="exportPayPayDetailListLGECH" name="exportPayPayDetailListLGECH" method="post">
