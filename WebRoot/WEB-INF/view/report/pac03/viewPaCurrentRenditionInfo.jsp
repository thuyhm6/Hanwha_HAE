...
<script type="text/javascript">
 ...
 function  openPaTranserExecl(a){
      ...
      var $from = $("#viewPaCurrentRenditionInfo");  
      
	  var url ="/report/pac03/viewPaCurrentRenditionTranserExcel";
...
<div class="pageHeader" style="border:1px #B8D0D6 solid">
	 <form id="viewPaCurrentRenditionInfo" method="post" action="viewPaCurrentRenditionTranserExcel">
		<div class="searchBar">
				...
				<tr>
					...
					<td width="25" style="text-align:left" >
						<select id="deptNo" name="deptNo">
							<option value ="C0327" <c:if test="${deptNo eq 'C0327'} }">selected</c:if>>店铺</option>
							<option value ="other" <c:if test="${deptNo eq 'other'} }">selected</c:if>>总部</option>
				<%--			<option value ="C0315" <c:if test="${deptNo eq 'C0315'} }">selected</c:if>>华北地区</option>
							<option value ="C034" <c:if test="${deptNo eq 'C034'} }">selected</c:if>>华东地区</option>--%>
						</select>
					</td>
				</tr>
			</table>
