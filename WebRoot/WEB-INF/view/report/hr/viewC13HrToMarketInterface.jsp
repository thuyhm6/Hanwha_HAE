...
//C13 hrto销售系统
function makeC13PortalSap() {
	$.ajax({
		type: 'POST',
		cache: false,
		url: '/report/hr/addC13DateForInterface',
...
//C13 hrto考勤食堂 部门
function makeC13PortalSapForMessDept() {
	$.ajax({
		type: 'POST',
		cache: false,
		url: '/report/hr/addC13DateForMessDeptInterface',
		success : function(responseText) {
				if (responseText == "Y"){
					alert('<spring:message code="alert.message.pa.salary.title.generateSAPHasSuccesed"/>');
				}else{
...
//C13 hrto考勤食堂 人事
function makeC13PortalSapForMessUserInfo() {
	$.ajax({
		type: 'POST',
		cache: false,
		url: '/report/hr/addC13DateForMessUserInfoInterface',
		success : function(responseText) {
				if (responseText == "Y"){
					alert('<spring:message code="alert.message.pa.salary.title.generateSAPHasSuccesed"/>');
				}else{
...
//C13 hrtosap人事信息
function makeC13PortalForSapPersonnelInfo() {
	$.ajax({
		type: 'POST',
		cache: false,
		url: '/report/hr/addC13DateForSapPersonnelInfoInterface',
		success : function(responseText) {
				if (responseText == "Y"){
					alert('<spring:message code="alert.message.pa.salary.title.generateSAPHasSuccesed"/>');
				}else{
...
//C13 hrtosap奖金
function makeC13PortalForSapMoneyAward() {
	$.ajax({
		type: 'POST',
		cache: false,
		url: '/report/hr/addC13DateForSapMoneyAwardInterface',
		success : function(responseText) {
				if (responseText == "Y"){
					alert('<spring:message code="alert.message.pa.salary.title.generateSAPHasSuccesed"/>');
				}else{
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/report/hr/viewC13HrToMarketInterface" method="post" rel="pagerForm" >
	<div class="searchBar">
		...
		<ul class="toolBar">
			...
			<li>
				<a class="l-button"
					...
					onclick="makeC13PortalSapForMessDept()">
			...
			<!-- <li>
				<a class="l-button"
					style="width: 120px; float: left; margin-left: 10px;"
					onclick="makeC13PortalSapForMessUserInfo()">
					HRTO考勤食堂 人事
				</a>
			</li> 
			 -->
			<li>
				<a class="l-button"
					style="width: 120px; float: left; margin-left: 10px;"
					onclick="makeC13PortalForSapPersonnelInfo()">
			...
			<li>
				<a class="l-button"
					style="width: 120px; float: left; margin-left: 10px;"
					onclick="makeC13PortalForSapMoneyAward()">
					HRTOSAP奖金
				</a>
			</li> 
			 -->
