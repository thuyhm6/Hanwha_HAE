...
<body>
   <%
		...
		response.setHeader("Content-Disposition", "attachment; filename=GongZiFaFangBiao(ZhengShi).xls");
    	...
    	<table width="45%" border="0" cellpadding="0" cellspacing="0" 
				...
				<c:forEach items="${viewPaOfficialPayOffList}" var="item" varStatus="i">
					<c:if test="${item.DEPTNO == temp.DEPTNO}">
						...
						<tr>
							...
							<td>${item.MEDICAL_PERSONAL }</td>
							<td>${item.ENDOWMENT_PERSONAL }</td>
							<td>${item.UNEMPLOYMENT_PERSONAL }</td>
							<td>${item.HOUSE_FUNDING_PERSONAL }</td>
				...
				<c:forEach items="${viewPaOfficialPayOffSumList}" var="sum">
					<c:if test="${sum.DEPTNO== temp.DEPTNO}">
						<tr>
							...
							<td>${sum.MEDICAL_PERSONAL }</td>
							<td>${sum.ENDOWMENT_PERSONAL }</td>
							<td>${sum.UNEMPLOYMENT_PERSONAL }</td>
							<td>${sum.HOUSE_FUNDING_PERSONAL }</td>
			...
			<c:forEach items="${viewPaOfficialPayOffZongJiList}" var="zong">
					<tr>
						...
						<td>${zong.MEDICAL_PERSONAL }</td>
						<td>${zong.ENDOWMENT_PERSONAL }</td>
						<td>${zong.UNEMPLOYMENT_PERSONAL }</td>
						<td>${zong.HOUSE_FUNDING_PERSONAL }</td>
