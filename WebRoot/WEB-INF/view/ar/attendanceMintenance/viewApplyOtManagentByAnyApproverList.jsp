...
$(document).ready(function(){
	$("#viewApplyOtManagentByAnyApproverList_Serch",navTab.getCurrentPanel()).click(function(){
		$("#viewApplyOtManagentByAnyApproverList",navTab.getCurrentPanel()).submit();
   });
	 $('#otApplyAffirmTable',navTab.getCurrentPanel()).on( 'draw.dt', function () {
			initEditFun_ar0708();
			init_ar0708();
		} );
	$(".orderList",navTab.getCurrentPanel()).dataTable({		
		...
		"bScrollInfinite":true,
	     ...
	     "deferRender":true,
	        "columnDefs": [//自定义排序类型
		                     { "orderable": false, "targets": [1,13,15,14,16] }
	                     ],
...
function init_ar0708(){
	var ids= $("input[name='c1']",navTab.getCurrentPanel());
			...
			$.ajax({
				...
				success: function(data){ //请求成功后处理函数。
					if(data.statusCode=="200"){
						navTabSearch($("#viewApplyOtManagentByAnyApproverList"));
...
function addOtApplyAffirm(){
	 $.ajax({
			...
			dataType:null,
			success:  function(data){ //请求成功后处理函数。
				navTab.reload('/ar/attendanceMintenance/viewApplyOtManagentByAnyApproverList?type=add&firstFlag=N&seach_START_DATE=${START_DATE}&seach_END_DATE=${END_DATE}');
	   	 	}  ,
			error: DWZ.ajaxError});
}
//社号姓名框
function submitKeyClick_OtAffirm(obj,index){
	...
	var personIdStr="applyOtManagementApprover";
	if(empid != ''){
	   	$.ajax({
			type: 'POST',
...
//点击全部反应获取每个人信息
function otAffirm_fillItem_ar0708(){
...
function otAffirm_callength(index,FLAG){
	...
	var deduct_yn = 0;
	var usecar_yn = 0;
	var apply_ot_date = $("#APPLY_OT_DATE_"+index,navTab.getCurrentPanel()).html().replaceAll('-','.');
	apply_ot_date = apply_ot_date.substring(6,10)+"."+apply_ot_date.substring(3,5)+"."+apply_ot_date.substring(0,2);
	var ot_from_date = $("#OT_FROM_DATE_"+index,navTab.getCurrentPanel()).html();
	ot_from_date = ot_from_date.substring(6,10)+"."+ot_from_date.substring(3,5)+"."+ot_from_date.substring(0,2);
	...
	var otFromTime = ot_from_date + " " + $("#OT_FROM_TIME_"+index,navTab.getCurrentPanel()).html();
	var otToTime = ot_to_date + " " + $("#OT_TO_TIME_"+index,navTab.getCurrentPanel()).html();
	var apply_person_id = $("#EMPID_"+index,navTab.getCurrentPanel()).attr('sysPersonId');
	var length = " ";
	if(apply_ot_date.substring(0,1)=="<"||apply_ot_date==''){
		alertMsg.error("<spring:message code='ar.viewArOvertimeManagentFast.QINGXUANZEZHENGQUERIQI.b'/>");//请选择正确日期
	...
	if($("#USECAR_YN_"+index,navTab.getCurrentPanel()).attr("checked")=='checked'){
		usecar_yn = 1;
	}
	...
	$.ajax({
		 ...
		 async:false,
		 url: '/ess/infoApply/getValidateInfo',
		 data:{"CNPY_ID":'${LoginUser.cpnyId}',"PERSON_ID":apply_person_id,"apply_ot_date":apply_ot_date,"otFromTime":otFromTime,"otToTime":otToTime,"apply_person_id":apply_person_id,"DEDUCT_YN":deduct_yn,"USECAR_YN":usecar_yn},
		 dataType:"json",
		 success: function(data) {
			 length = data.result[0].OT_LENGTH;
			 ...
			 if(data.result[0].DATETYPE == '1440'){
					...
					if(FLAG==1){
						...
						$("#OT_APPLY_HOUR_TEXT_"+index,navTab.getCurrentPanel()).html(length+"<spring:message code='ar.viewitemparameter.title.xiaoshi' />");
						$("#OT_APPLY_HOUR_"+index,navTab.getCurrentPanel()).val(length);
						$("#SHIFT_START_TIME_"+index,navTab.getCurrentPanel()).html(shiftStartTime);
						$("#SHIFT_END_TIME_"+index,navTab.getCurrentPanel()).html(shiftEndTime);
						$("#INDOOR_TIME_"+index,navTab.getCurrentPanel()).html(data.result[0].INDOOR_TIME);
...
function getAffirmor_ar0708(index){
	var htm = '';
	var applyNo = $("#c1_"+index,navTab.getCurrentPanel()).val();
...
function getOt_Totail(index){
	...
	$.ajax({
		...
		data:{sql:"select GET_AR_OT_TOTAIL(replace('" + APPLY_OT_DATE_FORMAT + "','-','/'),'${LoginUser.cpnyId}','" + PERSON_ID + "','141444') WEEKDAY_OT_TOTAIL," +
			...
			"GET_AR_OT_TOTAIL(replace('" + APPLY_OT_DATE_FORMAT + "','-','/'),'${LoginUser.cpnyId}','" + PERSON_ID + "','141447')  SATURDAY_OT_TOTAIL," +
		...
		success: function(data){
			...
			var saturday_ot_totail = data.result[0].SATURDAY_OT_TOTAIL;
			...
			$("#SATURDAY_OT_TOTAIL_"+index,navTab.getCurrentPanel()).html(saturday_ot_totail);
...
function changeURL_ar0708(applyNo){
	var href = "/ess/infoApply/viewApprovaledOtInfo?seach_APPLY_NO=" + applyNo;
	$.pdialog.open(href,"ar0708", "<spring:message code='ar.viewApplyAttenanceManagentInfoList.MINGXICHAKAN.b'/>", {width:1000,height:600,mask:true});//明细查看
}
</script>
<div class="panel"><h1><!-- 加班管理 --><spring:message code="ar.viewArNavigationPage.JIABANGUANLI.b"/></h1></div>
<div   id="viewApplyOtBatch"  class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ar/attendanceMintenance/viewApplyOtManagentByAnyApproverList?firstFlag=N" method="post"
		id="viewApplyOtManagentByAnyApproverList" name="viewApplyOtManagentByAnyApproverList">
		...
		<div class="searchBar">
				...
				<tr>
					...
					<td>
						<ait:deptList name="seach_DEPTNO" cpnyId="${defaultCpny}" limit="ar" id="viewOtAffirmLBatchList_seachDept" selected="${DEPTNO}"/>
						<ait:deptTreeIcon name="seach_DEPTNO" cpnyId="${defaultCpny}" limit="ar" id="viewOtAffirmLBatchList_seachDept" selected="${DEPTNO}"/>
					</td>
...
<div id="viewApplyAttenBatch" class="pageHeader" >
    <div class="searchBar">
			...
			<div class="subBar">
				<ul class="toolBar">
	             <li>
	             	<a class="buttonActive" onclick="otAffirm_fillItem_ar0708();"><span><!-- 全部反应 --><spring:message code="hrm.approve.ALL_REACTION"/></span></a>
	             </li>
				</ul>
			</div>
	</div>
...
<div class="formBar">
	<ul class="toolBar">
	    <li><a class="buttonActive" id="viewApplyOtManagentByAnyApproverList_Serch" href="#"><span><!-- 查询 --><spring:message code="hrm.empinfo.QUERY"/></span></a></li>
		...
		<li><a class="buttonActive" onclick="saveOtApplyAffirm()"><span><!-- 保存 --><spring:message code="org.title.SAVE"/></span></a></li>
		<li><a class="buttonActive" onclick="downloadExcel('viewApplyOtManagentByAnyApproverList','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=130','/ar/attendanceMintenance/viewApplyOtManagentByAnyApproverList?firstFlag=N')" ><span><!-- 导出到Excel --><spring:message code="org.title.exportLOtImportExcel"/></span></a></li>	
	</ul>
</div>
	<form name="delOtApplyAffirmForm" id="delOtApplyAffirmForm" method="post" action="/ess/infoApply/delLOvertimeApplyInBatch" 
	 ...
	 <%--  <div class="user_table" style="font:bold 12px/20px arial,sans-serif;">Total:${nullOTTSTOAffirmListCnt}</div>    --%>           
		<table class="orderList" width="4000px" id="otApplyAffirmTable">
			<thead>
			    <tr>
					...
					<th rowspan="2" >
				    	<input type="checkbox" class="checkboxCtrl" group="c1" width="5px"/>
				    </th>
					<th rowspan="2" width="180px"><!--申请人-->
					...
					<th rowspan="2" width="100px" class="titleColor"><!-- 加班日期 --><spring:message code="ess.infoApply.attendance_date"/>
					</th>
					<th colspan="13" ><!--申请--><spring:message code="ess.infoApply.title.apply"/>
					</th>
					<th rowspan="2" width="340px;" class="titleColor"><!--原因--><spring:message code="ess.infoApply.Reason"/>
					</th>
					<th rowspan="2" width="600px"><!-- 审批者--><spring:message code="hrm.contractInfo.APPROVAL_PERSON"/>
					</th>
					<th rowspan="2" width="160px;"><!-- 审批状态--><spring:message code="ess.affirmApply.title.remark.shenpizhuangtai"/>
					</th>
					<th colspan="6"><!--加班累计--><spring:message code="ess.title.JIABANLEIJI"/>
					</th>
					<%-- <th colspan="2">
						<!--加班上限--><spring:message code="ar.viewAdjustLeaveTSTOBatchList.JIABANSHANGXIAN.b" />
					</th> --%>
					<th style="text-align: center" rowspan="2" width="220px"><!-- 创建人--><spring:message code="ar.viewApplyAttenanceManagentInfoList.CHUANGJIANREN.b"/>
					</th>
					<th style="text-align: center" rowspan="2" width="80px"><!-- 创建日期--><spring:message code="pa.salary.canShu.chuanJianRiQi"/>
					</th>
					<th style="text-align: center" rowspan="2" width="220px"><!-- 修改者--><spring:message code="ar.viewApplyAttenanceManagentInfoList.XIUGAIZHE.b"/>
					</th>
					<th style="text-align: center" rowspan="2" width="100px"><!-- 修改日期--><spring:message code="ar.viewApplyAttenanceManagentInfoList.XIUGAIRIQI.b"/>
					</th>
				</tr>
				<tr>
					<%-- <th width="160px">
			...
			<tbody>
                 <c:forEach items="${nullOTTSTOAffirmList}" var="otApply" varStatus="i">
					<tr target="sid" rel="${admin.personId}">
					    ...
					    <td style="text-align: center; cursor: pointer; color: blue;" id="LOCAL_NAME_${i.index}" sysIndex="${i.index}" <c:if test='${otApply.PERSON_ID ne null}'> onclick='javascript:changeURL_ar0708(${otApply.APPLY_NO });'</c:if> >${otApply.LOCAL_NAME}</td>
					   ...
					   <%-- <td  style="text-align: center">
					    ...
					    <td  style="text-align: center" id="OT_APPLY_HOUR_TEXT_${i.index}">
					   		 ${otApply.OT_APPLY_HOUR}<!-- 小时-->&nbsp<spring:message code="ar.viewsummaryparameteritem.title.hour"/>
					    </td>
					    ...
					    <td  style="text-align: center">
					      ...
					      <%--&nbsp;<c:if test="${otApply.CONFIRM_FLAG ne '1' and otApply.CONFIRM_FLAG ne '2' }"><!-- 人事未确认--><spring:message code="ess.title.WEIQUEREN"/></c:if><c:if test="${otApply.CONFIRM_FLAG eq '1' }"><!-- 人事通过--><spring:message code="ess.title.RENSHITONGGUO"/></c:if><c:if test="${otApply.CONFIRM_FLAG eq '2' }"><!-- 人事否决--><spring:message code="ess.title.RENSHIFOUJUE"/></c:if>--%>
					    </td>	
					    ...
					    <td  style="text-align: center" id="SATURDAY_OT_TOTAIL_${i.index}">
					      ${otApply.SATURDAY_OT_TOTAIL}
					    </td>
