...
function validateArCardRecordApplyCallback(form,callback) {	
	...
	if($("#viewAddPaTempSalesFLAG").val() == 2 ){
	   url = $form.attr("action","/ar/attendanceMintenance/arOFF?FLAG=2");
	}else{
	   url = $form.attr("action","/ar/attendanceMintenance/arOFF?FLAG=0");
	} 
...
function saveTempSalary(flag){
		...
		<form id="addArCardRecordApplyView" method="post" action="/ar/attendanceMintenance/arOFF" class="pageForm required-validate" 
