...
$(document).ready(function(){
	...
	$(".orderList",navTab.getCurrentPanel()).dataTable({		
		...
		"bScrollInfinite":true,
	     ...
	     "deferRender":true,
	        "columnDefs": [//自定义排序类型
		                     { "orderable": false, "targets": [1,13,15,14] }
	                     ],
	     "fixedColumns":false,
	    "oLanguage": {//多语言配置
	        //正在加载中......
...
function addOtApplyAffirm(){
	 $.ajax({
			...
			dataType:null,
			success:  function(data){ //请求成功后处理函数。
				navTab.reload('/ar/attendanceMintenance/viewArOvertimeManagent_fast?type=add&firstFlag=N&seach_START_DATE=${START_DATE}&seach_END_DATE=${END_DATE}');
	   	 	}  ,
			error: DWZ.ajaxError});
}
							...
							success: function(data){ //请求成功后处理函数。
								if(data.statusCode=="200"){
									for(var i=0;i<data.nullOTTSTOAffirmList.length;i++){
										...
										var tb2 = document.getElementById("otApplyAffirmTable");
										var newFlag = 0;
										var htm = "";
                                       var ids = document.getElementsByName("c1");
...
//点击全部反应获取每个人信息
function otAffirm_fillItem_ar0701(){
		 ...
		 success: function(data) {
			 ...
			 if(data.result[0].DATETYPE == '1440'){
					...
					if(FLAG==1){
						...
						$("#OT_APPLY_HOUR_TEXT_"+index,navTab.getCurrentPanel()).html(otShiftLength+"小时");
						...
						$("#OT_APPLY_HOUR_TEXT_"+index,navTab.getCurrentPanel()).html(length+"小时");
						$("#OT_APPLY_HOUR_"+index,navTab.getCurrentPanel()).val(length);
					 }
				}else{
					$("#DEDUCT_YN_"+index,navTab.getCurrentPanel()).attr("checked",false);
...
function getAffirmor_ar0701(index){
	var htm = '';
...
function changeURL_ar0701(applyNo){
	var href = "/ess/infoApply/viewApprovaledOtInfo?seach_APPLY_NO=" + applyNo;
	$.pdialog.open(href,"ar0701", "<spring:message code='ar.viewApplyAttenanceManagentInfoList.MINGXICHAKAN.b'/>", {width:1000,height:600,mask:true});//明细查看
}
</script>
...
<div   id="viewApplyOtBatch"  class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/ar/attendanceMintenance/viewArOvertimeManagent_fast?firstFlag=N" method="post"
...
<div id="viewApplyAttenBatch" class="pageHeader" >
    <div class="searchBar">
			...
			<div class="subBar">
				<ul class="toolBar">
	             <li>
	             	<a class="buttonActive" onclick="otAffirm_fillItem_ar0701();"><span><!-- 全部反应 --><spring:message code="hrm.approve.ALL_REACTION"/></span></a>
	             </li>
				</ul>
			</div>
	</div>
...
<div class="formBar">
	<ul class="toolBar">
	    <li><a class="buttonActive" id="viewPiciOtAffirmLBatchList_Serch" href="#"><span><!-- 查询 --><spring:message code="hrm.empinfo.QUERY"/></span></a></li>
		...
		<li><a class="buttonActive" onclick="downloadExcel('viewPiciOtAffirmLBatchList','/disc/autoExcel/exportLOtImportExcel?SQL_SEQMEAN=130','/ar/attendanceMintenance/viewArOvertimeManagent_fast?firstFlag=N')" ><span><!-- 导出到Excel --><spring:message code="org.title.exportLOtImportExcel"/></span></a></li>	
	</ul>
</div>
	<form name="delOtApplyAffirmForm" id="delOtApplyAffirmForm" method="post" action="/ess/infoApply/delLOvertimeApplyInBatch" 
	 ...
	 <%--  <div class="user_table" style="font:bold 12px/20px arial,sans-serif;">Total:${nullOTTSTOAffirmListCnt}</div>    --%>           
		<table class="orderList" width="3800px" id="otApplyAffirmTable">
			<thead>
			    <tr>
					...
					<th rowspan="2" width="340px;" class="titleColor"><!--原因-->
					...
					<th style="text-align: center" rowspan="2" width="220px">
						<!-- 创建人--><spring:message code="ar.viewApplyAttenanceManagentInfoList.CHUANGJIANREN.b"/>
					</th>
					<th style="text-align: center" rowspan="2" width="80px">
					...
					<th style="text-align: center" rowspan="2" width="100px">
			...
			<tbody>
                 <c:forEach items="${nullOTTSTOAffirmList}" var="otApply" varStatus="i">
					<tr target="sid" rel="${admin.personId}">
					    ...
					    <td style="text-align: center; cursor: pointer; color: blue;" id="LOCAL_NAME_${i.index}" sysIndex="${i.index}" <c:if test='${otApply.PERSON_ID ne null}'> onclick='javascript:changeURL_ar0701(${otApply.APPLY_NO });'</c:if> >${otApply.LOCAL_NAME}</td>
					   ...
					   <td  style="text-align: center">
					    ...
					    <td  style="text-align: center" id="OT_APPLY_HOUR_TEXT_${i.index}">
					   		 ${otApply.OT_APPLY_HOUR}<!-- 小时--><spring:message code="ar.viewsummaryparameteritem.title.hour"/>
					    </td>
