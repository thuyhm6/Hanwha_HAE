...
function baocunresu(){
	$('#alreadyTrainResultInfo').submit();
	$('#resuweikao_'+"${BASIC_NO }").attr('style','color:red;');
	$('#resuweikaoping_'+"${BASIC_NO }").html("<spring:message code='edu.teacherEvaluate.YIKAOPING.a'/>");//已考评
 ...
 * 删除附件
 */
function deleteAttListAlreadyResu(id,val,callback){
	var fileNosStr="";
	var flag=false;
	...
	return false;
		...
		<form method="post" id="alreadyTrainResultInfo" action="/edu/traineducation/updateTrainResultInfo" class="pageForm required-validate" onsubmit="return validateCallback(this,dialogAjaxDone);">
		<div class="pageContent" layoutH="10" >
				...
				<td colspan='3' class="td_type" width="1%">${alreadyTrainResultInfo.IMPLE_CLASS_HOUR}个${alreadyTrainResultInfo.IMPLE_CLASS_UNIT_NAME}</td>
				</c:if>
				<c:if test="${alreadyTrainResultInfo.IMPLE_CLASS_HOUR==''}">
				...
				<td class="td_type" width="1%" colspan='8'>${alreadyTrainResultInfo.TRAIN_CONTENT}</td>
				</tr>
				<tr>
				...
				<td class="td_type"  width="20%" colspan='7'>
				   <a style="color:blue"  href="#" onclick="uploadAttDialog_new('alreadyTrainResultInfo','/edu/traineducation/trainResultInfo?PERSON_ID=${PERSON_ID}$RESULT_NO=${trainResultInfo.RESULT_NO }','${trainResultInfo.RESULT_NO }','eduTrainResult','<spring:message code="js.upload.msg.saveAndUpload" />','<spring:message code="js.upload.msg.fileToUpload" />')"><span><spring:message code="hrm.empinfo.upload"/><!--上传--></span></a>
				   <a style="color:blue" href="#" onclick="deleteAttListAlreadyResu('alreadyTrainResultInfo','/edu/traineducation/trainResultInfo?PERSON_ID=${PERSON_ID}&RESULT_NO=${trainResultInfo.RESULT_NO }',divAjaxDone)"><span><spring:message code="ess.empInfo.Delete"/><!--删除--></span></a>
