...
function baocun1(){
	...
	var localname="";
	 $("#tearcherTable input[name=teaCheck]").each(function(){ //遍历table里的全部checkbox
		 ...
		 $('#actname').html(localname);
         $('#ACT_EMPLOYEE_NAME').attr('value',localname);
         $('#ACT_EMPLOYEE_EMPID').attr('value',empid);
         $('#actcount').html(splitMethod(empid)+splitMethod("${checkotherplanempid}"));
...
<div class="pageContent" layoutH="10">
		...
		<span>Total:${planEmployeeListCount }</span>
		<tr>
		...
		<c:forEach items="${planEmployeeList}" var="d" varStatus="i">
		<tr onclick="childCheck1('${d.EMPID }')">
		<td class="td_type"  width="5%" ><input type="checkbox" onclick="childCheck1('${d.EMPID }')" id="teaCheck_${d.EMPID }" name="teaCheck" value="${d.EMPID }" valuename="${d.LOCAL_NAME }">${i.count }</td>
