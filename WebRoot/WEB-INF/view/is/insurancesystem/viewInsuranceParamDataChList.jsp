...
function pageFromSea(a){
	var seach_PAY_AREA_CD=$("#seach_PAY_AREA_CD",navTab.getCurrentPanel()).val()==undefined?
			"":$("#seach_PAY_AREA_CD",navTab.getCurrentPanel()).val();
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", "/is/insurancesystem/viewInsuranceParamDataChList?seach_PAY_AREA_CD="+seach_PAY_AREA_CD
			+"&seach_INSRAREA_ID="+seach_INSRAREA_ID+"&seach_INSURE_ID="+seach_INSURE_ID+"&seach_ACTIVITY_FLAG="+seach_ACTIVITY_FLAG);
}
...
function doIsParamDataExport(from){
  	var $from =$(from);
  	var url ="/is/insurancesystem/viewInsuranceParamDataChExcel";
  	window.location =url+ (url.indexOf('?') == -1 ? "?" : "&") + $from.serialize();
}

function expIsParamDataPre(a,navTabId){
  	var $this = $(a);
  	var title = $this.attr("title");
  	var $from = $("#viewInsuranceParamDataChList");
	alertMsg.confirm(title, {okCall: function(){ doIsParamDataExport($from);}}); 
}
//-->
</script>
...
<a id="importExcelDialog_bx0121" href="#" target="dialog" mask="true"></a>
<a id="importExcel_bx0121" href="#" target="navTab" mask="true">
	<span style="display:none;">保险参数导入</span>
</a>
<input type="hidden" value="${defaultCpny}" id="CPNY_ID" name="CPNY_ID"/>
<div class="pageHeader">
  	<form onsubmit="return navTabSearch(this);" action="/is/insurancesystem/viewInsuranceParamDataChList" method="post" rel="pagerForm" 
    	id="viewInsuranceParamDataChList" name="viewInsuranceParamDataChList">
    	<div class="searchBar">
			<table class="searchContent">
		        <tr>
					...
					<td>
						<select name="seach_PAY_AREA_CD" id="seach_PAY_AREA_CD">
					...
					<td>
						...
						<div id="insure_list_panel" class="deptContent_sso" style="display:none;">
							<div class="ztree_dept_sso">
								<div class="ztree_dept_title"> 
									<ul class="ztree_dept_table">
										<li class="deptTreeLi" style="background:#E0D8DC;color:#796C5E;"><div>编号</div><div>项目名称</div></li>
									</ul>
								</div>
								<div class="ztree_dept_type_sso">
...
<div class="pageContent">
	...
	<c:set value="/is/insurancesystem/addIsParamDataInfo" var="add_Url"/>
	<c:set value="dialog" var="edit_tab"/>
	<c:set value="450" var="edit_width"/>
	<c:set value="380" var="edit_height"/>
	<c:set value="/is/insurancesystem/viewEditIsParamDataInfo?DATA_NO={sid}" var="edit_Url"/>
	<div class="formBar">
		<ul class="toolBar">
			...
			<li>
				<a class="delete" id ="exportIsParamDataExcel" onclick="exportIsParamDataModel();" href="#">
			...
			<li>
				<a class="add" onclick="importIsParamData();">
			...
			<li>
				<a class="delete" onclick="expIsParamDataPre(this)" title="<spring:message code='rp.report.title.exportYN'/>">
	...
	<c:set value="/is/insurancesystem/viewInsuranceParamDataChList" var="pageUrl"/>
