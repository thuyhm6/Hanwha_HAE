...
function pageFromSea(a){
	...
	$("#pagerForm",navTab.getCurrentPanel()).attr("action", "/is/insurancesystem/viewInsuranceImportDataNChList?seach_RESULT_FLAG="+seach_RESULT_FLAG);
}
		...
		$.ajax({
		  ...
		  success: function(responseText){
			if (responseText == "Y"){
				//alert("删除成功！");
				//页面重载
				navTabSearch(document.viewInsuranceImportDataNChList);
			}else{
				alert("删除失败！");
			}
		  }
...
--%>
function exportIsParamNChImportExcel(){
	var url = "/is/insurancesystem/viewImportInsuranceParamDataNChExcel?navTabId=bx0122";
	document.getElementById("exportIsParamNChImportExcel").href=encodeURI(url);
}
</script>

<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="/is/insurancesystem/viewInsuranceImportDataNChList" method="post" rel="pagerForm"
		id="viewInsuranceImportDataNChList" name="viewInsuranceImportDataNChList">
	<div class="searchBar" >
...
<div class="pageContent">
		...
		<ul>
			<li>
				<a class="buttonActive" target="ajaxTodo" callback="navTabAjaxDoneRefreshCurrentPage" 
					href="/is/insurancesystem/saveIsParamDataNchImport" title="确定要提交吗?"><span>提交</span>
				</a>
			</li>
			<li>
				<a class="buttonActive" target="ajaxTodo" href="/is/insurancesystem/deleteIsParamDataNchImport" title="确定要全部取消吗?"><span>取消</span></a>
			</li>
			
			<li>
				<a class="buttonActive" onclick="importIsParamDataNoPayArea();">
					<span><spring:message code="ar.addempshift.title.excelimport"/><!-- EXCEL导入 --></span>
				</a>
			</li>
			
			<li>
				<a class="buttonActive" id ="exportIsParamNChImportExcel" onclick="exportIsParamNChImportExcel();" href="#">
					<span><spring:message code="ar.addempshift.title.excelexport"/><%--导出Excel--%></span>
				</a>
			</li>
			<%--<li>
				<a class="buttonActive" id ="exportIsParamDataNoPayAreaExcel" onclick="exportIsParamDataNoPayAreaModel();" href="#">
	...
	<c:set value="/is/insurancesystem/viewInsuranceImportDataNChList" var="pageUrl"/>
