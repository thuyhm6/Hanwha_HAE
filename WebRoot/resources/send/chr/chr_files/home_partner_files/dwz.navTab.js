...
var navTab = {
	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if($tab.data("parentParam") != undefined){
				if(url.indexOf('?')==-1){
					url = url + '?parentParam='+$tab.data("parentParam");
				}else{
					url = url + '&parentParam='+$tab.data("parentParam");
				}
			}
	...
	_reloadWithParentParam: function($tab,parentParam, flag){
		flag = flag || $tab.data("reloadFlag");
		var url = $tab.attr("url");
		
		
		if(url.indexof('?')==-1){
			url = url + '?parentParam='+parentParam;
		}else{
			url = url + '&parentParam='+parentParam;
		}
	...
	reloadFlagWithParentParam: function(tabid,parentParam){
		var $tab = this._getTab(tabid);
		...
		if ($tab){
			if (this._indexTabId(tabid) == this._currentIndex) this._reload($tab, true);
				...
				$tab.data("parentParam", parentParam);
			}
		}
	},
	 ...
	 * @param {Object} params: title, data, fresh
	 */
	openTabPost: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
