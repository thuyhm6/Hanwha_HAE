 ...
 * CODE级联
 **/
function codeRelation(parentCodeValue,sonCodeId,sonValue,typeFlag){
	var sonVal = '';
	if(parentCodeValue != ''){
...
function viewAffirmTarget1_confirm(){
	...
	$("input:[name='EVS_OBJECT_SEQ']",navTab.getCurrentPanel()).each(function(i, obj){
		if($("#evsPoint_" + $(obj).attr("sysIndex"),navTab.getCurrentPanel()).val() == ''){
	...
	if (jsonData.length == 2) {
		alertMsg.info("No need to implement the data"); //没有需要确定的数据
		return;
	}
