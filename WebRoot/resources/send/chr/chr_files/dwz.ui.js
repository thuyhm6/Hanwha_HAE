function initEnv() {
	...
	$("#leftside").jBar({minW:185, maxW:185});
...
function initLayout(){
	var iContentW = $(window).width() - (DWZ.ui.sbar ? $("#sidebar").width() + 0 : 10) - 1;
	var iContentH = $(window).height() - $("#header").height() - $("#footer").height();

	$("#container").width(iContentW);
	$("#container .tabsPageContent").height(iContentH-35).find("[layoutH]").layoutH();
	$("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH+1);
	$("#taskbar").css({top: iContentH + $("#header").height() - 30, width:$(window).width()});
}
...
function initUI(_box){
	...
	if ($.fn.xheditor) {
		$("textarea.editor", $p).each(function(){
			var $this = $(this);
			var op = {html5Upload:false, skin: 'default',tools: $this.attr("tools") || 'full'};
			var upAttrs = [
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			$.pdialog.open(encodeURI(url), rel, title, options);
	...
	$("a[target=ajaxSelect]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
			DWZ.debug(url);
