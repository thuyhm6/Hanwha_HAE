...
SWFUpload.prototype.initSWFUpload = function(settings) {
	try {
		...
		SWFUpload.PATH = {
			...
			DELETE_URL : this.settings.delete_url,
			DELETE_METHOD : this.settings.delete_method,
			UPLOAD_SINGLE_FILE : this.settings.upload_single_file
		};
		this.loadFlash();
...
SWFUpload.prototype.initSettings = function() {
	...
	this.ensureDefault("ctx", "");
	this.ensureDefault("enity_id", "");
	this.ensureDefault("delete_url", "Cs.do");
	this.ensureDefault("delete_method", "deleteFileForCosSwfupload");
	this.ensureDefault("upload_single_file", false);
...
SWFUpload.prototype.cancelUpload = function(fileID, triggerErrorEvent) {
	...
	//var infoTable = document.getElementById("hasFileSiz");
	//if(infoTable.value == null || infoTable.value == ''){
	//	alert("程序出错");
	//	return;
	//}
	//var lastVal = parseInt(infoTable.value) - file.size;
...
SWFUpload.prototype.fileQueued = function(file) {
	...
	var lastVal = parseInt(infoTable.value) - file.size;
	if(lastVal < 0){
		var fileMsg = document.getElementById("fileRemark");
		if(fileMsg){
			alert(fileMsg.innerHTML);
		}else{
			alert("附件总容量共不能超过5M");
		}
		document.getElementById("startUp").style.display = "none";
...
SWFUpload.prototype.fileDialogComplete = function(numFilesSelected,
	...
	var file_path_str = document.getElementsByName("file_path");
	if(file_path_str && (file_path_str.length  + this.getStats().files_queued) > this.settings.file_upload_limit){
		alert('附件不能超过'+this.settings.file_upload_limit+'个');
		document.getElementById("startUp").style.display = "none";
		for(var diNum = 0;diNum < numFilesSelected;diNum++){
			this.cancelUpload();
		}
		return;
	}
...
function FileProgress(file, targetID) {
	...
	if (!this.fileProgressWrapper) {
		...
		var progressBarInProgressContainer = document.createElement("div");
		progressBarInProgressContainer.className = "progressBarInProgressContainer";
		this.fileProgressElement.appendChild(progressCancel);
		this.fileProgressElement.appendChild(progressText);
		this.fileProgressElement.appendChild(progressStatus);
		this.fileProgressElement.appendChild(progressBar);
		this.fileProgressElement.appendChild(progressBarInProgressContainer);
		this.fileProgressWrapper.appendChild(this.fileProgressElement);
		//添加文件大小
		var fileInput = document.createElement("input");
		fileInput.id = this.fileProgressWrapper.id+"_fileSize";
		fileInput.name = this.fileProgressWrapper.id+"_fileSize";
		fileInput.value = file.size;
		fileInput.type = "hidden";
		var fileDiv = document.createElement("div");
		fileDiv.innerHTML = parseInt(file.size/1024)+"KB";
		fileDiv.className = "progressWrapper";
		this.fileProgressElement.appendChild(fileDiv);
		this.fileProgressElement.appendChild(fileInput);
...
FileProgress.prototype.toggleCancel = function(show, swfUploadInstance) {
	...
	if (swfUploadInstance) {
		...
		this.fileProgressElement.childNodes[0].onclick = function() {
			var hasFileSizObj = document.getElementById("hasFileSiz");
			var delObj = document.getElementById(fileID+"_fileSize")
			hasFileSizObj.value = parseInt(hasFileSizObj.value) + parseInt(delObj.value);
