...
function offset(node){
	node=document.getElementById("orgObject");
	...
	this.inIt= function(){
		...
		if(this.BoxWidth!=null){
			if(offset(this.Box).w < this.BoxWidth){
				this.Box.width(this.BoxWidth);
				if(offset(this.Box).w > this.BoxWidth){
					this.Box.width(this.BoxWidth - (offset(this.Box).w - this.BoxWidth)) ;
				}
			}
		}
		...
		if(this.BoxHeight!=null){
			if(offset(this.Box).h < this.BoxHeight){
				this.Box.height(this.BoxHeight);
				if(offset(this.Box).h > this.BoxHeight){
					this.Box.height(this.BoxHeight - (offset(this.Box).h - this.BoxHeight)) ;
				}
			}
		}
...
function OrgShow(OrgNode_){
	...
	this.Run=function(){
		...
		for(var n=0; n<this.Nodes.length; n++){//重新设置距离
			...
			this.Nodes[n].Box.css("left",this.Nodes[n].Left) ;
			this.Nodes[n].Box.css("top",this.Nodes[n].Top) ;
		}
			...
			for(var m=0; m<tempNodes.length; m++){
				...
				if(n!=1){//设置顶线条
					...
					if(this.DepthGroup[tempNodes[m].Depth].NodeGroups[tempNodes[m].NodeGroupId].length==1){//如果只有一个节点
						var tempBottomLineHeight=parseFloat(tempNodes[m].parentNode.BottomLine.height()) + this.LineSize;
						tempNodes[m].parentNode.BottomLine.height(tempLineHeight + tempBottomLineHeight) ;
					}else{
	...
	function CreateLine(type_, top_, left_, long_, size_, color_, id_){
		...
		tempDiv.innerHTML = "<div id=\"" + this.Id + "\" >x</div>" ;
		
		this.Line = $('#' + this.Id);
		
		this.Line.css("position" , "absolute");
		this.Line.css("top" , this.Top) ;
		this.Line.css("left" , this.Left) ;
		this.Line.css("overflow" , "hidden") ;
		
		if(this.Type==1){
			this.Line.css("borderTopColor" , this.Color) ;
			this.Line.css("borderTopWidth" , this.Size) ;
			this.Line.css("borderTopStyle" , "solid" ) ;
			this.Line.width(this.Long);
			this.Line.height(0) ;
		}else{
			this.Line.css("borderLeftColor" , this.Color) ;
			this.Line.css("borderLeftWidth" , this.Size) ;
			this.Line.css("borderLeftStyle" , "solid") ;
			this.Line.height(this.Long);
			this.Line.width(0);
		}
