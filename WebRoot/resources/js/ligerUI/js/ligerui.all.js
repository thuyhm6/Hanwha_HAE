            ...
            var po = {            	
                dataInit: function() {
                    ...
                    else if(g.valueField.val() !="")
                    {
                    	...
                    	if (p.isFirstSelect && p.data != null && p.data.length > 0){
                    		value = p.data[0][p.valueField] ;
                            var text = p.data[0][p.textField] ;
                            po.changeValue(value, text);
                    	}
                    }
            ...
            applyDrag: function() {
                if ($.fn.ligerDrag)
                    g.dialog.ligerDrag({ 
                    		...
                    		onDrag: function(obj){
                            	if((obj.left + obj.diffX) <= 0 || (obj.top + obj.diffY) <= 0){
                            		return false;
                            	}
                            }
                    	}
    ...
    $.ligerAddGrid = function(grid, p)
    {
        ...
        var g = {
            ...
            {  
                ...
                $(data[p.root]).each(function(i, item)
                {
                    ...
                    {  
                        ...
                        if(column.title){ 
                        	gridhtmlarr.push('"><a herf="#" onclick="$.ligerMessageBox.alert(\'\', \'' + content + '\');">' + content + '</a></div>'); 
                        }
                        else{
                        	gridhtmlarr.push('">' + content + '</div>');
                        }
            ...
            { 	
                ...
                else if ($(obj).parent().hasClass("l-grid-row-cell")) {
                	rowcell = $(obj).parent()[0] ;
                };
                
                if (!rowcell) return;
                
                var rowindex = $(rowcell).parent().attr("rowindex");//取得行ID
                ...
                var rowCellWith = $(obj).width() + 1 ;
                var rowCellHeight = $(obj).height() ;
                
                $(obj).html("") ;//清空html数据,修改操作文成,在回填
                if (column.editor && column.editor.type == 'date')
                { 
                    var $inputText = $("<input type='text' id='" + columnname + "edit' />");
                    $(obj).append($inputText);
                ...
                {  
                    var $inputText = $("<input type='text' id='" + columnname + "edit" + "' class='l-text-editing'/>");
                    $(obj).append($inputText);
                    $inputText.val(currentdata);
            ...
            endEdit:function(){
            	...
            	var rows = $(".l-selected", g.gridbody);
            	if(rows.length == 0) return ;
            	
            	var row = rows[0] ;
            	
            	//.l-grid-row-cell-edited
                $.each($(row).children(), function(columnindex, obj){//关闭并清空所有编辑域
                	var column = p.columns[columnindex] ;
                	
                	if(!column.editor) return ;
                	
                	var header = g.headers[columnindex] ;
                	var rowindex = $(obj).parent().attr('rowindex') ;
                	var item = g.getRowByRowIndex(rowindex) ;
                	var columnname = column.name ;
                	
                	var colwidth = header.width ;
                    ...
                    if (column && column.render)
                    {
                        content = column.render(item, rowindex, item[columnname]);
                    }
                    else if (columnname)
                    {
                        if (column.type && column.type == "date")
                        {
                            ...
                            content = item[columnname]==null?" ":item[columnname];
                        }
                        else
                        {
                            content = item[columnname]==null?" ":item[columnname];
            ...
            {    
                ...
                { 
                    ...
                    if(p.allowUnSelectRow || row.hasClass("l-selected-again")){
                    	...
                    	var innerDivList = $(".l-grid-row-cell-inner" , row);
                    	
                    	$.each(innerDivList , function(index, objValue){//循环显示单行所以可以编辑的编辑域
                    		if(p.columns[index].editor){
    ...
    $.fn.ligerLayout = function (p) {
        this.each(function () {
            ...
            if ($("> div[position=left]", g.layout).length > 0) {
                ...
                g.left.header = $('<div class="l-layout-header"><div class="l-layout-header-toggle"></div><div id="leftTitle" class="l-layout-header-inner"></div></div>');
...
(function($)
{
    ...
    $.ligerDefaults.Tab = {
            ...
            changeHeightOnResize: false,
            contextmenu : true,
            closeMessage : getMessage(0),
            closeOtherMessage : getMessage(1),
            closeAllMessage : getMessage(2),
            reloadMessage : getMessage(3),
            ...
            var po = { 
                ...
                {  
                    ...
                    for (var i = 0; i < data.length; i++)
                    {
                        ...
                        if(data[i][p.isexpandFieldName]==false || data[i][p.isexpandFieldName] == "false") isExpandCurrent = false;
                        ...
                        if(isExpandCurrent){
                            treehtmlarr.push('isexpand="' + data[i][p.isexpandFieldName] +'" ');
                        }
                        ...
                        if (g.hasChildren(data[i]))
                        {
                            ...
                            if(p.checkbox)
                            {
                                if(data[i][p.ischeckedFieldName]){
...
(function ($)
{
    ...
    $.fn.ligerApplyWindow = function (p)
    {
        return this.each(function ()
        {
            ...
            if ($.fn.ligerDrag)
            {
                g.window.ligerDrag({ handler: '.l-window-header', 
