...
(function($)
{
    ...
    $.ligerDefaults.Grid = { 
        ...
        selectDate : {},				//缓存下拉框选择数据
        selectLastChoose : {},			//上一次选择数据
        reverseLock : [],			//逆向锁（锁定数组未选的行，只有数组中的行可编辑）
        //获取时间
    ...
    $.ligerAddGrid = function(grid, p)
    {
        ...
        $.each(p.columns,function(index,value){//增加指定列的编辑域
        	if(value.editor)
        	gridhtmlarr.push("                    <div class='l-grid-editor' editName='"+value.name+"edit'></div>");
    	});
        ...
        var g = {
                    ...
                    {  
                        ...
                        else if (this.columnname)
                        {
                            if (column.type && column.type == "date")
                            {
                                ...
                                content = item[this.columnname]==null?" ":item[this.columnname];
                            }
                            else
                            {
            ...
            setRowEven : function(rowobj)
            {
                ...
                }).click(function(e)//对单击事件进行锁判断
                {	var editFlag=false;
                	$.each($("TR",$(".l-grid-body-table", g.gridbody).children()),function(indx,value){
                		if(value.innerHTML==rowobj.innerHTML)
                			if($.inArray(indx+1,g.gridParms.reverseLock)!=-1)
                				editFlag=true;
                	});
                	if(g.gridParms.reverseLock.length>0)
                		if(!editFlag)
                			return null;
                    if(p.checkbox)
                    {
            ...
            { 	
            	editName=$(obj).parent().attr('columnname')+"edit";
                g.grideditor = $("div[editName='"+editName+"']", grid);
                if (obj.href || obj.type) return true;
                var rowcell;//取得TD格
                if ($(obj).hasClass("l-grid-row-cell")) rowcell = obj;
                else if ($(obj).parent().hasClass("l-grid-row-cell")) rowcell = $(obj).parent()[0];
                if (!rowcell) return;
                var row = $(rowcell).parent();//取得行
                var rowindex = $(row).attr("rowindex");//取得行ID
                var columnindex = $(rowcell).attr("columnindex");//取得列顺序ID
                var columnname = $(rowcell).attr("columnname");//取得列名
                var column = p.columns[columnindex];//取得列头部信息
                var left = $(rowcell).offset().left - g.body.offset().left;//得到行的做坐标
                var top = $(rowcell).offset().top - $(grid).offset().top;//得到行的高度
                var rowdata = g.getRowByRowIndex(rowindex);//取得此行的数据
                var currentdata = rowdata[columnname]; //取得此单元格数据
                ...
                {	if(column.selected)//下拉框修改标识初始设置
	                {
	                	$.each($("TD[columnname='"+columnname+"']",$(grid)),function(index,value){
	                		$(value).addClass("l-grid-row-cell-isselected");
	                	});
	                	column.selected=false;
	                }
                	var tempLastSelected={rowID : rowindex,colName : columnname,curData : currentdata}
                	p.selectLastChoose[columnname+rowindex]=tempLastSelected;
                    var $inputText = $("<input type='text' id='"+ editName + "'/>");
                    g.grideditor.append($inputText);
                    ...
                    var options = {
                        ...
                        {
                            g.grideditor.editingValue = newValue;
                            var className=$(rowcell).attr("class");
                            if(className.indexOf("l-grid-row-cell-isselected")!=-1){//缓存下拉框选择原有数据项
                            	var tempSelected={rowID : rowindex,colName : columnname,curData : currentdata}
                            	p.selectDate[columnname+rowindex]=tempSelected;
                            	$(rowcell).removeClass("l-grid-row-cell-isselected");
                            }else if(p.selectDate[columnname+rowindex]&&p.selectDate[columnname+rowindex].curData!=newValue)
                            {
                            	$(rowcell).addClass("l-grid-row-cell-edited");
                            }else
                            {
                            	$(rowcell).removeClass("l-grid-row-cell-edited");
                            }
                            if(typeof(column.onChangeSelected)!='undefined')//执行选择后事件
                        	{
                        		if(p.selectLastChoose[columnname+rowindex]&&p.selectLastChoose[columnname+rowindex].curData!=newValue)
                        			$.extend(column.onChangeSelected,column.onChangeSelected(rowindex,columnname,newValue));
                        	}
                            p.selectLastChoose[columnname+rowindex].curData=newValue;//给最后选择赋值
                    ...
                    var a=$inputText.ligerComboBox(options);
                    g.grideditor.editingCell = rowcell;
                    g.grideditor.show();
                }
                else if (column.editor && (column.editor.type == 'int' || column.editor.type == 'float'))
                {
                    var $inputText = $("<input type='text'/>");
                    g.grideditor.append($inputText);
                    ...
                    $inputText.ligerSpinner(
                            {
                                ...
                                decimalplace: column.editor.decimalplace == null ? 2 : column.editor.decimalplace ,
                                onChangeValue: function(newValue)
                                {
                                    ...
                                    $.extend(column.onValueChange,column.onValueChange(rowindex)); 
                                } 
            ...
            endEdit:function(){
                ...
                $.each($(".l-grid-editor",grid),function(index,value){//关闭并清空所有编辑域
                	$(value).html("").hide();
                });
           //     g.grideditor.html("").hide();
            ...
            getFormatDate: function(date, dateformat)
            {
				...
				if(!(date instanceof Date)){
					date = eval('new Date("' +date.replace('-', '/') + '");');
				}
                var o = {
                    "M+": date.getMonth() + 1,
                    "d+": date.getDate(),
            ...
            {	var tempRow;
            	...
            	tempRow=$("tr[rowindex="+row+"]", g.gridbody);
            	else
            	tempRow=row;
                g.popup.hide();
                g.endEdit();
                ...
                rowindex = tempRow.attr("rowindex");
                $(tempRow).remove();
            ...
            {
            	if(g.currentData[p.root][rowindex][p.statusName]=='add'){
            		g.currentData[p.root][rowindex][p.statusName]='add-delete';
            	}
            	else{
            ...
            { 
                ...
                { 
                    ...
                    if (columnname)
                    {
                    	if (column.editor&& column.editor.type=='select') //是下拉框的单元格默认选择第一个
                    	{
                    		rowdata[columnname] = column.editor.data[0][column.editor.dataValueField];
                    		inputText=column.editor.data[0][column.editor.dataDisplayField];
                    	}else if(columnindex==0)							//将首行填写为复选框
                    	{
                    		rowdata[columnname] = g.currentData[p.root][rowindex-1][columnname];
                    	}
                       	else												//其他行数据清空
                       	{
                       		rowdata[columnname] = "";
                       	}
                        if (column.type && column.type == 'int') rowdata[columnname] = 0;		//数字类型默认为零
                        rowCell.attr({ columnname: columnname });
                    }
                    if (column.editor&& column.editor.type=='select') 
                    {
                    	$(".l-grid-row-cell-inner", rowCell).html(inputText);		//表格显示下拉框信息
                    }else if(columnindex==0)
                    {
                    	$(".l-grid-row-cell-inner", rowCell).html("<input type='checkbox'/>");//多选框
                    }
                    else{
        ...
        $(p.columns).each(function(i, item)
        {
            ...
            // 如果此列为可编辑,应加以符号进行区分,或者加以css样式
            if(item.editor){
            	headerText = headerText + "<font color=red size=1>@</font>" ;
            }
       ...
       $.fn.ligerCheckBox &&  $('input:checkbox', g.popup).ligerCheckBox(
        ...
        g.gridbody.scroll(function(){  
            ...
            $.each($(".l-grid-editor:visible", grid),function(index,value){//重新定位输入框位置
            	var editName=$(value).attr("editName").split("edit")[0];
            	$(value).css("left",$("TD[columnname='"+editName+"']",$(grid)).position().left);
        	});
        }); 
        //表体 - 数据 单元格事件
            ...
            {    
                ...
                { 
                    ...
                    if(p.allowUnSelectRow || row.hasClass("l-selected-again")){
                    	var tempTd;
                    	tempTd=$(".l-grid-row-cell-inner",$(obj).parent().parent());
                    	if(typeof($(".l-grid-row-cell-inner",$(obj)).attr("class"))!='undefined')
                    			tempTd=$(".l-grid-row-cell-inner",$(obj).parent());
                    	$.each(tempTd,function(index,objvalue){//循环显示单行所以可以编辑的编辑域
                    		g.applyEditor(objvalue);
                    	});
                    }
                } 
            }
